...
const page: Page = {
  ...
  init(_uploader: SfxUploader) {
    const allUploader = document.getElementById('all-uploader') as SfxUploader;
    allUploader.config = buildConfig({
      mode: 'inline',
      connectors: {
    ...
    const filteredUploader = document.getElementById('filtered-uploader') as SfxUploader;
    filteredUploader.config = buildConfig({
      mode: 'inline',
      connectors: {
        companionUrl: 'https://eu-on-24001.connector.filerobot.com',
        providers: ['google-drive', 'dropbox', 'onedrive', 'box', 'unsplash'],
        coreSources: ['device', 'url'],
      },
    });
...
export function App() {
  ...
  destroy() {
    const allUploader = document.getElementById('all-uploader') as SfxUploader | null;
    if (allUploader) allUploader.close();
    const filteredUploader = document.getElementById('filtered-uploader') as SfxUploader | null;
    if (filteredUploader) filteredUploader.close();
  },
};
