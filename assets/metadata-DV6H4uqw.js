import{b as F}from"./index-B2xh3pMk.js";import{r as B}from"./code-block-C_3oxnLY.js";const R="sfx-uploader-demo-meta",A={projectUuid:"",enforceRequired:"auto",language:"",sessionToken:"",companyToken:"",projectToken:""};function D(){try{const t=localStorage.getItem(R);if(t)return{...A,...JSON.parse(t)}}catch{}return{...A}}function L(t){localStorage.setItem(R,JSON.stringify(t))}let s=null;function m(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function c(t,n){if(!s)return;const r=new Date().toLocaleTimeString(),d=document.createElement("div"),p=n?` <span class="log-data">${m(JSON.stringify(n,null,0))}</span>`:"";d.innerHTML=`<span class="log-time">${m(r)}</span> <span class="log-event">${m(t)}</span>${p}`,s.appendChild(d),s.scrollTop=s.scrollHeight}const x={render(){return`
