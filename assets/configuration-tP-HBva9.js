    ...
    sassKey: string,
        ...
        <table class="config-table">
          ...
          <tbody>
            ...
            <tr><td><code>allowFolderUpload</code></td><td><code>boolean</code></td><td><code>true</code></td><td>Whether users may upload whole folders. Set to <code>false</code> to hide the "or upload a folder" affordance in every view and reject folders dragged, dropped or pasted onto the drop zone (a brief "folder upload is disabled" hint is shown; loose files dropped alongside a folder still upload). Independent of <code>preserveFolderStructure</code>. See <a href="#/examples/target-folder">Target folder example</a>.</td></tr>
            <tr><td><code>preserveFolderStructure</code></td><td><code>boolean</code></td><td><code>true</code></td><td>When an allowed folder is dropped or picked, recreate its nested layout under <code>targetFolder</code> — e.g. <code>photos/2026/jan/x.png</code> uploads to <code>&lt;targetFolder&gt;/photos/2026/jan</code>. Set to <code>false</code> to flatten everything into <code>targetFolder</code>. Only affects destination paths; a folder's files are ingested either way. See <a href="#/examples/target-folder">Target folder example</a>.</td></tr>
            <tr><td><code>forceName</code></td><td><code>string | (() =&gt; string)</code></td><td><code>undefined</code></td><td>Force every uploaded file to be saved under this exact name in <code>targetFolder</code>, overwriting any existing file with the same name. Translates to <code>&amp;opt_force_name=…</code> on the upload request and works across every source (local file, URL import, Google Drive, Unsplash, …). Setting this implicitly clamps <code>restrictions.maxNumberOfFiles</code> to <code>1</code>, disables the multi-select on the file picker, and bypasses tus regardless of file size. Also locks file renaming in the pre-upload list (see <code>allowFileRename</code>). Use for single-asset slots like watermarks, default images, or folder icons.</td></tr>
            <tr><td><code>allowFileRename</code></td><td><code>boolean</code></td><td><code>true</code></td><td>Allow users to edit file names in the pre-upload list. Set to <code>false</code> when the stored name must not change — e.g. uploading a new version of an existing asset, where the name decides which asset gets versioned. Treated as <code>false</code> whenever <code>forceName</code> is set, since the server overrides the name anyway.</td></tr>
        ...
        <table class="config-table">
          ...
          <tbody>
            ...
            <tr><td><code>transformRemoteThumbnail</code></td><td><code>(url, ctx) =&gt; string</code></td><td><code>undefined</code></td><td>Rewrite thumbnail URLs before they render. Covers third-party previews (URL imports + connector listings, typically proxied for CSP compliance) and the post-upload preview swap (<code>ctx.source === 'cdn-complete'</code>), where <code>ctx.urls</code> carries the full <code>response.file.url</code> map so the host can return a different variant — e.g. <code>ctx.urls.permalink</code> when the <code>cdn</code> URL is cached stale ("new version" uploads). See <a href="#csp-aware-deployments">CSP-aware deployments</a>.</td></tr>
          </tbody>
        </table>
...
uploaderB.config = {
    ...
    confidence: 'mid', // 'low' | 'mid' | 'high' (optional, default 'mid')
        ...
        <table class="config-table">
          ...
          <tbody>
            ...
            <tr><td><code>'high'</code></td><td>0.85</td><td>Strictest — only near-identical matches</td></tr>
          </tbody>
        </table>
  ...
  uploadSettings: {
    ...
    defaults: {
      ...
      resumable: true,
        ...
        <p>To override which URL the swap uses, handle <code>ctx.source === 'cdn-complete'</code> in <code>transformRemoteThumbnail</code>: <code>ctx.urls</code> carries the full <code>response.file.url</code> map (<code>public</code>, <code>cdn</code>, <code>cdn_permalink</code>, <code>permalink</code>). Returning <code>ctx.urls.permalink</code> — which carries the <code>?vh=</code> version hash — is the right move when uploading a <em>new version</em> of an existing asset, where the <code>cdn</code> URL points at the unchanged path and still serves the previous version's cached image.</p>
  ...
  transformRemoteThumbnail: (url, ctx) => {
    // ctx.source: 'url-import' | 'connector' | 'cdn-complete'
    // ctx.providerId: ProviderId | undefined         (only when source === 'connector')
    // ctx.urls: response.file.url map | undefined    (only when source === 'cdn-complete')
    if (ctx.source === 'cdn-complete') return ctx.urls?.permalink ?? url;
    return \`https://demo.cloudimg.io/v7/\${encodeURIComponent(url)}?w=200\`;
  },
};`)}
        <p>The hook applies in four places: the URL-import dialog, the file selected from a connector (Google Drive, Unsplash, etc.), the listing thumbnails shown <em>while browsing</em> a connector, and the post-upload preview swap (<code>cdn-complete</code>). If your host doesn't have CSP restrictions and doesn't need to override the post-upload preview, you can omit it entirely.</p>
