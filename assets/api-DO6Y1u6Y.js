import{h as d,c as e,d as t}from"./doc-utils-XkOyWBCy.js";const l={render(){return`
      <div class="doc-content">
        ...
        ${e("typescript",`const uploader = document.querySelector('sfx-uploader');
...
uploader.addEventListener('sfx-all-complete', (e) => {
  ...
  console.log('Failed:', e.detail.failed);
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>sfx-file-locate</code></td><td><code>{ file: UploadFile, url: string | null }</code></td><td>User clicked "Locate" on a completed file — either on the review-screen tile or the floating-panel icon (requires <code>showLocateButton: true</code>). <code>url</code> is the resolved deep-link the uploader is about to navigate to (or <code>null</code> when the file has no UUID). Cancelable — call <code>event.preventDefault()</code> to suppress the default current-tab navigation to <code>\${adminUrl}/library?lf=…</code> and route <code>url</code> through your SPA router instead.</td></tr>
  ...
  callbacks: {
    ...
    onFileLocate: (file, url) => {
      // Return false to suppress the uploader's default current-tab
      // navigation — useful inside SPAs where window.location.assign
      // would cause a full page reload. Route via your router instead.
      if (url) myRouter.push(url);
      return false;
    },
    onFileCopyCdn: (file, cdnUrl) => console.log('Copied CDN:', cdnUrl),
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>onFileLocate</code></td><td><code>(file, url) =&gt; boolean | void</code></td><td>User clicked "Locate" on a completed file. Receives the resolved Locate URL (or <code>null</code> when no UUID). Return <code>false</code> to suppress the uploader's default current-tab navigation and route <code>url</code> via your SPA router.</td></tr>
  ...
  return (
    <>
      ...
      <Uploader ref={ref} config={{ /* ... */ }} />
        ...
        ${t({href:"#/docs/configuration",label:"Configuration"},{href:"#/docs/theming",label:"Theming"})}
      </div>
    `},init(){d()}};export{l as default};
