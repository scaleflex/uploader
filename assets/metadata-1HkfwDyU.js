import{b as O}from"./index-B6n7x3mh.js";import{r as B}from"./code-block-C_3oxnLY.js";const q="sfx-uploader-demo-meta",R={projectUuid:"",enforceRequired:"auto",language:"",sessionToken:"",companyToken:"",projectToken:""};function F(){try{const t=localStorage.getItem(q);if(t)return{...R,...JSON.parse(t)}}catch{}return{...R}}function H(t){localStorage.setItem(q,JSON.stringify(t))}let l=null;function m(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function c(t,n){if(!l)return;const r=new Date().toLocaleTimeString(),d=document.createElement("div"),p=n?` <span class="log-data">${m(JSON.stringify(n,null,0))}</span>`:"";d.innerHTML=`<span class="log-time">${m(r)}</span> <span class="log-event">${m(t)}</span>${p}`,l.appendChild(d),l.scrollTop=l.scrollHeight}const x={render(){return`
    ...
    `},init(t){l=document.getElementById("event-log"),B("#code-container",[{label:"Basic (no Hub session)",lang:"javascript",code:`
uploader.config = {
  auth: {
