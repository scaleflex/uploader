"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const s=require("lit"),c=require("lit/decorators.js"),w=require("@scaleflex/dam-ui"),x=require("./sfx-uploader-BhfiaumS.cjs"),$=require("@scaleflex/dam-ui/i18n"),y="sfx-uploader-token:";function g(l){try{return localStorage.getItem(`${y}${l}`)}catch{return null}}function S(l,e){try{localStorage.setItem(`${y}${l}`,e)}catch{}}function v(l){try{localStorage.removeItem(`${y}${l}`)}catch{}}function F(l,e){const t=r=>{if(l&&r.source!==l)return;const o=typeof r.data=="string"?P(r.data):r.data;o!=null&&o.token&&e(o.token)};return window.addEventListener("message",t),()=>window.removeEventListener("message",t)}function P(l){try{return JSON.parse(l)}catch{return null}}var z=Object.defineProperty,p=(l,e,t,r)=>{for(var o=void 0,u=l.length-1,i;u>=0;u--)(i=l[u])&&(o=i(e,t,o)||o);return o&&z(e,t,o),o};const k=class k extends s.LitElement{constructor(){super(...arguments),this.t=$.fallbackT,this.provider="google-drive",this.companionUrl="",this.multi=!0,this.maxSelect=null,this.transformThumbnail=e=>e,this._authenticated=!1,this._loading=!1,this._items=[],this._selectedIds=new Set,this._breadcrumbs=[],this._nextPagePath=null,this._error=null,this._loadingMore=!1,this._username=null,this._resolvingFolders=!1,this._resolveProgress=0,this._cleanupAuthListener=null,this._authWindow=null,this._resolveAbort=null,this._handleConnect=()=>{var t;try{if(this._authWindow&&!this._authWindow.closed){this._authWindow.focus();return}}catch{this._authWindow=null}const e=x.getAuthUrl(this.companionUrl,this.provider);this._authWindow=window.open(e,"_blank","width=600,height=600"),(t=this._cleanupAuthListener)==null||t.call(this),this._cleanupAuthListener=F(this._authWindow,r=>{var o;this._authWindow=null,(o=this._cleanupAuthListener)==null||o.call(this),this._cleanupAuthListener=null,S(this.provider,r),this._authenticated=!0,this._loadFolder("")})},this._lastClickedIndex=null,this._toggleSelectAll=()=>{if(this._items.every(t=>this._selectedIds.has(t.id)))this._selectedIds=new Set;else{const t=new Set;let r=0;for(const o of this._items)o.isFolder?t.add(o.id):(this.maxSelect===null||r<this.maxSelect)&&(t.add(o.id),r++);this._selectedIds=t}},this._onAddSelected=async()=>{var u,i;const e=g(this.provider);if(!e)return;const t=this._items.filter(n=>!n.isFolder&&this._selectedIds.has(n.id)),r=this._items.filter(n=>n.isFolder&&this._selectedIds.has(n.id)),o=t.map(n=>({companionUrl:this.companionUrl,provider:this.provider,token:e,requestPath:n.requestPath,fileId:n.id,name:n.name,mimeType:n.mimeType,size:n.size,thumbnail:n.thumbnail}));if(r.length>0){(u=this._resolveAbort)==null||u.abort(),this._resolveAbort=new AbortController;const n=this._resolveAbort.signal;this._resolvingFolders=!0,this._resolveProgress=0;let h=0;try{for(const a of r){const m=await x.listFolderRecursive(this.companionUrl,this.provider,e,a.requestPath,a.name,n);if(n.aborted)return;for(const f of m)o.push({companionUrl:this.companionUrl,provider:this.provider,token:e,requestPath:f.requestPath,fileId:f.id,name:f.name,mimeType:f.mimeType,size:f.size,thumbnail:f.thumbnail,relativeFolder:f.relativeFolder});h+=m.length,this._resolveProgress=h}}catch(a){if(n.aborted)return;if(this._resolvingFolders=!1,a instanceof x.AuthExpiredError){v(this.provider),this._authenticated=!1;return}this._error=a instanceof Error?a.message:this.t("failedToReadFolder","Failed to read folder contents");return}finally{((i=this._resolveAbort)==null?void 0:i.signal)===n&&(this._resolveAbort=null)}this._resolvingFolders=!1}this.dispatchEvent(new CustomEvent("connector-files-selected",{detail:{files:o},bubbles:!0,composed:!0}))},this._onClose=()=>{this.dispatchEvent(new CustomEvent("connector-close",{bubbles:!0,composed:!0}))},this._handleLogout=async()=>{const e=g(this.provider);if(e){try{await x.logout(this.companionUrl,this.provider,e)}catch{}v(this.provider)}this._reset()},this._onBack=()=>{this._breadcrumbs.length!==0&&this._onBreadcrumbClick(this._breadcrumbs.length-2)},this._cancelResolve=()=>{var e;(e=this._resolveAbort)==null||e.abort(),this._resolveAbort=null,this._resolvingFolders=!1,this._resolveProgress=0}}connectedCallback(){super.connectedCallback(),this._checkAuth()}disconnectedCallback(){var e,t;super.disconnectedCallback(),(e=this._cleanupAuthListener)==null||e.call(this),this._cleanupAuthListener=null,(t=this._resolveAbort)==null||t.abort(),this._resolveAbort=null}willUpdate(e){e.has("provider")&&e.get("provider")!==void 0&&(this._reset(),this._checkAuth())}_reset(){var e;(e=this._resolveAbort)==null||e.abort(),this._resolveAbort=null,this._authenticated=!1,this._loading=!1,this._items=[],this._selectedIds=new Set,this._lastClickedIndex=null,this._breadcrumbs=[],this._nextPagePath=null,this._error=null,this._username=null,this._resolvingFolders=!1,this._resolveProgress=0}_checkAuth(){g(this.provider)&&(this._authenticated=!0,this._loadFolder(""))}get _providerDef(){return x.getProviderSources([this.provider])[0]??null}get _providerLabel(){var e;return((e=this._providerDef)==null?void 0:e.label)??this.provider}async _loadFolder(e){const t=g(this.provider);if(!t){this._authenticated=!1;return}this.offsetHeight>0&&(this.style.minHeight=`${this.offsetHeight}px`),this._loading=!0,this._error=null,this._items=[],this._selectedIds=new Set,this._lastClickedIndex=null,this._nextPagePath=null;try{const r=await x.listFiles(this.companionUrl,this.provider,t,e);this._items=r.items,this._nextPagePath=r.nextPagePath,r.username&&(this._username=r.username)}catch(r){r instanceof x.AuthExpiredError?(v(this.provider),this._authenticated=!1):this._error=r instanceof Error?r.message:this.t("failedToLoadFiles","Failed to load files")}finally{this._loading=!1}}_onFolderClick(e){this._breadcrumbs=[...this._breadcrumbs,{name:e.name,path:e.requestPath}],this._loadFolder(e.requestPath)}_onBreadcrumbClick(e){if(e<0)this._breadcrumbs=[],this._loadFolder("");else{const t=this._breadcrumbs[e];this._breadcrumbs=this._breadcrumbs.slice(0,e+1),this._loadFolder(t.path)}}async _onLoadMore(){const e=g(this.provider);if(!(!e||!this._nextPagePath)){this._loadingMore=!0;try{const t=await x.listNextPage(this.companionUrl,e,this._nextPagePath);this._items=[...this._items,...t.items],this._nextPagePath=t.nextPagePath}catch(t){t instanceof x.AuthExpiredError&&(v(this.provider),this._authenticated=!1)}finally{this._loadingMore=!1}}}get _selectedFileCount(){return this._items.filter(e=>!e.isFolder&&this._selectedIds.has(e.id)).length}_toggleSelect(e,t){if(!this.multi){if(e.isFolder)return;this._selectedIds=this._selectedIds.has(e.id)?new Set:new Set([e.id]);const n=this._items.filter(h=>!h.isFolder).findIndex(h=>h.id===e.id);n!==-1&&(this._lastClickedIndex=n);return}const r=this._items.filter(i=>!i.isFolder),o=r.findIndex(i=>i.id===e.id),u=this.maxSelect!==null&&this._selectedFileCount>=this.maxSelect;if(!e.isFolder&&(t!=null&&t.shiftKey)&&this._lastClickedIndex!==null&&o!==-1){const i=Math.min(this._lastClickedIndex,o),n=Math.max(this._lastClickedIndex,o),h=new Set(this._selectedIds);for(let a=i;a<=n;a++)h.has(r[a].id)||this.maxSelect!==null&&[...h].filter(f=>r.some(b=>b.id===f)).length>=this.maxSelect||h.add(r[a].id);this._selectedIds=h}else{const i=new Set(this._selectedIds);i.has(e.id)?i.delete(e.id):(e.isFolder||!u)&&i.add(e.id),this._selectedIds=i}o!==-1&&(this._lastClickedIndex=o)}render(){return s.html`
      ${this._renderHeader()}
    ...
    `}_renderHeader(){const e=this._providerDef,t=this._authenticated&&this._breadcrumbs.length>0;return s.html`
      <div class="browser-header">
        ...
        ${this._authenticated?s.html`<button class="logout-btn" @click=${this._handleLogout}>
    ...
    `}_renderAuthView(){const e=this._providerDef;return s.html`
      <div class="auth-view">
        ...
        <div class="auth-logo-wrap">
          <div class="auth-ring">
            <div class="auth-logo">
              ${e!=null&&e.brandHtml?s.html`<span
                    ${w.cspStyle({display:"flex","align-items":"center","justify-content":"center",transform:"scale(2.2)"})}
                    >${x.brandIcon(e)}</span
                  >`:s.html`<svg
                    viewBox="0 0 24 24"
                    fill="none"
        ...
        ${e.map(i=>{const n=this._selectedIds.has(i.id),h=!n&&o;return s.html`
            ...
            >
              ...
              <div class="file-thumb">
                ${i.thumbnail?s.html`<img
                      src=${this.transformThumbnail(i.thumbnail)}
                      alt=""
                      loading="lazy"
                      referrerpolicy="no-referrer"
                      @error=${a=>{const m=a.target;m.style.display="none",m.parentElement.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>'}}
                    />`:s.html`<svg
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
              ...
              <div class="file-info">
                <div class="file-name">${i.name}</div>
                <div class="file-meta">
                  ${i.size?s.html`<span class="file-size">${C(i.size)}</span>`:s.nothing}
                </div>
              </div>
              <span class="file-modified">${_(i.modifiedDate,this.t)}</span>
            </div>
    ...
    `}_renderBreadcrumbs(){return this._breadcrumbs.length===0?s.nothing:s.html`
      <div class="breadcrumbs">
        ...
        ${this._breadcrumbs.map((e,t)=>s.html`
            ...
            ${t<this._breadcrumbs.length-1?s.html`<button class="crumb" @click=${()=>this._onBreadcrumbClick(t)}>
                  ${e.name}
                </button>`:s.html`<span class="crumb-current">${e.name}</span>`}
          `)}
      </div>
    `}};k.styles=s.css`
    :host {
