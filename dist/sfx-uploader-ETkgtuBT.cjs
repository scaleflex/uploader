"use strict";const a=require("lit"),w=require("lit/decorators.js"),ut=require("lit/directive.js"),ne=require("lit/directives/unsafe-svg.js"),Gt=require("lit/directives/unsafe-html.js"),ii=require("lit/directives/class-map.js"),at=i=>i.includes("-")?i:i.replace(/[A-Z]/g,e=>`-${e.toLowerCase()}`);class oi extends ut.Directive{constructor(e){if(super(e),this._appliedProps=new Set,e.type!==ut.PartType.ELEMENT)throw new Error("cspStyle must be used as an element directive: <el ${cspStyle({...})}>")}render(e){return a.noChange}update(e,[t]){if(t===this._lastStyles)return a.noChange;this._lastStyles=t;const{style:r}=e.element,o=t??{};for(const n of this._appliedProps)(!(n in o)||o[n]==null||o[n]==="")&&(r.removeProperty(at(n)),this._appliedProps.delete(n));for(const[n,s]of Object.entries(o))s!=null&&s!==""?(r.setProperty(at(n),s),this._appliedProps.add(n)):this._appliedProps.has(n)&&(r.removeProperty(at(n)),this._appliedProps.delete(n));return a.noChange}}const N=ut.directive(oi);class Sr{constructor(e){this.listeners=new Set,this._notifying=!1,this._pendingState=null,this.state=e}getState(){return this.state}setState(e){if(this._notifying){this._pendingState={...this._pendingState||{},...e};return}const t=this.state;this.state={...t,...e},this._notifying=!0;try{this.listeners.forEach(r=>r(this.state,t))}finally{this._notifying=!1}if(this._pendingState){const r=this._pendingState;this._pendingState=null,this.setState(r)}}subscribe(e){return this.listeners.add(e),()=>this.listeners.delete(e)}destroy(){this.listeners.clear()}}function M(i,e,t){const r=i.getState().files,o=r.get(e);if(!o)return;const n=new Map(r);n.set(e,{...o,...t}),i.setState({files:n})}function ve(i,e){const t=new Map(i.getState().files);t.set(e.id,e),i.setState({files:t})}function Wt(i,e){const t=i.getState().files;if(!t.has(e))return;const r=new Map(t);r.delete(e),i.setState({files:r})}function $r(){return new Sr({files:new Map,queueConfig:{concurrency:3,autoProceed:!1,retryConfig:{maxRetries:0,baseDelay:1e3,maxDelay:3e4,backoffFactor:2}},isPaused:!1,restrictions:{maxFileSize:null,maxTotalFilesSize:null,maxNumberOfFiles:null,minNumberOfFiles:null,allowedFileTypes:null,blockedFileTypes:null},targetFolder:"/",totalProgress:0,totalSpeed:0,totalBytesUploaded:0,totalBytes:0,isUploading:!1})}class ni{constructor(e,t){this.host=e,this.store=t,e.addController(this)}get state(){return this.store.getState()}setState(e){this.store.setState(e)}hostConnected(){this.unsubscribe=this.store.subscribe(()=>{this.host.requestUpdate()})}hostDisconnected(){var e;(e=this.unsubscribe)==null||e.call(this)}}function Ur(i,e,t){let o=`${i.replace(/\/+$/,"")}/v4/files?folder=${encodeURIComponent(e)}`;if(t)for(const[n,s]of Object.entries(t))s!=null&&(o+=`&${encodeURIComponent(n)}=${encodeURIComponent(s)}`);return o}function si(i,e){const t=new XMLHttpRequest;let r=!1;const o=Ur(e.apiBase,e.folder,e.extraParams);t.open("POST",o);for(const[s,l]of Object.entries(e.authHeaders))t.setRequestHeader(s,l);t.upload.addEventListener("progress",s=>{s.lengthComputable&&!r&&e.onProgress(s.loaded,s.total)}),t.addEventListener("load",()=>{if(r)return;let s;try{s=JSON.parse(t.responseText)}catch{e.onError(new Error(`Invalid JSON response (HTTP ${t.status})`));return}t.status>=200&&t.status<300&&s.status==="success"?e.onComplete(s):e.onError(new Error(s.hint||s.msg||`Upload failed (HTTP ${t.status})`))}),t.addEventListener("error",()=>{r||e.onError(new Error("Network error — check your connection"))}),t.addEventListener("timeout",()=>{r||e.onError(new Error("Upload timed out"))});const n=new FormData;if(i.file){const s={name:i.name,type:i.type};n.append("info[files[]]",JSON.stringify(s)),Object.keys(i.meta).length>0&&n.append("meta[files[]]",JSON.stringify(i.meta)),i.tags.length>0&&n.append("tags[files[]]",JSON.stringify(i.tags)),n.append("files[]",i.file,i.name)}return t.timeout=6e4,t.send(n),{abort(){r=!0,t.abort()}}}function ai(i,e){if(!i.remoteUrl)return e.onError(new Error("Remote URL is required for URL upload")),{abort(){}};const t=new XMLHttpRequest;let r=!1;const o=Ur(e.apiBase,e.folder,e.extraParams);t.open("POST",o);for(const[s,l]of Object.entries(e.authHeaders))t.setRequestHeader(s,l);t.setRequestHeader("Content-Type","application/json"),t.addEventListener("load",()=>{if(r)return;let s;try{s=JSON.parse(t.responseText)}catch{e.onError(new Error(`Invalid JSON response (HTTP ${t.status})`));return}t.status>=200&&t.status<300&&s.status==="success"?e.onComplete(s):e.onError(new Error(s.hint||s.msg||`Upload failed (HTTP ${t.status})`))}),t.addEventListener("error",()=>{r||e.onError(new Error("Network error — check your connection"))}),t.addEventListener("timeout",()=>{r||e.onError(new Error("Upload timed out"))});const n={files_urls:[{url:i.remoteUrl,name:i.name}]};return t.timeout=6e4,t.send(JSON.stringify(n)),{abort(){r=!0,t.abort()}}}function it(i){return{Accept:"application/json","Content-Type":"application/json","uppy-auth-token":i}}function Ue(i){return i.replace(/\/+$/,"")}const li={"google-drive":"drive",dropbox:"dropbox",onedrive:"onedrive",box:"box",instagram:"instagram",facebook:"facebook",unsplash:"unsplash"};function He(i){return li[i]??i}function di(i,e){const t=Ue(i),r=btoa(JSON.stringify({origin:window.location.origin})),o=He(e);return`${t}/${o}/connect?state=${encodeURIComponent(r)}`}async function ci(i,e,t,r=""){const o=Ue(i),n=r?`/${r}`:"",s=He(e),l=await fetch(`${o}/${s}/list${n}`,{method:"GET",headers:it(t),credentials:"same-origin"});if(l.status===401)throw new ot;if(!l.ok){const d=await l.json().catch(()=>null);throw new Error((d==null?void 0:d.message)||`Companion list failed (HTTP ${l.status})`)}return l.json()}async function pi(i,e,t){const r=Ue(i),o=await fetch(`${r}/${t}`,{method:"GET",headers:it(e),credentials:"same-origin"});if(o.status===401)throw new ot;if(!o.ok){const n=await o.json().catch(()=>null);throw new Error((n==null?void 0:n.message)||`Companion list failed (HTTP ${o.status})`)}return o.json()}async function ui(i,e,t,r){const o=Ue(i),n=He(e),s=r?`q=${encodeURIComponent(t)}&${r}`:`q=${encodeURIComponent(t)}`,l=await fetch(`${o}/search/${n}/list?${s}`,{method:"GET",headers:{Accept:"application/json","Content-Type":"application/json"},credentials:"same-origin"});if(!l.ok){const d=await l.json().catch(()=>null);throw new Error((d==null?void 0:d.message)||`Search failed (HTTP ${l.status})`)}return l.json()}async function fi(i,e,t,r,o,n=!1){const s=Ue(i),l=He(e),d=n?`${s}/search/${l}/get/${r}`:`${s}/${l}/get/${r}`,u=n?{Accept:"application/json","Content-Type":"application/json"}:it(t),h=await fetch(d,{method:"POST",headers:u,credentials:"same-origin",body:JSON.stringify({...o,httpMethod:o.httpMethod??"POST",useFormData:o.useFormData??!0,fieldname:o.fieldname??"files[]"})});if(h.status===401)throw new ot;if(!h.ok){const C=await h.json().catch(()=>null);throw new Error((C==null?void 0:C.message)||`Companion upload failed (HTTP ${h.status})`)}return h.json()}async function hi(i,e,t){const r=Ue(i),o=He(e),n=await fetch(`${r}/${o}/logout`,{method:"GET",headers:it(t),credentials:"same-origin"});return n.ok?n.json():{ok:!1,revoked:!1}}function gi(i){var o;const t=((o=/^(?:https?:\/\/|\/\/)?(?:[^@\n]+@)?(?:www\.)?([^\n]+)/i.exec(i))==null?void 0:o[1])??i;return`${/^https:\/\//i.test(i)?"wss":"ws"}://${t}`}class ot extends Error{constructor(){super("Authentication expired"),this.name="AuthExpiredError"}}function mi(i,e){const t=i.remoteInfo;if(!t)return e.onError(new Error("remoteInfo is required for companion upload")),{abort(){}};let r=!1,o=null,s=`${e.apiBase.replace(/\/+$/,"")}/v4/files?folder=${encodeURIComponent(e.folder)}`;if(e.extraParams)for(const[u,h]of Object.entries(e.extraParams))h!=null&&(s+=`&${encodeURIComponent(u)}=${encodeURIComponent(h)}`);const l={};i.meta&&Object.keys(i.meta).length>0&&Object.assign(l,i.meta),i.tags&&i.tags.length>0&&(l.tags=i.tags);const d=!t.token;return fi(t.companionUrl,t.provider,t.token,t.requestPath,{fileId:t.fileId,endpoint:s,headers:e.authHeaders,size:t.size,metadata:Object.keys(l).length>0?l:void 0},d).then(u=>{if(r)return;const C=`${gi(t.companionUrl)}/api/${u.token}`;try{o=new WebSocket(C)}catch{e.onError(new Error("Failed to connect to upload progress channel"));return}o.onmessage=$=>{var z,O,_;if(!r)try{const P=JSON.parse($.data);switch(P.action){case"progress":{const S=P.payload,m=S.bytesUploaded??0,v=S.bytesTotal??(t.size||1);e.onProgress(m,v);break}case"success":{const S=P.payload;if(o==null||o.close(),(z=S.response)!=null&&z.responseText)try{const m=JSON.parse(S.response.responseText);if(m.status==="success"){e.onComplete(m);return}e.onError(new Error(m.msg||"Upload failed"));return}catch{}e.onError(new Error("Upload completed but no valid response received"));break}case"error":{o==null||o.close();const S=P.payload;let m=((O=S.error)==null?void 0:O.message)||"Upload failed";if((_=S.response)!=null&&_.responseText)try{const v=JSON.parse(S.response.responseText);m=v.hint||v.msg||v.message||m}catch{}e.onError(new Error(m));break}}}catch{}},o.onerror=()=>{r||e.onError(new Error("Upload progress connection failed"))},o.onclose=()=>{o=null}}).catch(u=>{r||e.onError(u instanceof Error?u:new Error(String(u)))}),{abort(){if(r=!0,o){try{o.send(JSON.stringify({action:"cancel",payload:{}}))}catch{}o.close(),o=null}}}}function ft(i){"@babel/helpers - typeof";return ft=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},ft(i)}function vi(i,e,t){return Object.defineProperty(i,"prototype",{writable:!1}),i}function xi(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function bi(i,e,t){return e=Le(e),yi(i,Ut()?Reflect.construct(e,t||[],Le(i).constructor):e.apply(i,t))}function yi(i,e){if(e&&(ft(e)==="object"||typeof e=="function"))return e;if(e!==void 0)throw new TypeError("Derived constructors may only return object or undefined");return wi(i)}function wi(i){if(i===void 0)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return i}function _i(i,e){if(typeof e!="function"&&e!==null)throw new TypeError("Super expression must either be null or a function");i.prototype=Object.create(e&&e.prototype,{constructor:{value:i,writable:!0,configurable:!0}}),Object.defineProperty(i,"prototype",{writable:!1}),e&&De(i,e)}function ht(i){var e=typeof Map=="function"?new Map:void 0;return ht=function(r){if(r===null||!Ci(r))return r;if(typeof r!="function")throw new TypeError("Super expression must either be null or a function");if(typeof e<"u"){if(e.has(r))return e.get(r);e.set(r,o)}function o(){return ki(r,arguments,Le(this).constructor)}return o.prototype=Object.create(r.prototype,{constructor:{value:o,enumerable:!1,writable:!0,configurable:!0}}),De(o,r)},ht(i)}function ki(i,e,t){if(Ut())return Reflect.construct.apply(null,arguments);var r=[null];r.push.apply(r,e);var o=new(i.bind.apply(i,r));return t&&De(o,t.prototype),o}function Ut(){try{var i=!Boolean.prototype.valueOf.call(Reflect.construct(Boolean,[],function(){}))}catch{}return(Ut=function(){return!!i})()}function Ci(i){try{return Function.toString.call(i).indexOf("[native code]")!==-1}catch{return typeof i=="function"}}function De(i,e){return De=Object.setPrototypeOf?Object.setPrototypeOf.bind():function(r,o){return r.__proto__=o,r},De(i,e)}function Le(i){return Le=Object.setPrototypeOf?Object.getPrototypeOf.bind():function(t){return t.__proto__||Object.getPrototypeOf(t)},Le(i)}var ze=(function(i){function e(t){var r,o=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null,n=arguments.length>2&&arguments[2]!==void 0?arguments[2]:null,s=arguments.length>3&&arguments[3]!==void 0?arguments[3]:null;if(xi(this,e),r=bi(this,e,[t]),r.originalRequest=n,r.originalResponse=s,r.causingError=o,o!=null&&(t+=", caused by ".concat(o.toString())),n!=null){var l=n.getHeader("X-Request-ID")||"n/a",d=n.getMethod(),u=n.getURL(),h=s?s.getStatus():"n/a",C=s?s.getBody()||"":"n/a";t+=", originated from request (method: ".concat(d,", url: ").concat(u,", response code: ").concat(h,", response text: ").concat(C,", request id: ").concat(l,")")}return r.message=t,r}return _i(e,i),vi(e)})(ht(Error));function je(i){"@babel/helpers - typeof";return je=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},je(i)}function Si(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function $i(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,Ei(r.key),r)}}function Ui(i,e,t){return e&&$i(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function Ei(i){var e=Pi(i,"string");return je(e)=="symbol"?e:e+""}function Pi(i,e){if(je(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(je(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var Oi=(function(){function i(){Si(this,i)}return Ui(i,[{key:"listAllUploads",value:function(){return Promise.resolve([])}},{key:"findUploadsByFingerprint",value:function(t){return Promise.resolve([])}},{key:"removeUpload",value:function(t){return Promise.resolve()}},{key:"addUpload",value:function(t,r){return Promise.resolve(null)}}])})();const Er="3.7.8",zi=Er,Ee=typeof Buffer=="function",Xt=typeof TextDecoder=="function"?new TextDecoder:void 0,Kt=typeof TextEncoder=="function"?new TextEncoder:void 0,Ri="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",Re=Array.prototype.slice.call(Ri),Ge=(i=>{let e={};return i.forEach((t,r)=>e[t]=r),e})(Re),Ti=/^(?:[A-Za-z\d+\/]{4})*?(?:[A-Za-z\d+\/]{2}(?:==)?|[A-Za-z\d+\/]{3}=?)?$/,q=String.fromCharCode.bind(String),Zt=typeof Uint8Array.from=="function"?Uint8Array.from.bind(Uint8Array):i=>new Uint8Array(Array.prototype.slice.call(i,0)),Pr=i=>i.replace(/=/g,"").replace(/[+\/]/g,e=>e=="+"?"-":"_"),Or=i=>i.replace(/[^A-Za-z0-9\+\/]/g,""),zr=i=>{let e,t,r,o,n="";const s=i.length%3;for(let l=0;l<i.length;){if((t=i.charCodeAt(l++))>255||(r=i.charCodeAt(l++))>255||(o=i.charCodeAt(l++))>255)throw new TypeError("invalid character found");e=t<<16|r<<8|o,n+=Re[e>>18&63]+Re[e>>12&63]+Re[e>>6&63]+Re[e&63]}return s?n.slice(0,s-3)+"===".substring(s):n},Et=typeof btoa=="function"?i=>btoa(i):Ee?i=>Buffer.from(i,"binary").toString("base64"):zr,gt=Ee?i=>Buffer.from(i).toString("base64"):i=>{let t=[];for(let r=0,o=i.length;r<o;r+=4096)t.push(q.apply(null,i.subarray(r,r+4096)));return Et(t.join(""))},Xe=(i,e=!1)=>e?Pr(gt(i)):gt(i),Di=i=>{if(i.length<2){var e=i.charCodeAt(0);return e<128?i:e<2048?q(192|e>>>6)+q(128|e&63):q(224|e>>>12&15)+q(128|e>>>6&63)+q(128|e&63)}else{var e=65536+(i.charCodeAt(0)-55296)*1024+(i.charCodeAt(1)-56320);return q(240|e>>>18&7)+q(128|e>>>12&63)+q(128|e>>>6&63)+q(128|e&63)}},Li=/[\uD800-\uDBFF][\uDC00-\uDFFFF]|[^\x00-\x7F]/g,Rr=i=>i.replace(Li,Di),Jt=Ee?i=>Buffer.from(i,"utf8").toString("base64"):Kt?i=>gt(Kt.encode(i)):i=>Et(Rr(i)),Ce=(i,e=!1)=>e?Pr(Jt(i)):Jt(i),Qt=i=>Ce(i,!0),ji=/[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF7][\x80-\xBF]{3}/g,Fi=i=>{switch(i.length){case 4:var e=(7&i.charCodeAt(0))<<18|(63&i.charCodeAt(1))<<12|(63&i.charCodeAt(2))<<6|63&i.charCodeAt(3),t=e-65536;return q((t>>>10)+55296)+q((t&1023)+56320);case 3:return q((15&i.charCodeAt(0))<<12|(63&i.charCodeAt(1))<<6|63&i.charCodeAt(2));default:return q((31&i.charCodeAt(0))<<6|63&i.charCodeAt(1))}},Tr=i=>i.replace(ji,Fi),Dr=i=>{if(i=i.replace(/\s+/g,""),!Ti.test(i))throw new TypeError("malformed base64.");i+="==".slice(2-(i.length&3));let e,t,r,o=[];for(let n=0;n<i.length;)e=Ge[i.charAt(n++)]<<18|Ge[i.charAt(n++)]<<12|(t=Ge[i.charAt(n++)])<<6|(r=Ge[i.charAt(n++)]),t===64?o.push(q(e>>16&255)):r===64?o.push(q(e>>16&255,e>>8&255)):o.push(q(e>>16&255,e>>8&255,e&255));return o.join("")},Pt=typeof atob=="function"?i=>atob(Or(i)):Ee?i=>Buffer.from(i,"base64").toString("binary"):Dr,Lr=Ee?i=>Zt(Buffer.from(i,"base64")):i=>Zt(Pt(i).split("").map(e=>e.charCodeAt(0))),jr=i=>Lr(Fr(i)),Ai=Ee?i=>Buffer.from(i,"base64").toString("utf8"):Xt?i=>Xt.decode(Lr(i)):i=>Tr(Pt(i)),Fr=i=>Or(i.replace(/[-_]/g,e=>e=="-"?"+":"/")),mt=i=>Ai(Fr(i)),Bi=i=>{if(typeof i!="string")return!1;const e=i.replace(/\s+/g,"").replace(/={0,2}$/,"");return!/[^\s0-9a-zA-Z\+/]/.test(e)||!/[^\s0-9a-zA-Z\-_]/.test(e)},Ar=i=>({value:i,enumerable:!1,writable:!0,configurable:!0}),Br=function(){const i=(e,t)=>Object.defineProperty(String.prototype,e,Ar(t));i("fromBase64",function(){return mt(this)}),i("toBase64",function(e){return Ce(this,e)}),i("toBase64URI",function(){return Ce(this,!0)}),i("toBase64URL",function(){return Ce(this,!0)}),i("toUint8Array",function(){return jr(this)})},Ir=function(){const i=(e,t)=>Object.defineProperty(Uint8Array.prototype,e,Ar(t));i("toBase64",function(e){return Xe(this,e)}),i("toBase64URI",function(){return Xe(this,!0)}),i("toBase64URL",function(){return Xe(this,!0)})},Ii=()=>{Br(),Ir()},Mi={version:Er,VERSION:zi,atob:Pt,atobPolyfill:Dr,btoa:Et,btoaPolyfill:zr,fromBase64:mt,toBase64:Ce,encode:Ce,encodeURI:Qt,encodeURL:Qt,utob:Rr,btou:Tr,decode:mt,isValid:Bi,fromUint8Array:Xe,toUint8Array:jr,extendString:Br,extendUint8Array:Ir,extendBuiltins:Ii};var er=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{};function Hi(i){return i&&i.__esModule&&Object.prototype.hasOwnProperty.call(i,"default")?i.default:i}var lt,tr;function qi(){return tr||(tr=1,lt=function(e,t){if(t=t.split(":")[0],e=+e,!e)return!1;switch(t){case"http":case"ws":return e!==80;case"https":case"wss":return e!==443;case"ftp":return e!==21;case"gopher":return e!==70;case"file":return!1}return e!==0}),lt}var We={},rr;function Ni(){if(rr)return We;rr=1;var i=Object.prototype.hasOwnProperty,e;function t(s){try{return decodeURIComponent(s.replace(/\+/g," "))}catch{return null}}function r(s){try{return encodeURIComponent(s)}catch{return null}}function o(s){for(var l=/([^=?#&]+)=?([^&]*)/g,d={},u;u=l.exec(s);){var h=t(u[1]),C=t(u[2]);h===null||C===null||h in d||(d[h]=C)}return d}function n(s,l){l=l||"";var d=[],u,h;typeof l!="string"&&(l="?");for(h in s)if(i.call(s,h)){if(u=s[h],!u&&(u===null||u===e||isNaN(u))&&(u=""),h=r(h),u=r(u),h===null||u===null)continue;d.push(h+"="+u)}return d.length?l+d.join("&"):""}return We.stringify=n,We.parse=o,We}var dt,ir;function Yi(){if(ir)return dt;ir=1;var i=qi(),e=Ni(),t=/^[\x00-\x20\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff]+/,r=/[\n\r\t]/g,o=/^[A-Za-z][A-Za-z0-9+-.]*:\/\//,n=/:\d+$/,s=/^([a-z][a-z0-9.+-]*:)?(\/\/)?([\\/]+)?([\S\s]*)/i,l=/^[a-zA-Z]:/;function d(m){return(m||"").toString().replace(t,"")}var u=[["#","hash"],["?","query"],function(v,g){return $(g.protocol)?v.replace(/\\/g,"/"):v},["/","pathname"],["@","auth",1],[NaN,"host",void 0,1,1],[/:(\d*)$/,"port",void 0,1],[NaN,"hostname",void 0,1,1]],h={hash:1,query:1};function C(m){var v;typeof window<"u"?v=window:typeof er<"u"?v=er:typeof self<"u"?v=self:v={};var g=v.location||{};m=m||g;var x={},R=typeof m,U;if(m.protocol==="blob:")x=new _(unescape(m.pathname),{});else if(R==="string"){x=new _(m,{});for(U in h)delete x[U]}else if(R==="object"){for(U in m)U in h||(x[U]=m[U]);x.slashes===void 0&&(x.slashes=o.test(m.href))}return x}function $(m){return m==="file:"||m==="ftp:"||m==="http:"||m==="https:"||m==="ws:"||m==="wss:"}function z(m,v){m=d(m),m=m.replace(r,""),v=v||{};var g=s.exec(m),x=g[1]?g[1].toLowerCase():"",R=!!g[2],U=!!g[3],L=0,D;return R?U?(D=g[2]+g[3]+g[4],L=g[2].length+g[3].length):(D=g[2]+g[4],L=g[2].length):U?(D=g[3]+g[4],L=g[3].length):D=g[4],x==="file:"?L>=2&&(D=D.slice(2)):$(x)?D=g[4]:x?R&&(D=D.slice(2)):L>=2&&$(v.protocol)&&(D=g[4]),{protocol:x,slashes:R||$(x),slashesCount:L,rest:D}}function O(m,v){if(m==="")return v;for(var g=(v||"/").split("/").slice(0,-1).concat(m.split("/")),x=g.length,R=g[x-1],U=!1,L=0;x--;)g[x]==="."?g.splice(x,1):g[x]===".."?(g.splice(x,1),L++):L&&(x===0&&(U=!0),g.splice(x,1),L--);return U&&g.unshift(""),(R==="."||R==="..")&&g.push(""),g.join("/")}function _(m,v,g){if(m=d(m),m=m.replace(r,""),!(this instanceof _))return new _(m,v,g);var x,R,U,L,D,G,re=u.slice(),me=typeof v,E=this,Q=0;for(me!=="object"&&me!=="string"&&(g=v,v=null),g&&typeof g!="function"&&(g=e.parse),v=C(v),R=z(m||"",v),x=!R.protocol&&!R.slashes,E.slashes=R.slashes||x&&v.slashes,E.protocol=R.protocol||v.protocol||"",m=R.rest,(R.protocol==="file:"&&(R.slashesCount!==2||l.test(m))||!R.slashes&&(R.protocol||R.slashesCount<2||!$(E.protocol)))&&(re[3]=[/(.*)/,"pathname"]);Q<re.length;Q++){if(L=re[Q],typeof L=="function"){m=L(m,E);continue}U=L[0],G=L[1],U!==U?E[G]=m:typeof U=="string"?(D=U==="@"?m.lastIndexOf(U):m.indexOf(U),~D&&(typeof L[2]=="number"?(E[G]=m.slice(0,D),m=m.slice(D+L[2])):(E[G]=m.slice(D),m=m.slice(0,D)))):(D=U.exec(m))&&(E[G]=D[1],m=m.slice(0,D.index)),E[G]=E[G]||x&&L[3]&&v[G]||"",L[4]&&(E[G]=E[G].toLowerCase())}g&&(E.query=g(E.query)),x&&v.slashes&&E.pathname.charAt(0)!=="/"&&(E.pathname!==""||v.pathname!=="")&&(E.pathname=O(E.pathname,v.pathname)),E.pathname.charAt(0)!=="/"&&$(E.protocol)&&(E.pathname="/"+E.pathname),i(E.port,E.protocol)||(E.host=E.hostname,E.port=""),E.username=E.password="",E.auth&&(D=E.auth.indexOf(":"),~D?(E.username=E.auth.slice(0,D),E.username=encodeURIComponent(decodeURIComponent(E.username)),E.password=E.auth.slice(D+1),E.password=encodeURIComponent(decodeURIComponent(E.password))):E.username=encodeURIComponent(decodeURIComponent(E.auth)),E.auth=E.password?E.username+":"+E.password:E.username),E.origin=E.protocol!=="file:"&&$(E.protocol)&&E.host?E.protocol+"//"+E.host:"null",E.href=E.toString()}function P(m,v,g){var x=this;switch(m){case"query":typeof v=="string"&&v.length&&(v=(g||e.parse)(v)),x[m]=v;break;case"port":x[m]=v,i(v,x.protocol)?v&&(x.host=x.hostname+":"+v):(x.host=x.hostname,x[m]="");break;case"hostname":x[m]=v,x.port&&(v+=":"+x.port),x.host=v;break;case"host":x[m]=v,n.test(v)?(v=v.split(":"),x.port=v.pop(),x.hostname=v.join(":")):(x.hostname=v,x.port="");break;case"protocol":x.protocol=v.toLowerCase(),x.slashes=!g;break;case"pathname":case"hash":if(v){var R=m==="pathname"?"/":"#";x[m]=v.charAt(0)!==R?R+v:v}else x[m]=v;break;case"username":case"password":x[m]=encodeURIComponent(v);break;case"auth":var U=v.indexOf(":");~U?(x.username=v.slice(0,U),x.username=encodeURIComponent(decodeURIComponent(x.username)),x.password=v.slice(U+1),x.password=encodeURIComponent(decodeURIComponent(x.password))):x.username=encodeURIComponent(decodeURIComponent(v))}for(var L=0;L<u.length;L++){var D=u[L];D[4]&&(x[D[1]]=x[D[1]].toLowerCase())}return x.auth=x.password?x.username+":"+x.password:x.username,x.origin=x.protocol!=="file:"&&$(x.protocol)&&x.host?x.protocol+"//"+x.host:"null",x.href=x.toString(),x}function S(m){(!m||typeof m!="function")&&(m=e.stringify);var v,g=this,x=g.host,R=g.protocol;R&&R.charAt(R.length-1)!==":"&&(R+=":");var U=R+(g.protocol&&g.slashes||$(g.protocol)?"//":"");return g.username?(U+=g.username,g.password&&(U+=":"+g.password),U+="@"):g.password?(U+=":"+g.password,U+="@"):g.protocol!=="file:"&&$(g.protocol)&&!x&&g.pathname!=="/"&&(U+="@"),(x[x.length-1]===":"||n.test(g.hostname)&&!g.port)&&(x+=":"),U+=x+g.pathname,v=typeof g.query=="object"?m(g.query):g.query,v&&(U+=v.charAt(0)!=="?"?"?"+v:v),g.hash&&(U+=g.hash),U}return _.prototype={set:P,toString:S},_.extractProtocol=z,_.location=C,_.trimLeft=d,_.qs=e,dt=_,dt}var Vi=Yi();const Gi=Hi(Vi);function Wi(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(i){var e=Math.random()*16|0,t=i==="x"?e:e&3|8;return t.toString(16)})}function vt(){/*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/facebook/regenerator/blob/main/LICENSE */vt=function(){return e};var i,e={},t=Object.prototype,r=t.hasOwnProperty,o=Object.defineProperty||function(f,c,p){f[c]=p.value},n=typeof Symbol=="function"?Symbol:{},s=n.iterator||"@@iterator",l=n.asyncIterator||"@@asyncIterator",d=n.toStringTag||"@@toStringTag";function u(f,c,p){return Object.defineProperty(f,c,{value:p,enumerable:!0,configurable:!0,writable:!0}),f[c]}try{u({},"")}catch{u=function(p,b,k){return p[b]=k}}function h(f,c,p,b){var k=c&&c.prototype instanceof S?c:S,y=Object.create(k.prototype),T=new Q(b||[]);return o(y,"_invoke",{value:G(f,p,T)}),y}function C(f,c,p){try{return{type:"normal",arg:f.call(c,p)}}catch(b){return{type:"throw",arg:b}}}e.wrap=h;var $="suspendedStart",z="suspendedYield",O="executing",_="completed",P={};function S(){}function m(){}function v(){}var g={};u(g,s,function(){return this});var x=Object.getPrototypeOf,R=x&&x(x(pe([])));R&&R!==t&&r.call(R,s)&&(g=R);var U=v.prototype=S.prototype=Object.create(g);function L(f){["next","throw","return"].forEach(function(c){u(f,c,function(p){return this._invoke(c,p)})})}function D(f,c){function p(k,y,T,F){var A=C(f[k],f,y);if(A.type!=="throw"){var Y=A.arg,H=Y.value;return H&&ue(H)=="object"&&r.call(H,"__await")?c.resolve(H.__await).then(function(V){p("next",V,T,F)},function(V){p("throw",V,T,F)}):c.resolve(H).then(function(V){Y.value=V,T(Y)},function(V){return p("throw",V,T,F)})}F(A.arg)}var b;o(this,"_invoke",{value:function(y,T){function F(){return new c(function(A,Y){p(y,T,A,Y)})}return b=b?b.then(F,F):F()}})}function G(f,c,p){var b=$;return function(k,y){if(b===O)throw Error("Generator is already running");if(b===_){if(k==="throw")throw y;return{value:i,done:!0}}for(p.method=k,p.arg=y;;){var T=p.delegate;if(T){var F=re(T,p);if(F){if(F===P)continue;return F}}if(p.method==="next")p.sent=p._sent=p.arg;else if(p.method==="throw"){if(b===$)throw b=_,p.arg;p.dispatchException(p.arg)}else p.method==="return"&&p.abrupt("return",p.arg);b=O;var A=C(f,c,p);if(A.type==="normal"){if(b=p.done?_:z,A.arg===P)continue;return{value:A.arg,done:p.done}}A.type==="throw"&&(b=_,p.method="throw",p.arg=A.arg)}}}function re(f,c){var p=c.method,b=f.iterator[p];if(b===i)return c.delegate=null,p==="throw"&&f.iterator.return&&(c.method="return",c.arg=i,re(f,c),c.method==="throw")||p!=="return"&&(c.method="throw",c.arg=new TypeError("The iterator does not provide a '"+p+"' method")),P;var k=C(b,f.iterator,c.arg);if(k.type==="throw")return c.method="throw",c.arg=k.arg,c.delegate=null,P;var y=k.arg;return y?y.done?(c[f.resultName]=y.value,c.next=f.nextLoc,c.method!=="return"&&(c.method="next",c.arg=i),c.delegate=null,P):y:(c.method="throw",c.arg=new TypeError("iterator result is not an object"),c.delegate=null,P)}function me(f){var c={tryLoc:f[0]};1 in f&&(c.catchLoc=f[1]),2 in f&&(c.finallyLoc=f[2],c.afterLoc=f[3]),this.tryEntries.push(c)}function E(f){var c=f.completion||{};c.type="normal",delete c.arg,f.completion=c}function Q(f){this.tryEntries=[{tryLoc:"root"}],f.forEach(me,this),this.reset(!0)}function pe(f){if(f||f===""){var c=f[s];if(c)return c.call(f);if(typeof f.next=="function")return f;if(!isNaN(f.length)){var p=-1,b=function k(){for(;++p<f.length;)if(r.call(f,p))return k.value=f[p],k.done=!1,k;return k.value=i,k.done=!0,k};return b.next=b}}throw new TypeError(ue(f)+" is not iterable")}return m.prototype=v,o(U,"constructor",{value:v,configurable:!0}),o(v,"constructor",{value:m,configurable:!0}),m.displayName=u(v,d,"GeneratorFunction"),e.isGeneratorFunction=function(f){var c=typeof f=="function"&&f.constructor;return!!c&&(c===m||(c.displayName||c.name)==="GeneratorFunction")},e.mark=function(f){return Object.setPrototypeOf?Object.setPrototypeOf(f,v):(f.__proto__=v,u(f,d,"GeneratorFunction")),f.prototype=Object.create(U),f},e.awrap=function(f){return{__await:f}},L(D.prototype),u(D.prototype,l,function(){return this}),e.AsyncIterator=D,e.async=function(f,c,p,b,k){k===void 0&&(k=Promise);var y=new D(h(f,c,p,b),k);return e.isGeneratorFunction(c)?y:y.next().then(function(T){return T.done?T.value:y.next()})},L(U),u(U,d,"Generator"),u(U,s,function(){return this}),u(U,"toString",function(){return"[object Generator]"}),e.keys=function(f){var c=Object(f),p=[];for(var b in c)p.push(b);return p.reverse(),function k(){for(;p.length;){var y=p.pop();if(y in c)return k.value=y,k.done=!1,k}return k.done=!0,k}},e.values=pe,Q.prototype={constructor:Q,reset:function(c){if(this.prev=0,this.next=0,this.sent=this._sent=i,this.done=!1,this.delegate=null,this.method="next",this.arg=i,this.tryEntries.forEach(E),!c)for(var p in this)p.charAt(0)==="t"&&r.call(this,p)&&!isNaN(+p.slice(1))&&(this[p]=i)},stop:function(){this.done=!0;var c=this.tryEntries[0].completion;if(c.type==="throw")throw c.arg;return this.rval},dispatchException:function(c){if(this.done)throw c;var p=this;function b(Y,H){return T.type="throw",T.arg=c,p.next=Y,H&&(p.method="next",p.arg=i),!!H}for(var k=this.tryEntries.length-1;k>=0;--k){var y=this.tryEntries[k],T=y.completion;if(y.tryLoc==="root")return b("end");if(y.tryLoc<=this.prev){var F=r.call(y,"catchLoc"),A=r.call(y,"finallyLoc");if(F&&A){if(this.prev<y.catchLoc)return b(y.catchLoc,!0);if(this.prev<y.finallyLoc)return b(y.finallyLoc)}else if(F){if(this.prev<y.catchLoc)return b(y.catchLoc,!0)}else{if(!A)throw Error("try statement without catch or finally");if(this.prev<y.finallyLoc)return b(y.finallyLoc)}}}},abrupt:function(c,p){for(var b=this.tryEntries.length-1;b>=0;--b){var k=this.tryEntries[b];if(k.tryLoc<=this.prev&&r.call(k,"finallyLoc")&&this.prev<k.finallyLoc){var y=k;break}}y&&(c==="break"||c==="continue")&&y.tryLoc<=p&&p<=y.finallyLoc&&(y=null);var T=y?y.completion:{};return T.type=c,T.arg=p,y?(this.method="next",this.next=y.finallyLoc,P):this.complete(T)},complete:function(c,p){if(c.type==="throw")throw c.arg;return c.type==="break"||c.type==="continue"?this.next=c.arg:c.type==="return"?(this.rval=this.arg=c.arg,this.method="return",this.next="end"):c.type==="normal"&&p&&(this.next=p),P},finish:function(c){for(var p=this.tryEntries.length-1;p>=0;--p){var b=this.tryEntries[p];if(b.finallyLoc===c)return this.complete(b.completion,b.afterLoc),E(b),P}},catch:function(c){for(var p=this.tryEntries.length-1;p>=0;--p){var b=this.tryEntries[p];if(b.tryLoc===c){var k=b.completion;if(k.type==="throw"){var y=k.arg;E(b)}return y}}throw Error("illegal catch attempt")},delegateYield:function(c,p,b){return this.delegate={iterator:pe(c),resultName:p,nextLoc:b},this.method==="next"&&(this.arg=i),P}},e}function or(i,e,t,r,o,n,s){try{var l=i[n](s),d=l.value}catch(u){t(u);return}l.done?e(d):Promise.resolve(d).then(r,o)}function Xi(i){return function(){var e=this,t=arguments;return new Promise(function(r,o){var n=i.apply(e,t);function s(d){or(n,r,o,s,l,"next",d)}function l(d){or(n,r,o,s,l,"throw",d)}s(void 0)})}}function Mr(i,e){return Ji(i)||Zi(i,e)||Hr(i,e)||Ki()}function Ki(){throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}function Zi(i,e){var t=i==null?null:typeof Symbol<"u"&&i[Symbol.iterator]||i["@@iterator"];if(t!=null){var r,o,n,s,l=[],d=!0,u=!1;try{if(n=(t=t.call(i)).next,e!==0)for(;!(d=(r=n.call(t)).done)&&(l.push(r.value),l.length!==e);d=!0);}catch(h){u=!0,o=h}finally{try{if(!d&&t.return!=null&&(s=t.return(),Object(s)!==s))return}finally{if(u)throw o}}return l}}function Ji(i){if(Array.isArray(i))return i}function ue(i){"@babel/helpers - typeof";return ue=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},ue(i)}function Qi(i,e){var t=typeof Symbol<"u"&&i[Symbol.iterator]||i["@@iterator"];if(!t){if(Array.isArray(i)||(t=Hr(i))||e){t&&(i=t);var r=0,o=function(){};return{s:o,n:function(){return r>=i.length?{done:!0}:{done:!1,value:i[r++]}},e:function(u){throw u},f:o}}throw new TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}var n=!0,s=!1,l;return{s:function(){t=t.call(i)},n:function(){var u=t.next();return n=u.done,u},e:function(u){s=!0,l=u},f:function(){try{!n&&t.return!=null&&t.return()}finally{if(s)throw l}}}}function Hr(i,e){if(i){if(typeof i=="string")return nr(i,e);var t=Object.prototype.toString.call(i).slice(8,-1);if(t==="Object"&&i.constructor&&(t=i.constructor.name),t==="Map"||t==="Set")return Array.from(i);if(t==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t))return nr(i,e)}}function nr(i,e){(e==null||e>i.length)&&(e=i.length);for(var t=0,r=new Array(e);t<e;t++)r[t]=i[t];return r}function sr(i,e){var t=Object.keys(i);if(Object.getOwnPropertySymbols){var r=Object.getOwnPropertySymbols(i);e&&(r=r.filter(function(o){return Object.getOwnPropertyDescriptor(i,o).enumerable})),t.push.apply(t,r)}return t}function xe(i){for(var e=1;e<arguments.length;e++){var t=arguments[e]!=null?arguments[e]:{};e%2?sr(Object(t),!0).forEach(function(r){eo(i,r,t[r])}):Object.getOwnPropertyDescriptors?Object.defineProperties(i,Object.getOwnPropertyDescriptors(t)):sr(Object(t)).forEach(function(r){Object.defineProperty(i,r,Object.getOwnPropertyDescriptor(t,r))})}return i}function eo(i,e,t){return e=qr(e),e in i?Object.defineProperty(i,e,{value:t,enumerable:!0,configurable:!0,writable:!0}):i[e]=t,i}function to(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function ar(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,qr(r.key),r)}}function ro(i,e,t){return e&&ar(i.prototype,e),t&&ar(i,t),Object.defineProperty(i,"prototype",{writable:!1}),i}function qr(i){var e=io(i,"string");return ue(e)=="symbol"?e:e+""}function io(i,e){if(ue(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(ue(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var Ke="tus-v1",Ze="ietf-draft-03",Te="ietf-draft-05",oo={endpoint:null,uploadUrl:null,metadata:{},metadataForPartialUploads:{},fingerprint:null,uploadSize:null,onProgress:null,onChunkComplete:null,onSuccess:null,onError:null,onUploadUrlAvailable:null,overridePatchMethod:!1,headers:{},addRequestId:!1,onBeforeRequest:null,onAfterResponse:null,onShouldRetry:Nr,chunkSize:Number.POSITIVE_INFINITY,retryDelays:[0,1e3,3e3,5e3],parallelUploads:1,parallelUploadBoundaries:null,storeFingerprintForResuming:!0,removeFingerprintOnSuccess:!1,uploadLengthDeferred:!1,uploadDataDuringCreation:!1,urlStorage:null,fileReader:null,httpStack:null,protocol:Ke},Je=(function(){function i(e,t){to(this,i),"resume"in t&&console.log("tus: The `resume` option has been removed in tus-js-client v2. Please use the URL storage API instead."),this.options=t,this.options.chunkSize=Number(this.options.chunkSize),this._urlStorage=this.options.urlStorage,this.file=e,this.url=null,this._req=null,this._fingerprint=null,this._urlStorageKey=null,this._offset=null,this._aborted=!1,this._size=null,this._source=null,this._retryAttempt=0,this._retryTimeout=null,this._offsetBeforeRetry=0,this._parallelUploads=null,this._parallelUploadUrls=null}return ro(i,[{key:"findPreviousUploads",value:function(){var t=this;return this.options.fingerprint(this.file,this.options).then(function(r){return t._urlStorage.findUploadsByFingerprint(r)})}},{key:"resumeFromPreviousUpload",value:function(t){this.url=t.uploadUrl||null,this._parallelUploadUrls=t.parallelUploadUrls||null,this._urlStorageKey=t.urlStorageKey}},{key:"start",value:function(){var t=this,r=this.file;if(!r){this._emitError(new Error("tus: no file or stream to upload provided"));return}if(![Ke,Ze,Te].includes(this.options.protocol)){this._emitError(new Error("tus: unsupported protocol ".concat(this.options.protocol)));return}if(!this.options.endpoint&&!this.options.uploadUrl&&!this.url){this._emitError(new Error("tus: neither an endpoint or an upload URL is provided"));return}var o=this.options.retryDelays;if(o!=null&&Object.prototype.toString.call(o)!=="[object Array]"){this._emitError(new Error("tus: the `retryDelays` option must either be an array or null"));return}if(this.options.parallelUploads>1)for(var n=0,s=["uploadUrl","uploadSize","uploadLengthDeferred"];n<s.length;n++){var l=s[n];if(this.options[l]){this._emitError(new Error("tus: cannot use the ".concat(l," option when parallelUploads is enabled")));return}}if(this.options.parallelUploadBoundaries){if(this.options.parallelUploads<=1){this._emitError(new Error("tus: cannot use the `parallelUploadBoundaries` option when `parallelUploads` is disabled"));return}if(this.options.parallelUploads!==this.options.parallelUploadBoundaries.length){this._emitError(new Error("tus: the `parallelUploadBoundaries` must have the same length as the value of `parallelUploads`"));return}}this.options.fingerprint(r,this.options).then(function(d){return t._fingerprint=d,t._source?t._source:t.options.fileReader.openFile(r,t.options.chunkSize)}).then(function(d){if(t._source=d,t.options.uploadLengthDeferred)t._size=null;else if(t.options.uploadSize!=null){if(t._size=Number(t.options.uploadSize),Number.isNaN(t._size)){t._emitError(new Error("tus: cannot convert `uploadSize` option into a number"));return}}else if(t._size=t._source.size,t._size==null){t._emitError(new Error("tus: cannot automatically derive upload's size from input. Specify it manually using the `uploadSize` option or use the `uploadLengthDeferred` option"));return}t.options.parallelUploads>1||t._parallelUploadUrls!=null?t._startParallelUpload():t._startSingleUpload()}).catch(function(d){t._emitError(d)})}},{key:"_startParallelUpload",value:function(){var t,r=this,o=this._size,n=0;this._parallelUploads=[];var s=this._parallelUploadUrls!=null?this._parallelUploadUrls.length:this.options.parallelUploads,l=(t=this.options.parallelUploadBoundaries)!==null&&t!==void 0?t:so(this._source.size,s);this._parallelUploadUrls&&l.forEach(function(h,C){h.uploadUrl=r._parallelUploadUrls[C]||null}),this._parallelUploadUrls=new Array(l.length);var d=l.map(function(h,C){var $=0;return r._source.slice(h.start,h.end).then(function(z){var O=z.value;return new Promise(function(_,P){var S=xe(xe({},r.options),{},{uploadUrl:h.uploadUrl||null,storeFingerprintForResuming:!1,removeFingerprintOnSuccess:!1,parallelUploads:1,parallelUploadBoundaries:null,metadata:r.options.metadataForPartialUploads,headers:xe(xe({},r.options.headers),{},{"Upload-Concat":"partial"}),onSuccess:_,onError:P,onProgress:function(g){n=n-$+g,$=g,r._emitProgress(n,o)},onUploadUrlAvailable:function(){r._parallelUploadUrls[C]=m.url,r._parallelUploadUrls.filter(function(g){return!!g}).length===l.length&&r._saveUploadInUrlStorage()}}),m=new i(O,S);m.start(),r._parallelUploads.push(m)})})}),u;Promise.all(d).then(function(){u=r._openRequest("POST",r.options.endpoint),u.setHeader("Upload-Concat","final;".concat(r._parallelUploadUrls.join(" ")));var h=lr(r.options.metadata);return h!==""&&u.setHeader("Upload-Metadata",h),r._sendRequest(u,null)}).then(function(h){if(!we(h.getStatus(),200)){r._emitHttpError(u,h,"tus: unexpected response while creating upload");return}var C=h.getHeader("Location");if(C==null){r._emitHttpError(u,h,"tus: invalid or missing Location header");return}r.url=ur(r.options.endpoint,C),"Created upload at ".concat(r.url),r._emitSuccess(h)}).catch(function(h){r._emitError(h)})}},{key:"_startSingleUpload",value:function(){if(this._aborted=!1,this.url!=null){"Resuming upload from previous URL: ".concat(this.url),this._resumeUpload();return}if(this.options.uploadUrl!=null){"Resuming upload from provided URL: ".concat(this.options.uploadUrl),this.url=this.options.uploadUrl,this._resumeUpload();return}this._createUpload()}},{key:"abort",value:function(t){var r=this;if(this._parallelUploads!=null){var o=Qi(this._parallelUploads),n;try{for(o.s();!(n=o.n()).done;){var s=n.value;s.abort(t)}}catch(l){o.e(l)}finally{o.f()}}return this._req!==null&&this._req.abort(),this._aborted=!0,this._retryTimeout!=null&&(clearTimeout(this._retryTimeout),this._retryTimeout=null),!t||this.url==null?Promise.resolve():i.terminate(this.url,this.options).then(function(){return r._removeFromUrlStorage()})}},{key:"_emitHttpError",value:function(t,r,o,n){this._emitError(new ze(o,n,t,r))}},{key:"_emitError",value:function(t){var r=this;if(!this._aborted){if(this.options.retryDelays!=null){var o=this._offset!=null&&this._offset>this._offsetBeforeRetry;if(o&&(this._retryAttempt=0),pr(t,this._retryAttempt,this.options)){var n=this.options.retryDelays[this._retryAttempt++];this._offsetBeforeRetry=this._offset,this._retryTimeout=setTimeout(function(){r.start()},n);return}}if(typeof this.options.onError=="function")this.options.onError(t);else throw t}}},{key:"_emitSuccess",value:function(t){this.options.removeFingerprintOnSuccess&&this._removeFromUrlStorage(),typeof this.options.onSuccess=="function"&&this.options.onSuccess({lastResponse:t})}},{key:"_emitProgress",value:function(t,r){typeof this.options.onProgress=="function"&&this.options.onProgress(t,r)}},{key:"_emitChunkComplete",value:function(t,r,o){typeof this.options.onChunkComplete=="function"&&this.options.onChunkComplete(t,r,o)}},{key:"_createUpload",value:function(){var t=this;if(!this.options.endpoint){this._emitError(new Error("tus: unable to create upload because no endpoint is provided"));return}var r=this._openRequest("POST",this.options.endpoint);this.options.uploadLengthDeferred?r.setHeader("Upload-Defer-Length","1"):r.setHeader("Upload-Length","".concat(this._size));var o=lr(this.options.metadata);o!==""&&r.setHeader("Upload-Metadata",o);var n;this.options.uploadDataDuringCreation&&!this.options.uploadLengthDeferred?(this._offset=0,n=this._addChunkToRequest(r)):((this.options.protocol===Ze||this.options.protocol===Te)&&r.setHeader("Upload-Complete","?0"),n=this._sendRequest(r,null)),n.then(function(s){if(!we(s.getStatus(),200)){t._emitHttpError(r,s,"tus: unexpected response while creating upload");return}var l=s.getHeader("Location");if(l==null){t._emitHttpError(r,s,"tus: invalid or missing Location header");return}if(t.url=ur(t.options.endpoint,l),"Created upload at ".concat(t.url),typeof t.options.onUploadUrlAvailable=="function"&&t.options.onUploadUrlAvailable(),t._size===0){t._emitSuccess(s),t._source.close();return}t._saveUploadInUrlStorage().then(function(){t.options.uploadDataDuringCreation?t._handleUploadResponse(r,s):(t._offset=0,t._performUpload())})}).catch(function(s){t._emitHttpError(r,null,"tus: failed to create upload",s)})}},{key:"_resumeUpload",value:function(){var t=this,r=this._openRequest("HEAD",this.url),o=this._sendRequest(r,null);o.then(function(n){var s=n.getStatus();if(!we(s,200)){if(s===423){t._emitHttpError(r,n,"tus: upload is currently locked; retry later");return}if(we(s,400)&&t._removeFromUrlStorage(),!t.options.endpoint){t._emitHttpError(r,n,"tus: unable to resume upload (new upload cannot be created without an endpoint)");return}t.url=null,t._createUpload();return}var l=Number.parseInt(n.getHeader("Upload-Offset"),10);if(Number.isNaN(l)){t._emitHttpError(r,n,"tus: invalid or missing offset value");return}var d=Number.parseInt(n.getHeader("Upload-Length"),10);if(Number.isNaN(d)&&!t.options.uploadLengthDeferred&&t.options.protocol===Ke){t._emitHttpError(r,n,"tus: invalid or missing length value");return}typeof t.options.onUploadUrlAvailable=="function"&&t.options.onUploadUrlAvailable(),t._saveUploadInUrlStorage().then(function(){if(l===d){t._emitProgress(d,d),t._emitSuccess(n);return}t._offset=l,t._performUpload()})}).catch(function(n){t._emitHttpError(r,null,"tus: failed to resume upload",n)})}},{key:"_performUpload",value:function(){var t=this;if(!this._aborted){var r;this.options.overridePatchMethod?(r=this._openRequest("POST",this.url),r.setHeader("X-HTTP-Method-Override","PATCH")):r=this._openRequest("PATCH",this.url),r.setHeader("Upload-Offset","".concat(this._offset));var o=this._addChunkToRequest(r);o.then(function(n){if(!we(n.getStatus(),200)){t._emitHttpError(r,n,"tus: unexpected response while uploading chunk");return}t._handleUploadResponse(r,n)}).catch(function(n){t._aborted||t._emitHttpError(r,null,"tus: failed to upload chunk at offset ".concat(t._offset),n)})}}},{key:"_addChunkToRequest",value:function(t){var r=this,o=this._offset,n=this._offset+this.options.chunkSize;return t.setProgressHandler(function(s){r._emitProgress(o+s,r._size)}),this.options.protocol===Ke?t.setHeader("Content-Type","application/offset+octet-stream"):this.options.protocol===Te&&t.setHeader("Content-Type","application/partial-upload"),(n===Number.POSITIVE_INFINITY||n>this._size)&&!this.options.uploadLengthDeferred&&(n=this._size),this._source.slice(o,n).then(function(s){var l=s.value,d=s.done,u=l!=null&&l.size?l.size:0;r.options.uploadLengthDeferred&&d&&(r._size=r._offset+u,t.setHeader("Upload-Length","".concat(r._size)));var h=r._offset+u;return!r.options.uploadLengthDeferred&&d&&h!==r._size?Promise.reject(new Error("upload was configured with a size of ".concat(r._size," bytes, but the source is done after ").concat(h," bytes"))):l===null?r._sendRequest(t):((r.options.protocol===Ze||r.options.protocol===Te)&&t.setHeader("Upload-Complete",d?"?1":"?0"),r._emitProgress(r._offset,r._size),r._sendRequest(t,l))})}},{key:"_handleUploadResponse",value:function(t,r){var o=Number.parseInt(r.getHeader("Upload-Offset"),10);if(Number.isNaN(o)){this._emitHttpError(t,r,"tus: invalid or missing offset value");return}if(this._emitProgress(o,this._size),this._emitChunkComplete(o-this._offset,o,this._size),this._offset=o,o===this._size){this._emitSuccess(r),this._source.close();return}this._performUpload()}},{key:"_openRequest",value:function(t,r){var o=dr(t,r,this.options);return this._req=o,o}},{key:"_removeFromUrlStorage",value:function(){var t=this;this._urlStorageKey&&(this._urlStorage.removeUpload(this._urlStorageKey).catch(function(r){t._emitError(r)}),this._urlStorageKey=null)}},{key:"_saveUploadInUrlStorage",value:function(){var t=this;if(!this.options.storeFingerprintForResuming||!this._fingerprint||this._urlStorageKey!==null)return Promise.resolve();var r={size:this._size,metadata:this.options.metadata,creationTime:new Date().toString()};return this._parallelUploads?r.parallelUploadUrls=this._parallelUploadUrls:r.uploadUrl=this.url,this._urlStorage.addUpload(this._fingerprint,r).then(function(o){t._urlStorageKey=o})}},{key:"_sendRequest",value:function(t){var r=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null;return cr(t,r,this.options)}}],[{key:"terminate",value:function(t){var r=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},o=dr("DELETE",t,r);return cr(o,null,r).then(function(n){if(n.getStatus()!==204)throw new ze("tus: unexpected response while terminating upload",null,o,n)}).catch(function(n){if(n instanceof ze||(n=new ze("tus: failed to terminate upload",n,o,null)),!pr(n,0,r))throw n;var s=r.retryDelays[0],l=r.retryDelays.slice(1),d=xe(xe({},r),{},{retryDelays:l});return new Promise(function(u){return setTimeout(u,s)}).then(function(){return i.terminate(t,d)})})}}])})();function lr(i){return Object.entries(i).map(function(e){var t=Mr(e,2),r=t[0],o=t[1];return"".concat(r," ").concat(Mi.encode(String(o)))}).join(",")}function we(i,e){return i>=e&&i<e+100}function dr(i,e,t){var r=t.httpStack.createRequest(i,e);t.protocol===Ze?r.setHeader("Upload-Draft-Interop-Version","5"):t.protocol===Te?r.setHeader("Upload-Draft-Interop-Version","6"):r.setHeader("Tus-Resumable","1.0.0");for(var o=t.headers||{},n=0,s=Object.entries(o);n<s.length;n++){var l=Mr(s[n],2),d=l[0],u=l[1];r.setHeader(d,u)}if(t.addRequestId){var h=Wi();r.setHeader("X-Request-ID",h)}return r}function cr(i,e,t){return xt.apply(this,arguments)}function xt(){return xt=Xi(vt().mark(function i(e,t,r){var o;return vt().wrap(function(s){for(;;)switch(s.prev=s.next){case 0:if(typeof r.onBeforeRequest!="function"){s.next=3;break}return s.next=3,r.onBeforeRequest(e);case 3:return s.next=5,e.send(t);case 5:if(o=s.sent,typeof r.onAfterResponse!="function"){s.next=9;break}return s.next=9,r.onAfterResponse(e,o);case 9:return s.abrupt("return",o);case 10:case"end":return s.stop()}},i)})),xt.apply(this,arguments)}function no(){var i=!0;return typeof navigator<"u"&&navigator.onLine===!1&&(i=!1),i}function pr(i,e,t){return t.retryDelays==null||e>=t.retryDelays.length||i.originalRequest==null?!1:t&&typeof t.onShouldRetry=="function"?t.onShouldRetry(i,e,t):Nr(i)}function Nr(i){var e=i.originalResponse?i.originalResponse.getStatus():0;return(!we(e,400)||e===409||e===423)&&no()}function ur(i,e){return new Gi(e,i).toString()}function so(i,e){for(var t=Math.floor(i/e),r=[],o=0;o<e;o++)r.push({start:t*o,end:t*(o+1)});return r[e-1].end=i,r}Je.defaultOptions=oo;var Yr=function(){return typeof navigator<"u"&&typeof navigator.product=="string"&&navigator.product.toLowerCase()==="reactnative"};function ao(i){return new Promise(function(e,t){var r=new XMLHttpRequest;r.responseType="blob",r.onload=function(){var o=r.response;e(o)},r.onerror=function(o){t(o)},r.open("GET",i),r.send()})}var lo=function(){return typeof window<"u"&&(typeof window.PhoneGap<"u"||typeof window.Cordova<"u"||typeof window.cordova<"u")};function co(i){return new Promise(function(e,t){var r=new FileReader;r.onload=function(){var o=new Uint8Array(r.result);e({value:o})},r.onerror=function(o){t(o)},r.readAsArrayBuffer(i)})}function Fe(i){"@babel/helpers - typeof";return Fe=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},Fe(i)}function po(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function uo(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,ho(r.key),r)}}function fo(i,e,t){return e&&uo(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function ho(i){var e=go(i,"string");return Fe(e)=="symbol"?e:e+""}function go(i,e){if(Fe(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(Fe(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var fr=(function(){function i(e){po(this,i),this._file=e,this.size=e.size}return fo(i,[{key:"slice",value:function(t,r){if(lo())return co(this._file.slice(t,r));var o=this._file.slice(t,r),n=r>=this.size;return Promise.resolve({value:o,done:n})}},{key:"close",value:function(){}}])})();function Ae(i){"@babel/helpers - typeof";return Ae=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},Ae(i)}function mo(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function vo(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,bo(r.key),r)}}function xo(i,e,t){return e&&vo(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function bo(i){var e=yo(i,"string");return Ae(e)=="symbol"?e:e+""}function yo(i,e){if(Ae(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(Ae(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}function hr(i){return i===void 0?0:i.size!==void 0?i.size:i.length}function wo(i,e){if(i.concat)return i.concat(e);if(i instanceof Blob)return new Blob([i,e],{type:i.type});if(i.set){var t=new i.constructor(i.length+e.length);return t.set(i),t.set(e,i.length),t}throw new Error("Unknown data type")}var _o=(function(){function i(e){mo(this,i),this._buffer=void 0,this._bufferOffset=0,this._reader=e,this._done=!1}return xo(i,[{key:"slice",value:function(t,r){return t<this._bufferOffset?Promise.reject(new Error("Requested data is before the reader's current offset")):this._readUntilEnoughDataOrDone(t,r)}},{key:"_readUntilEnoughDataOrDone",value:function(t,r){var o=this,n=r<=this._bufferOffset+hr(this._buffer);if(this._done||n){var s=this._getDataFromBuffer(t,r),l=s==null?this._done:!1;return Promise.resolve({value:s,done:l})}return this._reader.read().then(function(d){var u=d.value,h=d.done;return h?o._done=!0:o._buffer===void 0?o._buffer=u:o._buffer=wo(o._buffer,u),o._readUntilEnoughDataOrDone(t,r)})}},{key:"_getDataFromBuffer",value:function(t,r){t>this._bufferOffset&&(this._buffer=this._buffer.slice(t-this._bufferOffset),this._bufferOffset=t);var o=hr(this._buffer)===0;return this._done&&o?null:this._buffer.slice(0,r-t)}},{key:"close",value:function(){this._reader.cancel&&this._reader.cancel()}}])})();function fe(i){"@babel/helpers - typeof";return fe=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},fe(i)}function bt(){/*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/facebook/regenerator/blob/main/LICENSE */bt=function(){return e};var i,e={},t=Object.prototype,r=t.hasOwnProperty,o=Object.defineProperty||function(f,c,p){f[c]=p.value},n=typeof Symbol=="function"?Symbol:{},s=n.iterator||"@@iterator",l=n.asyncIterator||"@@asyncIterator",d=n.toStringTag||"@@toStringTag";function u(f,c,p){return Object.defineProperty(f,c,{value:p,enumerable:!0,configurable:!0,writable:!0}),f[c]}try{u({},"")}catch{u=function(p,b,k){return p[b]=k}}function h(f,c,p,b){var k=c&&c.prototype instanceof S?c:S,y=Object.create(k.prototype),T=new Q(b||[]);return o(y,"_invoke",{value:G(f,p,T)}),y}function C(f,c,p){try{return{type:"normal",arg:f.call(c,p)}}catch(b){return{type:"throw",arg:b}}}e.wrap=h;var $="suspendedStart",z="suspendedYield",O="executing",_="completed",P={};function S(){}function m(){}function v(){}var g={};u(g,s,function(){return this});var x=Object.getPrototypeOf,R=x&&x(x(pe([])));R&&R!==t&&r.call(R,s)&&(g=R);var U=v.prototype=S.prototype=Object.create(g);function L(f){["next","throw","return"].forEach(function(c){u(f,c,function(p){return this._invoke(c,p)})})}function D(f,c){function p(k,y,T,F){var A=C(f[k],f,y);if(A.type!=="throw"){var Y=A.arg,H=Y.value;return H&&fe(H)=="object"&&r.call(H,"__await")?c.resolve(H.__await).then(function(V){p("next",V,T,F)},function(V){p("throw",V,T,F)}):c.resolve(H).then(function(V){Y.value=V,T(Y)},function(V){return p("throw",V,T,F)})}F(A.arg)}var b;o(this,"_invoke",{value:function(y,T){function F(){return new c(function(A,Y){p(y,T,A,Y)})}return b=b?b.then(F,F):F()}})}function G(f,c,p){var b=$;return function(k,y){if(b===O)throw Error("Generator is already running");if(b===_){if(k==="throw")throw y;return{value:i,done:!0}}for(p.method=k,p.arg=y;;){var T=p.delegate;if(T){var F=re(T,p);if(F){if(F===P)continue;return F}}if(p.method==="next")p.sent=p._sent=p.arg;else if(p.method==="throw"){if(b===$)throw b=_,p.arg;p.dispatchException(p.arg)}else p.method==="return"&&p.abrupt("return",p.arg);b=O;var A=C(f,c,p);if(A.type==="normal"){if(b=p.done?_:z,A.arg===P)continue;return{value:A.arg,done:p.done}}A.type==="throw"&&(b=_,p.method="throw",p.arg=A.arg)}}}function re(f,c){var p=c.method,b=f.iterator[p];if(b===i)return c.delegate=null,p==="throw"&&f.iterator.return&&(c.method="return",c.arg=i,re(f,c),c.method==="throw")||p!=="return"&&(c.method="throw",c.arg=new TypeError("The iterator does not provide a '"+p+"' method")),P;var k=C(b,f.iterator,c.arg);if(k.type==="throw")return c.method="throw",c.arg=k.arg,c.delegate=null,P;var y=k.arg;return y?y.done?(c[f.resultName]=y.value,c.next=f.nextLoc,c.method!=="return"&&(c.method="next",c.arg=i),c.delegate=null,P):y:(c.method="throw",c.arg=new TypeError("iterator result is not an object"),c.delegate=null,P)}function me(f){var c={tryLoc:f[0]};1 in f&&(c.catchLoc=f[1]),2 in f&&(c.finallyLoc=f[2],c.afterLoc=f[3]),this.tryEntries.push(c)}function E(f){var c=f.completion||{};c.type="normal",delete c.arg,f.completion=c}function Q(f){this.tryEntries=[{tryLoc:"root"}],f.forEach(me,this),this.reset(!0)}function pe(f){if(f||f===""){var c=f[s];if(c)return c.call(f);if(typeof f.next=="function")return f;if(!isNaN(f.length)){var p=-1,b=function k(){for(;++p<f.length;)if(r.call(f,p))return k.value=f[p],k.done=!1,k;return k.value=i,k.done=!0,k};return b.next=b}}throw new TypeError(fe(f)+" is not iterable")}return m.prototype=v,o(U,"constructor",{value:v,configurable:!0}),o(v,"constructor",{value:m,configurable:!0}),m.displayName=u(v,d,"GeneratorFunction"),e.isGeneratorFunction=function(f){var c=typeof f=="function"&&f.constructor;return!!c&&(c===m||(c.displayName||c.name)==="GeneratorFunction")},e.mark=function(f){return Object.setPrototypeOf?Object.setPrototypeOf(f,v):(f.__proto__=v,u(f,d,"GeneratorFunction")),f.prototype=Object.create(U),f},e.awrap=function(f){return{__await:f}},L(D.prototype),u(D.prototype,l,function(){return this}),e.AsyncIterator=D,e.async=function(f,c,p,b,k){k===void 0&&(k=Promise);var y=new D(h(f,c,p,b),k);return e.isGeneratorFunction(c)?y:y.next().then(function(T){return T.done?T.value:y.next()})},L(U),u(U,d,"Generator"),u(U,s,function(){return this}),u(U,"toString",function(){return"[object Generator]"}),e.keys=function(f){var c=Object(f),p=[];for(var b in c)p.push(b);return p.reverse(),function k(){for(;p.length;){var y=p.pop();if(y in c)return k.value=y,k.done=!1,k}return k.done=!0,k}},e.values=pe,Q.prototype={constructor:Q,reset:function(c){if(this.prev=0,this.next=0,this.sent=this._sent=i,this.done=!1,this.delegate=null,this.method="next",this.arg=i,this.tryEntries.forEach(E),!c)for(var p in this)p.charAt(0)==="t"&&r.call(this,p)&&!isNaN(+p.slice(1))&&(this[p]=i)},stop:function(){this.done=!0;var c=this.tryEntries[0].completion;if(c.type==="throw")throw c.arg;return this.rval},dispatchException:function(c){if(this.done)throw c;var p=this;function b(Y,H){return T.type="throw",T.arg=c,p.next=Y,H&&(p.method="next",p.arg=i),!!H}for(var k=this.tryEntries.length-1;k>=0;--k){var y=this.tryEntries[k],T=y.completion;if(y.tryLoc==="root")return b("end");if(y.tryLoc<=this.prev){var F=r.call(y,"catchLoc"),A=r.call(y,"finallyLoc");if(F&&A){if(this.prev<y.catchLoc)return b(y.catchLoc,!0);if(this.prev<y.finallyLoc)return b(y.finallyLoc)}else if(F){if(this.prev<y.catchLoc)return b(y.catchLoc,!0)}else{if(!A)throw Error("try statement without catch or finally");if(this.prev<y.finallyLoc)return b(y.finallyLoc)}}}},abrupt:function(c,p){for(var b=this.tryEntries.length-1;b>=0;--b){var k=this.tryEntries[b];if(k.tryLoc<=this.prev&&r.call(k,"finallyLoc")&&this.prev<k.finallyLoc){var y=k;break}}y&&(c==="break"||c==="continue")&&y.tryLoc<=p&&p<=y.finallyLoc&&(y=null);var T=y?y.completion:{};return T.type=c,T.arg=p,y?(this.method="next",this.next=y.finallyLoc,P):this.complete(T)},complete:function(c,p){if(c.type==="throw")throw c.arg;return c.type==="break"||c.type==="continue"?this.next=c.arg:c.type==="return"?(this.rval=this.arg=c.arg,this.method="return",this.next="end"):c.type==="normal"&&p&&(this.next=p),P},finish:function(c){for(var p=this.tryEntries.length-1;p>=0;--p){var b=this.tryEntries[p];if(b.finallyLoc===c)return this.complete(b.completion,b.afterLoc),E(b),P}},catch:function(c){for(var p=this.tryEntries.length-1;p>=0;--p){var b=this.tryEntries[p];if(b.tryLoc===c){var k=b.completion;if(k.type==="throw"){var y=k.arg;E(b)}return y}}throw Error("illegal catch attempt")},delegateYield:function(c,p,b){return this.delegate={iterator:pe(c),resultName:p,nextLoc:b},this.method==="next"&&(this.arg=i),P}},e}function gr(i,e,t,r,o,n,s){try{var l=i[n](s),d=l.value}catch(u){t(u);return}l.done?e(d):Promise.resolve(d).then(r,o)}function ko(i){return function(){var e=this,t=arguments;return new Promise(function(r,o){var n=i.apply(e,t);function s(d){gr(n,r,o,s,l,"next",d)}function l(d){gr(n,r,o,s,l,"throw",d)}s(void 0)})}}function Co(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function So(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,Uo(r.key),r)}}function $o(i,e,t){return e&&So(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function Uo(i){var e=Eo(i,"string");return fe(e)=="symbol"?e:e+""}function Eo(i,e){if(fe(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(fe(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var Po=(function(){function i(){Co(this,i)}return $o(i,[{key:"openFile",value:(function(){var e=ko(bt().mark(function r(o,n){var s;return bt().wrap(function(d){for(;;)switch(d.prev=d.next){case 0:if(!(Yr()&&o&&typeof o.uri<"u")){d.next=11;break}return d.prev=1,d.next=4,ao(o.uri);case 4:return s=d.sent,d.abrupt("return",new fr(s));case 8:throw d.prev=8,d.t0=d.catch(1),new Error("tus: cannot fetch `file.uri` as Blob, make sure the uri is correct and accessible. ".concat(d.t0));case 11:if(!(typeof o.slice=="function"&&typeof o.size<"u")){d.next=13;break}return d.abrupt("return",Promise.resolve(new fr(o)));case 13:if(typeof o.read!="function"){d.next=18;break}if(n=Number(n),Number.isFinite(n)){d.next=17;break}return d.abrupt("return",Promise.reject(new Error("cannot create source for stream without a finite value for the `chunkSize` option")));case 17:return d.abrupt("return",Promise.resolve(new _o(o,n)));case 18:return d.abrupt("return",Promise.reject(new Error("source object may only be an instance of File, Blob, or Reader in this environment")));case 19:case"end":return d.stop()}},r,null,[[1,8]])}));function t(r,o){return e.apply(this,arguments)}return t})()}])})();function Oo(i,e){return Yr()?Promise.resolve(zo(i,e)):Promise.resolve(["tus-br",i.name,i.type,i.size,i.lastModified,e.endpoint].join("-"))}function zo(i,e){var t=i.exif?Ro(JSON.stringify(i.exif)):"noexif";return["tus-rn",i.name||"noname",i.size||"nosize",t,e.endpoint].join("/")}function Ro(i){var e=0;if(i.length===0)return e;for(var t=0;t<i.length;t++){var r=i.charCodeAt(t);e=(e<<5)-e+r,e&=e}return e}function Be(i){"@babel/helpers - typeof";return Be=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},Be(i)}function Ot(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function To(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,Do(r.key),r)}}function zt(i,e,t){return e&&To(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function Do(i){var e=Lo(i,"string");return Be(e)=="symbol"?e:e+""}function Lo(i,e){if(Be(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(Be(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var jo=(function(){function i(){Ot(this,i)}return zt(i,[{key:"createRequest",value:function(t,r){return new Fo(t,r)}},{key:"getName",value:function(){return"XHRHttpStack"}}])})(),Fo=(function(){function i(e,t){Ot(this,i),this._xhr=new XMLHttpRequest,this._xhr.open(e,t,!0),this._method=e,this._url=t,this._headers={}}return zt(i,[{key:"getMethod",value:function(){return this._method}},{key:"getURL",value:function(){return this._url}},{key:"setHeader",value:function(t,r){this._xhr.setRequestHeader(t,r),this._headers[t]=r}},{key:"getHeader",value:function(t){return this._headers[t]}},{key:"setProgressHandler",value:function(t){"upload"in this._xhr&&(this._xhr.upload.onprogress=function(r){r.lengthComputable&&t(r.loaded)})}},{key:"send",value:function(){var t=this,r=arguments.length>0&&arguments[0]!==void 0?arguments[0]:null;return new Promise(function(o,n){t._xhr.onload=function(){o(new Ao(t._xhr))},t._xhr.onerror=function(s){n(s)},t._xhr.send(r)})}},{key:"abort",value:function(){return this._xhr.abort(),Promise.resolve()}},{key:"getUnderlyingObject",value:function(){return this._xhr}}])})(),Ao=(function(){function i(e){Ot(this,i),this._xhr=e}return zt(i,[{key:"getStatus",value:function(){return this._xhr.status}},{key:"getHeader",value:function(t){return this._xhr.getResponseHeader(t)}},{key:"getBody",value:function(){return this._xhr.responseText}},{key:"getUnderlyingObject",value:function(){return this._xhr}}])})();function Ie(i){"@babel/helpers - typeof";return Ie=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},Ie(i)}function Bo(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function Io(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,Ho(r.key),r)}}function Mo(i,e,t){return e&&Io(i.prototype,e),Object.defineProperty(i,"prototype",{writable:!1}),i}function Ho(i){var e=qo(i,"string");return Ie(e)=="symbol"?e:e+""}function qo(i,e){if(Ie(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(Ie(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(i)}var yt=!1;try{yt="localStorage"in window;var ct="tusSupport",mr=localStorage.getItem(ct);localStorage.setItem(ct,mr),mr===null&&localStorage.removeItem(ct)}catch(i){if(i.code===i.SECURITY_ERR||i.code===i.QUOTA_EXCEEDED_ERR)yt=!1;else throw i}var No=yt,Yo=(function(){function i(){Bo(this,i)}return Mo(i,[{key:"findAllUploads",value:function(){var t=this._findEntries("tus::");return Promise.resolve(t)}},{key:"findUploadsByFingerprint",value:function(t){var r=this._findEntries("tus::".concat(t,"::"));return Promise.resolve(r)}},{key:"removeUpload",value:function(t){return localStorage.removeItem(t),Promise.resolve()}},{key:"addUpload",value:function(t,r){var o=Math.round(Math.random()*1e12),n="tus::".concat(t,"::").concat(o);return localStorage.setItem(n,JSON.stringify(r)),Promise.resolve(n)}},{key:"_findEntries",value:function(t){for(var r=[],o=0;o<localStorage.length;o++){var n=localStorage.key(o);if(n.indexOf(t)===0)try{var s=JSON.parse(localStorage.getItem(n));s.urlStorageKey=n,r.push(s)}catch{}}return r}}])})();function Se(i){"@babel/helpers - typeof";return Se=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(e){return typeof e}:function(e){return e&&typeof Symbol=="function"&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e},Se(i)}function Vo(i,e){if(!(i instanceof e))throw new TypeError("Cannot call a class as a function")}function Go(i,e){for(var t=0;t<e.length;t++){var r=e[t];r.enumerable=r.enumerable||!1,r.configurable=!0,"value"in r&&(r.writable=!0),Object.defineProperty(i,Gr(r.key),r)}}function Wo(i,e,t){return t&&Go(i,t),Object.defineProperty(i,"prototype",{writable:!1}),i}function Xo(i,e,t){return e=Qe(e),Ko(i,Vr()?Reflect.construct(e,t||[],Qe(i).constructor):e.apply(i,t))}function Ko(i,e){if(e&&(Se(e)==="object"||typeof e=="function"))return e;if(e!==void 0)throw new TypeError("Derived constructors may only return object or undefined");return Zo(i)}function Zo(i){if(i===void 0)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return i}function Vr(){try{var i=!Boolean.prototype.valueOf.call(Reflect.construct(Boolean,[],function(){}))}catch{}return(Vr=function(){return!!i})()}function Qe(i){return Qe=Object.setPrototypeOf?Object.getPrototypeOf.bind():function(t){return t.__proto__||Object.getPrototypeOf(t)},Qe(i)}function Jo(i,e){if(typeof e!="function"&&e!==null)throw new TypeError("Super expression must either be null or a function");i.prototype=Object.create(e&&e.prototype,{constructor:{value:i,writable:!0,configurable:!0}}),Object.defineProperty(i,"prototype",{writable:!1}),e&&wt(i,e)}function wt(i,e){return wt=Object.setPrototypeOf?Object.setPrototypeOf.bind():function(r,o){return r.__proto__=o,r},wt(i,e)}function vr(i,e){var t=Object.keys(i);if(Object.getOwnPropertySymbols){var r=Object.getOwnPropertySymbols(i);e&&(r=r.filter(function(o){return Object.getOwnPropertyDescriptor(i,o).enumerable})),t.push.apply(t,r)}return t}function _e(i){for(var e=1;e<arguments.length;e++){var t=arguments[e]!=null?arguments[e]:{};e%2?vr(Object(t),!0).forEach(function(r){Qo(i,r,t[r])}):Object.getOwnPropertyDescriptors?Object.defineProperties(i,Object.getOwnPropertyDescriptors(t)):vr(Object(t)).forEach(function(r){Object.defineProperty(i,r,Object.getOwnPropertyDescriptor(t,r))})}return i}function Qo(i,e,t){return e=Gr(e),e in i?Object.defineProperty(i,e,{value:t,enumerable:!0,configurable:!0,writable:!0}):i[e]=t,i}function Gr(i){var e=en(i,"string");return Se(e)=="symbol"?e:e+""}function en(i,e){if(Se(i)!="object"||!i)return i;var t=i[Symbol.toPrimitive];if(t!==void 0){var r=t.call(i,e);if(Se(r)!="object")return r;throw new TypeError("@@toPrimitive must return a primitive value.")}return(e==="string"?String:Number)(i)}var xr=_e(_e({},Je.defaultOptions),{},{httpStack:new jo,fileReader:new Po,urlStorage:No?new Yo:new Oi,fingerprint:Oo}),tn=(function(i){function e(){var t=arguments.length>0&&arguments[0]!==void 0?arguments[0]:null,r=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{};return Vo(this,e),r=_e(_e({},xr),r),Xo(this,e,[t,r])}return Jo(e,i),Wo(e,null,[{key:"terminate",value:function(r){var o=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{};return o=_e(_e({},xr),o),Je.terminate(r,o)}}])})(Je);const rn=10*1024*1024,on=5*1024*1024,nn="https://eu-on-24001.connector.filerobot.com/files",sn="https://eu-on-24001.connector.filerobot.com/json";function an(i,e){if(!e||!i.file)return!1;const t=e.sizeThreshold??rn;return i.size>=t}function ln(i,e){const{tusConfig:t}=e,r=e.apiBase.replace(/\/+$/,""),o=t.endpoint||nn,n=t.chunkSize??on,s=t.resumable!==!1,l=t.parallelChunks??1,d=t.retryDelays??[0,1e3,3e3,5e3],u=r.split("/").pop()||"";let h=!1,C=!1,$=!1;const z={name:i.name,type:i.type,"filerobot-folder":e.folder},O=async()=>`tus-${i.id}-${o}`,_=new tn(i.file,{endpoint:o,chunkSize:n,retryDelays:d,parallelUploads:l,storeFingerprintForResuming:s,removeFingerprintOnSuccess:!0,headers:{},metadata:z,fingerprint:O,onBeforeRequest(g){const x=e.getAuthHeaders?e.getAuthHeaders():e.authHeaders;for(const[R,U]of Object.entries(x))g.setHeader(R,U);g.setHeader("X-Filerobot-Token",u)},onUploadUrlAvailable(){_.url&&e.onUploadUrlAvailable&&!$&&($=!0,e.onUploadUrlAvailable(_.url))},onProgress(g,x){!C&&!h&&e.onProgress(g,x)},onSuccess(){var R;if(C)return;m();const g=_.url||"",x=(R=g.match(/files\/([^/?]+)/))==null?void 0:R[1];x?cn(x,i.size).then(U=>{C||e.onComplete(U)}).catch(U=>{C||e.onError(U)}):e.onComplete({status:"success",file:{uuid:"",name:i.name,extension:i.name.split(".").pop()||"",type:i.type,size:i.size,url:{public:g,cdn:g},meta:i.meta,tags:i.tags,info:{},created_at:new Date().toISOString(),modified_at:new Date().toISOString()}})},onError(g){C||(m(),dn(g)?e.onError(new Error("Network error during upload — check your connection or firewall settings")):e.onError(g instanceof Error?g:new Error(String(g))))},onShouldRetry(g,x,R){var L;const U=(L=g.originalResponse)==null?void 0:L.getStatus();return U===429?!0:!(U&&U>400&&U<500&&U!==409)}});let P=null,S=null;typeof window<"u"&&(P=()=>{var g;!h&&!C&&(h=!0,_.abort(!1),(g=e.onPause)==null||g.call(e))},S=()=>{var g;h&&!C&&(h=!1,_.start(),(g=e.onResume)==null||g.call(e))},window.addEventListener("offline",P),window.addEventListener("online",S));const m=()=>{P&&window.removeEventListener("offline",P),S&&window.removeEventListener("online",S)},v=()=>{try{_.start()}catch(g){m(),e.onError(g instanceof Error?g:new Error(String(g)))}};return s?_.findPreviousUploads().then(g=>{g.length>0&&!C&&_.resumeFromPreviousUpload(g[0]),C||v()}):v(),{abort(){C=!0,h=!1,m(),_.abort(!0)},pause(){!h&&!C&&(h=!0,_.abort(!1))},resume(){h&&!C&&(h=!1,_.start())},isPaused(){return h}}}function dn(i){var e;if(i instanceof ze){const t=(e=i.originalRequest)==null?void 0:e.getUnderlyingObject();return t&&typeof t.readyState=="number"&&typeof t.status=="number"?t.readyState!==0&&t.readyState!==4||t.status===0:i.originalResponse==null&&i.causingError!=null}return!1}async function cn(i,e){const t=`${sn}/${i}`,r=e>1e8?13e3:6e3,o=3;for(let n=0;n<=o;n++){n>0&&await new Promise(d=>setTimeout(d,r));const s=await fetch(t);if(s.status===404&&n<o)continue;if(!s.ok)throw new Error(`Failed to fetch file record (HTTP ${s.status})`);const l=await s.json();if(l.file)return{status:"success",file:l.file};if(l.status==="success")return l;if(!(n<o))throw new Error(l.msg||"File record not available after upload")}throw new Error("File record not available after upload")}class Wr{constructor(e,t){this.activeUploads=new Map,this.pausedUploads=new Map,this.retryTimers=new Map,this.unsubscribe=null,this.store=e,this.config=t}start(){this.unsubscribe||(this.unsubscribe=this.store.subscribe(()=>this.processQueue()),this.processQueue())}uploadAll(){const{files:e}=this.store.getState();let t=!1;for(const r of e.values())r.status==="idle"?(M(this.store,r.id,{status:"queued"}),t=!0):r.status==="queued"&&(t=!0);t&&(this.store.setState({isUploading:!0}),this.processQueue())}retryFile(e){const t=this.store.getState().files.get(e);!t||t.status!=="error"&&t.status!=="failed"||(M(this.store,e,{status:"queued",error:null,progress:0,bytesUploaded:0,speed:0}),this.processQueue())}retryAll(){const{files:e}=this.store.getState();for(const t of e.values())(t.status==="error"||t.status==="failed")&&M(this.store,t.id,{status:"queued",error:null,progress:0,bytesUploaded:0,speed:0});this.processQueue()}pauseFile(e){const t=this.activeUploads.get(e);t&&"pause"in t&&(t.pause(),this.activeUploads.delete(e),this.pausedUploads.set(e,t),M(this.store,e,{status:"paused"}),this.processQueue())}resumeFile(e){const t=this.pausedUploads.get(e);if(!t)return;const{concurrency:r}=this.store.getState().queueConfig;this.activeUploads.size<r?(this.pausedUploads.delete(e),t.resume(),this.activeUploads.set(e,t),M(this.store,e,{status:"uploading"})):M(this.store,e,{status:"queued"})}cancelFile(e){const t=this.store.getState().files.get(e);!t||!br(t.status)||(this.abortPausedUpload(e),this.abortUpload(e),M(this.store,e,{status:"cancelled"}))}cancelAll(){const{files:e}=this.store.getState();for(const t of e.values())br(t.status)&&(this.abortPausedUpload(t.id),this.abortUpload(t.id),M(this.store,t.id,{status:"cancelled"}));this.store.setState({isUploading:!1})}updateConfig(e){Object.assign(this.config,e)}destroy(){var e;for(const t of this.activeUploads.keys())this.abortUpload(t);for(const t of[...this.pausedUploads.keys()])this.abortPausedUpload(t);for(const t of this.retryTimers.values())clearTimeout(t);this.retryTimers.clear(),(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=null}processQueue(){const e=this.store.getState();if(e.isPaused)return;const{concurrency:t}=e.queueConfig,r=this.activeUploads.size,o=t-r;if(o<=0)return;const s=[...e.files.values()].filter(l=>l.status==="queued").sort((l,d)=>l.retryCount!==d.retryCount?d.retryCount-l.retryCount:l.addedAt-d.addedAt).slice(0,o);for(const l of s){const d=this.pausedUploads.get(l.id);d?(this.pausedUploads.delete(l.id),d.resume(),this.activeUploads.set(l.id,d),M(this.store,l.id,{status:"uploading"})):this.startUpload(l)}}startUpload(e){var C,$;const t=($=(C=this.config).resolveUploadParams)==null?void 0:$.call(C,e),r=!!t&&Object.keys(t).length>0,o=!r&&!e.remoteInfo&&!e.remoteUrl&&an(e,this.config.tusConfig);M(this.store,e.id,{status:"uploading",error:null,isTus:o});let n=0,s=Date.now(),l=0;const d={apiBase:this.config.apiBase,authHeaders:this.config.authHeaders,folder:this.store.getState().targetFolder,extraParams:r?t:void 0,onComplete:z=>this.handleComplete(e.id,z),onError:z=>this.handleError(e.id,z)},u=(z,O)=>{const _=Date.now(),P=(_-s)/1e3;if(P>0){const m=(z-n)/P;l=l===0?m:.3*m+.7*l}n=z,s=_;const S=O>0?Math.min(z/O*100,100):0;M(this.store,e.id,{progress:S,bytesUploaded:z,speed:l}),this.updateTotalProgress()};let h;if(e.remoteInfo)h=mi(e,{...d,onProgress:u});else if(e.remoteUrl)h=ai(e,d);else if(o){const z=ln(e,{...d,onProgress:u,tusConfig:this.config.tusConfig,getAuthHeaders:()=>this.config.authHeaders,onUploadUrlAvailable:O=>{M(this.store,e.id,{tusUploadUrl:O})},onPause:()=>{this.activeUploads.delete(e.id),this.pausedUploads.set(e.id,z),M(this.store,e.id,{status:"paused"}),this.processQueue()},onResume:()=>{this.pausedUploads.delete(e.id),this.activeUploads.set(e.id,z),M(this.store,e.id,{status:"uploading"})}});h=z}else h=si(e,{...d,onProgress:u});this.activeUploads.set(e.id,h)}handleComplete(e,t){var s,l,d;this.activeUploads.delete(e);const r=this.store.getState().files.get(e),o=((l=(s=t.file)==null?void 0:s.url)==null?void 0:l.cdn)??null,n={status:"complete",progress:100,response:t};r&&o&&r.type.startsWith("image/")&&((d=r.previewUrl)!=null&&d.startsWith("blob:")&&URL.revokeObjectURL(r.previewUrl),n.previewUrl=o),M(this.store,e,n),this.updateTotalProgress(),this.checkAllComplete(),this.processQueue()}handleError(e,t){this.activeUploads.delete(e);const r=this.store.getState().files.get(e);if(!r)return;const{retryConfig:o}=this.store.getState().queueConfig,n=r.retryCount+1;if(n<=o.maxRetries){const s=Math.min(o.baseDelay*Math.pow(o.backoffFactor,r.retryCount),o.maxDelay);M(this.store,e,{status:"retrying",error:t.message,retryCount:n});const l=setTimeout(()=>{this.retryTimers.delete(e),M(this.store,e,{status:"queued"}),this.processQueue()},s);this.retryTimers.set(e,l)}else M(this.store,e,{status:"failed",error:t.message}),this.checkAllComplete(),this.processQueue()}abortPausedUpload(e){const t=this.pausedUploads.get(e);t&&(t.abort(),this.pausedUploads.delete(e))}abortUpload(e){var r;(r=this.activeUploads.get(e))==null||r.abort(),this.activeUploads.delete(e);const t=this.retryTimers.get(e);t&&(clearTimeout(t),this.retryTimers.delete(e))}updateTotalProgress(){const{files:e}=this.store.getState();let t=0,r=0,o=0;for(const n of e.values())(n.status==="queued"||n.status==="uploading"||n.status==="paused"||n.status==="retrying"||n.status==="complete"||n.status==="failed")&&(t+=n.size,r+=n.status==="complete"?n.size:n.bytesUploaded),n.status==="uploading"&&(o+=n.speed);this.store.setState({totalBytes:t,totalBytesUploaded:r,totalSpeed:o,totalProgress:t>0?Math.min(r/t*100,100):0})}checkAllComplete(){const{files:e}=this.store.getState();![...e.values()].some(r=>r.status==="queued"||r.status==="uploading"||r.status==="retrying"||r.status==="paused")&&this.store.getState().isUploading&&this.store.setState({isUploading:!1})}}function br(i){return i==="queued"||i==="uploading"||i==="retrying"||i==="paused"}function nt(i){return`https://api.filerobot.com/${i}`}async function Xr(i,e){const t=`${nt(i)}/key/${encodeURIComponent(e)}`,r=new AbortController,o=setTimeout(()=>r.abort(),3e4);try{const n=await fetch(t,{signal:r.signal});if(clearTimeout(o),!n.ok)throw new Error(`SASS key exchange failed (HTTP ${n.status})`);const s=await n.json();if(s.status==="error")throw new Error(`SASS key exchange failed: ${s.msg||"Unknown error"}`);return s.key}catch(n){throw clearTimeout(o),n instanceof DOMException&&n.name==="AbortError"?new Error("SASS key exchange timed out"):n}}function et(i,e){const t={};switch(i.mode){case"security-template":if(!e)throw new Error("[sfx-uploader] Cannot build auth headers for security-template mode: SASS key exchange has not been performed. Call resolveAuth() first or use sass-key mode with a pre-resolved key.");t["X-Filerobot-Key"]=e;break;case"sass-key":t["X-Filerobot-Key"]=i.sassKey;break}return i.airboxPuid&&(t["X-Filerobot-Airbox-Puid"]=i.airboxPuid),t}async function Kr(i){const e=nt(i.container);if(i.mode==="security-template"){const t=await Xr(i.container,i.securityTemplateId);return{apiBase:e,headers:et(i,t),sassKey:t}}return{apiBase:e,headers:et(i)}}const tt="sfx-uploader:last-upload:",Zr=1;function pn(i){var n,s,l;const{file:e,previewUrl:t,...r}=i,o=i.status==="complete"&&((l=(s=(n=i.response)==null?void 0:n.file)==null?void 0:s.url)!=null&&l.cdn)?i.response.file.url.cdn:null;return{...r,previewUrl:o}}function un(i){try{const e=sessionStorage.getItem(tt+i);if(!e)return null;const t=JSON.parse(e);return(t==null?void 0:t.__schemaVersion)!==Zr?null:t}catch{return null}}function fn(i,e){try{sessionStorage.setItem(tt+i,JSON.stringify(e))}catch{}}const Oe={save(i,e){if(e.length===0){this.clear(i);return}const t={__schemaVersion:Zr,savedAt:Date.now(),files:e.map(pn)};fn(i,t)},load(i){const e=un(i);return e?e.files.map(t=>({...t,file:null,previewUrl:t.previewUrl??null})):null},exists(i){try{return sessionStorage.getItem(tt+i)!=null}catch{return!1}},clear(i){try{sessionStorage.removeItem(tt+i)}catch{}}},j={FILE_ADDED:"sfx-file-added",FILE_REMOVED:"sfx-file-removed",FILE_REJECTED:"sfx-file-rejected",UPLOAD_STARTED:"sfx-upload-started",UPLOAD_PROGRESS:"sfx-upload-progress",UPLOAD_COMPLETE:"sfx-upload-complete",UPLOAD_ERROR:"sfx-upload-error",UPLOAD_RETRY:"sfx-upload-retry",UPLOAD_PAUSED:"sfx-upload-paused",UPLOAD_RESUMED:"sfx-upload-resumed",ALL_COMPLETE:"sfx-all-complete",TOTAL_PROGRESS:"sfx-total-progress",BEFORE_UPLOAD:"sfx-before-upload",OPEN:"sfx-open",CLOSE:"sfx-close",CANCEL:"sfx-cancel",COMPLETE_ACTION:"sfx-complete-action",FILE_PREVIEW:"sfx-file-preview",FILL_METADATA:"sfx-fill-metadata",FILE_LOCATE:"sfx-file-locate",FILE_COPY_CDN:"sfx-file-copy-cdn"};let hn=0;function be(){return`file-${Date.now()}-${++hn}`}function se(i){if(i<=0)return"0 B";const e=["B","KB","MB","GB"],t=Math.min(Math.floor(Math.log(i)/Math.log(1024)),e.length-1),r=i/Math.pow(1024,t);return`${t===0?r:r.toFixed(1)} ${e[t]}`}function pt(i){if(!isFinite(i)||i<=0)return"0s";const e=Math.round(i);if(e<60)return`${e}s`;const t=Math.floor(e/60),r=e%60;return r>0?`${t}m ${r}s`:`${t}m`}function Jr(i){var t;const e=((t=i.name.split(".").pop())==null?void 0:t.toLowerCase())??"";return i.type.startsWith("image/")?"image":i.type.startsWith("video/")||["mp4","mov","avi","webm","mkv","flv","wmv"].includes(e)?"vid":i.type.startsWith("audio/")||["mp3","wav","ogg","flac","aac","m4a","wma"].includes(e)?"audio":i.type==="application/pdf"||e==="pdf"?"pdf":["xls","xlsx","csv","tsv","ods"].includes(e)?"sheet":["doc","docx","txt","rtf","odt","pages"].includes(e)?"doc":["ppt","pptx","key","odp"].includes(e)?"slide":["zip","rar","7z","tar","gz","bz2","xz","zst"].includes(e)?"zip":["js","ts","jsx","tsx","py","rb","go","rs","java","c","cpp","h","cs","php","swift","kt","sh","bash"].includes(e)?"code":["html","css","scss","xml","svg","json","yaml","yml","toml","md","mdx","ini","env","log"].includes(e)?"markup":["ttf","otf","woff","woff2","eot"].includes(e)?"font":["ai","psd","sketch","fig","xd","indd","eps"].includes(e)?"design":["exe","dmg","app","msi","deb","rpm","apk","ipa"].includes(e)?"binary":["sql","db","sqlite","mdb"].includes(e)?"data":"gen"}function gn(i){const e=i.lastIndexOf(".");return e>=0?i.slice(e+1).toUpperCase():""}const mn="https://scaleflex.cloudimg.io/v7/assets/file-types/v3/",Qr={_default:"9a518a",png:"96cd9a",jpg:"06e819",jpg2:"f0eb7f",jpeg:"6a65e9",gif:"c3c2c3",bmp:"d2243a",webp:"fedd74",svg:"a15e46",tiff:"1f30c3",tif:"b383c9",heic:"84adfe",avif:"536b30",ico:"79063d",psd:"be6140",psb:"678646",ai:"84b254",dwg:"971fb3",mp4:"42f175",webm:"26a84a",avi:"d22ba8",mpeg:"ba93bb",ogv:"74d453","3gp":"f0d388","3g2":"04c652",swf:"3955e2",fla:"daf585",m3u8:"7d5e62",mp3:"66bbef",wav:"d7a7d5",aac:"07f3f9",oga:"a5c622",opus:"9548b1",weba:"4dcf70",mid:"3f0e29",midi:"9fedec",cda:"85b83b",pdf:"18c5f7",doc:"d1b47c",docx:"1eb6b0",txt:"307979",rtf:"978c5f",xls:"13b5f7",xlsx:"79d64a",ppt:"4ee29b",pptx:"8b1568",csv:"4add78",odt:"940781",ods:"9fbe9a",odp:"bf892d",dbf:"457bd4",vsd:"8a9ccb",abw:"313dc7",epub:"15263d",azw:"a018b1",ics:"909f63",ogx:"f694d2",zip:"84f98b",rar:"1d6423","7z":"e007e5",tar:"603aed",gz:"de13f7",bz:"0374ff",bz2:"e14294",arc:"942fad",jar:"149796",mpkg:"dea655",ttf:"d2e2c1",otf:"c904fd",woff:"4b8177",woff2:"b532d3",eot:"a54980",js:"524691",mjs:"d57921",ts:"9af3ae",css:"287863",html:"fa7a87",htm:"21323d",xhtml:"e6d6a9",xul:"6c9c71",json:"104c9e",jsonld:"f30c0f",xml:"7f7194",php:"503e36",sh:"3b820e",csh:"08c0cc",exe:"ccca53",iso:"064b8f",bin:"1e9618"};function _t(i){const e=i==="_default"?"GENERIC":i.toUpperCase();return`${mn}${e}.svg?vh=${Qr[i]}`}function Rt(i){const e=(i==null?void 0:i.toLowerCase().replaceAll(".",""))||"";return e in Qr?_t(e):_t("_default")}function Tt(){return _t("_default")}const vn={jpg:"image/jpeg",jpeg:"image/jpeg",png:"image/png",gif:"image/gif",webp:"image/webp",svg:"image/svg+xml",bmp:"image/bmp",ico:"image/x-icon",mp4:"video/mp4",mov:"video/quicktime",avi:"video/x-msvideo",webm:"video/webm",pdf:"application/pdf",zip:"application/zip",doc:"application/msword",docx:"application/vnd.openxmlformats-officedocument.wordprocessingml.document"};function xn(i){var t;const e=((t=i.split(".").pop())==null?void 0:t.toLowerCase())??"";return vn[e]||""}function bn(i){return new Promise(e=>{const t=document.createElement("video");t.preload="metadata",t.muted=!0,t.playsInline=!0;const r=URL.createObjectURL(i);let o=!1;const n=()=>{o||(o=!0,e(null)),t.removeAttribute("src"),t.load(),URL.revokeObjectURL(r)};t.addEventListener("seeked",()=>{try{const s=document.createElement("canvas");s.width=t.videoWidth||320,s.height=t.videoHeight||240;const l=s.getContext("2d");if(l){l.drawImage(t,0,0,s.width,s.height),s.toBlob(d=>{o||(o=!0,e(d?URL.createObjectURL(d):null),t.removeAttribute("src"),t.load(),URL.revokeObjectURL(r))},"image/jpeg",.7);return}}catch{}n()},{once:!0}),t.addEventListener("error",()=>n(),{once:!0}),setTimeout(()=>n(),5e3),t.src=r,t.addEventListener("loadeddata",()=>{t.currentTime=.1},{once:!0})})}function kt(i,e,t){var r,o;if(e.maxFileSize!=null&&i.size>0&&i.size>e.maxFileSize)return`File exceeds ${(e.maxFileSize/1048576).toFixed(1)} MB limit`;if(e.maxTotalFilesSize!=null&&i.size>0){let n=i.size;for(const s of t.values())s.status!=="rejected"&&s.status!=="cancelled"&&(n+=s.size);if(n>e.maxTotalFilesSize)return"Total file size limit exceeded"}if(e.maxNumberOfFiles!=null){let n=0;for(const s of t.values())s.status!=="rejected"&&s.status!=="cancelled"&&n++;if(n>=e.maxNumberOfFiles)return`Maximum ${e.maxNumberOfFiles} files allowed`}if(e.allowedFileTypes!=null){const n=e.allowedFileTypes,s="."+(((r=i.name.split(".").pop())==null?void 0:r.toLowerCase())??"");if(!n.some(d=>d.startsWith(".")?s===d.toLowerCase():d.endsWith("/*")?i.type.startsWith(d.slice(0,-1)):i.type===d))return"File type not allowed"}if(e.blockedFileTypes!=null){const n=e.blockedFileTypes,s="."+(((o=i.name.split(".").pop())==null?void 0:o.toLowerCase())??"");if(n.some(d=>d.startsWith(".")?s===d.toLowerCase():d.endsWith("/*")?i.type.startsWith(d.slice(0,-1)):i.type===d))return"File type is blocked"}return null}function yn(i,e,t){return kt(i,e,t)}function yr(i){return i.allowedFileTypes?i.allowedFileTypes.join(","):""}const wr={"google-drive":{id:"google-drive",label:"Google Drive",fillIcon:!0,icon:"",brandStyle:{background:"transparent"},brandHtml:'<svg width="16" height="16" viewBox="0 0 87.3 78"><path d="M6.6 66.85l3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3L27.5 53H0c0 1.55.4 3.1 1.2 4.5z" fill="#0066da"/><path d="M43.65 25L29.9 1.2C28.55 2 27.4 3.1 26.6 4.5L1.2 48.5C.4 49.9 0 51.45 0 53h27.5z" fill="#00ac47"/><path d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5H59.8l5.65 10.85z" fill="#ea4335"/><path d="M43.65 25L57.4 1.2C56.05.4 54.5 0 52.9 0H34.4c-1.6 0-3.15.45-4.5 1.2z" fill="#00832d"/><path d="M59.8 53H27.5L13.75 76.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z" fill="#2684fc"/><path d="M73.4 26.5l-12.7-22c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25 59.8 53h27.45c0-1.55-.4-3.1-1.2-4.5z" fill="#ffba00"/></svg>'},dropbox:{id:"dropbox",label:"Dropbox",fillIcon:!0,icon:"",brandStyle:{background:"#0061ff"},brandHtml:'<svg width="11" height="11" viewBox="0 0 528 512" fill="white"><path d="M264.4 116.3l-132 84.3 132 84.3-132 84.3L0 284.1l132.3-84.3L0 116.3 132.3 32l132.1 84.3zm-132 284.5l132-84.3 132 84.3-132 84.4-132-84.4zm132-116.6l132.3-84.3-132.3-83.9 131.6-84.3L528 116.3l-132.3 84.1L528 284.7l-132.4 83.9-131.2-84.4z"/></svg>'},onedrive:{id:"onedrive",label:"OneDrive",fillIcon:!0,icon:"",brandStyle:{background:"#0078d4"},brandHtml:'<svg width="11" height="11" viewBox="0 0 24 24" fill="white"><path d="M10.5 13.5C10.5 11.57 12.07 10 14 10h6.5c.17 0 .34.01.5.02A6 6 0 009.01 11.6 4 4 0 0010.5 13.5zM12 14.5a5 5 0 00-5-5 5 5 0 00-5 5 3 3 0 003 3h9.5A3.5 3.5 0 0018 14c0-.18-.01-.35-.03-.52A5.48 5.48 0 0112 14.5z"/></svg>'},box:{id:"box",label:"Box",fillIcon:!0,icon:"",brandStyle:{background:"#0e50a0","font-size":"9px","font-weight":"800",color:"#fff"},brandHtml:"box"},instagram:{id:"instagram",label:"Instagram",fillIcon:!0,icon:"",brandStyle:{background:"linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)"},brandHtml:'<svg width="12" height="12" viewBox="0 0 24 24" fill="white"><path d="M12 2.16c2.94 0 3.29.01 4.45.06 1.07.05 1.8.22 2.43.46.66.25 1.21.6 1.77 1.16.55.55.9 1.1 1.16 1.77.25.64.41 1.37.46 2.43.05 1.16.06 1.51.06 4.45s-.01 3.29-.06 4.45c-.05 1.07-.22 1.8-.46 2.43a4.9 4.9 0 01-1.16 1.77c-.55.55-1.1.9-1.77 1.16-.64.25-1.37.41-2.43.46-1.16.05-1.51.06-4.45.06s-3.29-.01-4.45-.06c-1.07-.05-1.8-.22-2.43-.46a4.9 4.9 0 01-1.77-1.16 4.9 4.9 0 01-1.16-1.77c-.25-.64-.41-1.37-.46-2.43C2.17 15.29 2.16 14.94 2.16 12s.01-3.29.06-4.45c.05-1.07.22-1.8.46-2.43a4.9 4.9 0 011.16-1.77A4.9 4.9 0 015.61 2.2c.64-.25 1.37-.41 2.43-.46C9.21 2.17 9.56 2.16 12 2.16zM12 16a4 4 0 110-8 4 4 0 010 8zm6.4-9.85a1.44 1.44 0 100 2.88 1.44 1.44 0 000-2.88z"/></svg>'},facebook:{id:"facebook",label:"Facebook",fillIcon:!0,icon:"",brandStyle:{background:"#1877f2"},brandHtml:'<svg width="12" height="12" viewBox="0 0 24 24" fill="white"><path d="M24 12.07C24 5.41 18.63 0 12 0S0 5.41 0 12.07c0 6.02 4.39 11.02 10.12 11.93v-8.44H7.08v-3.49h3.04V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.95.93-1.95 1.89v2.26h3.33l-.53 3.49h-2.8v8.44C19.61 23.09 24 18.09 24 12.07z"/></svg>'},unsplash:{id:"unsplash",label:"Unsplash",fillIcon:!0,icon:"",brandStyle:{background:"#111"},brandHtml:'<svg width="12" height="12" viewBox="0 0 24 24" fill="white"><path d="M8.5 11.5v5h7v-5h5.5V21h-18v-9.5h5.5zm7-8v5h-7v-5h7z"/></svg>'}};function ei(i){return i.filter(e=>e in wr).map(e=>wr[e])}function ae(i){return i.brandStyle?a.html`<span
    class=${ii.classMap({"brand-ico":!0,"brand-ico--transparent":i.brandStyle.background==="transparent"})}
    ${N(i.brandStyle)}
  >${Gt.unsafeHTML(i.brandHtml)}</span>`:Gt.unsafeHTML(i.brandHtml)}var wn=Object.defineProperty,_n=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&wn(e,t,o),o};const kn='<rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>',Cn='<path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/>',Sn='<path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/>',$n='<rect x="2" y="3" width="20" height="14" rx="2"/><circle cx="12" cy="10" r="1"/><path d="M7 21l5-5 5 5"/>',ke=[{id:"device",label:"My Device",icon:kn,iconColor:"#2563eb"},{id:"url",label:"URL link",icon:Cn,iconColor:"#16a34a"},{id:"camera",label:"Camera",icon:Sn,iconColor:"#7c3aed"},{id:"screen-cast",label:"Screen capture",icon:$n,iconColor:"#ea580c"}],jt=class jt extends a.LitElement{constructor(){super(...arguments),this.sources=ke}_handleClick(e){this.dispatchEvent(new CustomEvent("source-click",{detail:{source:e.id},bubbles:!0,composed:!0}))}render(){return a.html`
      ${this.sources.map(e=>a.html`
          <button @click=${()=>this._handleClick(e)}>
            ${e.brandHtml?ae(e):a.svg`<svg viewBox="0 0 24 24" class=${e.fillIcon?"fill-icon":""}>${ne.unsafeSVG(e.icon)}</svg>`}
            ${e.label}
          </button>
        `)}
    `}};jt.styles=a.css`
  ...
  `;let rt=jt;_n([w.property({type:Array})],rt.prototype,"sources");function ti(i){let e=i;for(;e;){if(e instanceof ShadowRoot){e=e.host;continue}if(e instanceof HTMLDialogElement&&e.open)return e;if(e instanceof Element&&e.shadowRoot){const t=e.shadowRoot.querySelector("dialog[open]");if(t instanceof HTMLDialogElement)return t}e=e.parentNode}return document.body}var Un=Object.defineProperty,J=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&Un(e,t,o),o};const _r=3,Ct=new CSSStyleSheet;Ct.replaceSync(`
...
`);var le;const K=(le=class extends a.LitElement{constructor(){super(...arguments),this.compact=!1,this.externalDragOver=!1,this.accept="",this.multi=!0,this.sources=[],this.sourcesLayout="pills",this.mode="modal",this._resizeObserver=null,this._dragOver=!1,this._moreOpen=!1,this._visiblePills=_r,this._dragCounter=0,this._onDragEnter=e=>{e.preventDefault(),this._dragCounter++,this._dragCounter===1&&(this._dragOver=!0)},this._onDragOver=e=>{e.preventDefault()},this._onDragLeave=e=>{e.preventDefault(),this._dragCounter--,this._dragCounter<=0&&(this._dragCounter=0,this._dragOver=!1)},this._onDrop=e=>{var r;e.preventDefault(),e.stopPropagation(),this._dragCounter=0,this._dragOver=!1;const t=Array.from(((r=e.dataTransfer)==null?void 0:r.files)??[]);t.length>0&&this._emitFiles(t)},this._onClick=e=>{const t=this.shadowRoot.querySelector(".drop-zone");if(t&&this._rippleEl){const r=t.getBoundingClientRect();this._rippleEl.style.left=`${e.clientX-r.left}px`,this._rippleEl.style.top=`${e.clientY-r.top}px`,this._rippleEl.classList.remove("go"),this._rippleEl.offsetWidth,this._rippleEl.classList.add("go")}this.browse()},this._onKeyDown=e=>{(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),this.browse())},this._onFileChange=e=>{const t=e.target,r=Array.from(t.files??[]);r.length>0&&this._emitFiles(r),t.value=""},this._onPaste=e=>{var o;if(!this.isConnected||this.offsetWidth===0)return;const t=(o=e.clipboardData)==null?void 0:o.items;if(!t)return;const r=[];for(const n of t)if(n.kind==="file"){const s=n.getAsFile();s&&r.push(s)}r.length>0&&(e.preventDefault(),this._emitFiles(r))},this._portalContainer=null,this._onDocClick=e=>{var t;this._moreOpen&&((t=this._portalContainer)!=null&&t.contains(e.target)||(this._moreOpen=!1,this._updateDropdownPortal()))},this._onDocKeyDown=e=>{e.key==="Escape"&&this._moreOpen&&(this._moreOpen=!1,this._updateDropdownPortal())},this._resizeTimer=null,this._onScrollOrResize=()=>{this._moreOpen&&this._positionDropdown(),this._resizeTimer&&clearTimeout(this._resizeTimer),this._resizeTimer=setTimeout(()=>this._updateVisiblePills(),100)}}browse(){var e;(e=this.fileInput)==null||e.click()}_onSourceIconClick(e){this.dispatchEvent(new CustomEvent("source-click",{detail:{source:e.id},bubbles:!0,composed:!0}))}_emitFiles(e){this.dispatchEvent(new CustomEvent("files-selected",{detail:{files:e},bubbles:!0,composed:!0}))}_toggleMore(e){e.stopPropagation(),this._moreOpen=!this._moreOpen,this._updateDropdownPortal()}_updateDropdownPortal(){if(this._moreOpen){const e=this.sources.slice(this._visiblePills);this._portalContainer||(this._portalContainer=document.createElement("div"),this._portalContainer.setAttribute("data-sfx-more-dropdown",""),ti(this).appendChild(this._portalContainer),this._injectDropdownStyles()),a.render(a.html`<div class="sfx-more-dropdown open">
    ...
    `}_renderCard(e){return a.html`
      <button
        ...
        @click=${t=>{t.stopPropagation(),this._onSourceIconClick(e)}}
      >
        ${e.brandHtml?a.html`<span class="card-ico">${ae(e)}</span>`:a.html`<span
              class="card-ico"
              ${N(e.iconColor?{color:e.iconColor}:null)}
            >
              ${a.svg`<svg viewBox="0 0 24 24" class=${e.fillIcon?"fill-icon":""}>${ne.unsafeSVG(e.icon)}</svg>`}
        ...
        ${this.compact&&this.sources.length>0?a.html`
              <div class="sources-row">
                ${this.sources.map(o=>a.html`
                    <button
                      class="src-ico"
                      ${N(o.iconColor&&!o.brandHtml?{color:o.iconColor}:null)}
                      data-tip=${o.label}
                      aria-label=${o.label}
                      @click=${n=>{n.stopPropagation(),this._onSourceIconClick(o)}}
                    >
                      ${o.brandHtml?ae(o):a.svg`<svg viewBox="0 0 24 24" class=${o.fillIcon?"fill-icon":""}>${ne.unsafeSVG(o.icon)}</svg>`}
                    </button>
                  `)}
              </div>
            `:a.nothing}
  ...
  `,le._WIDE_THRESHOLD_PX=1200,le);J([w.property({type:Boolean,reflect:!0})],K.prototype,"compact");J([w.property({type:Boolean,attribute:"external-drag-over"})],K.prototype,"externalDragOver");J([w.property({type:String})],K.prototype,"accept");J([w.property({type:Boolean})],K.prototype,"multi");J([w.property({type:Array})],K.prototype,"sources");J([w.property({type:String,attribute:"sources-layout"})],K.prototype,"sourcesLayout");J([w.property({type:String,reflect:!0})],K.prototype,"mode");J([w.state()],K.prototype,"_dragOver");J([w.state()],K.prototype,"_moreOpen");J([w.state()],K.prototype,"_visiblePills");J([w.query(".ripple")],K.prototype,"_rippleEl");J([w.query('input[type="file"]')],K.prototype,"fileInput");let En=K;const Ft=class Ft extends a.LitElement{render(){return a.html`
      <div class="line"></div>
  ...
  `;let St=Ft;var Pn=Object.defineProperty,ee=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&Pn(e,t,o),o};const $t=new CSSStyleSheet;$t.replaceSync(`
...
`);const At=class At extends a.LitElement{constructor(){super(...arguments),this.files=[],this.showDropTile=!1,this.sources=[],this.accept="",this.multi=!0,this.mode="upload",this.showLocateButton=!1,this.showCopyCdnButton=!1,this._moreOpen=!1,this._dropTileMaxVisible=3,this._portalContainer=null,this._outsideClickHandler=e=>{var o;if((o=this._portalContainer)!=null&&o.contains(e.target))return;const t=this.renderRoot.querySelector(".drop-tile-more-wrap"),r=e.composedPath();t&&r.includes(t)||(this._moreOpen=!1,this._closePortal(),document.removeEventListener("click",this._outsideClickHandler,!0))},this._onScrollOrResize=()=>{this._moreOpen&&this._positionPortal()},this._onKeyDown=e=>{e.key==="Escape"&&this._moreOpen&&(this._moreOpen=!1,this._closePortal(),this._removeGlobalListeners())},this._updateDropTileMaxVisible=()=>{const e=window.innerWidth<=768?1:3;e!==this._dropTileMaxVisible&&(this._dropTileMaxVisible=e)}}_onDropTileClick(){const e=this.renderRoot.querySelector('input[type="file"]');e==null||e.click()}_onFileInput(e){const t=e.target,r=Array.from(t.files??[]);r.length>0&&this.dispatchEvent(new CustomEvent("files-selected",{detail:{files:r},bubbles:!0,composed:!0})),t.value=""}_onSourceClick(e,t){if(e.stopPropagation(),t.id==="device"){const r=this.renderRoot.querySelector('input[type="file"]');r==null||r.click();return}this.dispatchEvent(new CustomEvent("source-click",{detail:{source:t},bubbles:!0,composed:!0}))}_addGlobalListeners(){requestAnimationFrame(()=>document.addEventListener("click",this._outsideClickHandler,!0)),document.addEventListener("keydown",this._onKeyDown),window.addEventListener("scroll",this._onScrollOrResize,!0),window.addEventListener("resize",this._onScrollOrResize)}_removeGlobalListeners(){document.removeEventListener("click",this._outsideClickHandler,!0),document.removeEventListener("keydown",this._onKeyDown),window.removeEventListener("scroll",this._onScrollOrResize,!0),window.removeEventListener("resize",this._onScrollOrResize)}_toggleMore(e){e.stopPropagation(),this._moreOpen=!this._moreOpen,this._moreOpen?(this._openPortal(),this._addGlobalListeners()):(this._closePortal(),this._removeGlobalListeners())}_openPortal(){const e=this.sources.slice(this._dropTileMaxVisible);this._portalContainer||(this._portalContainer=document.createElement("div"),this._portalContainer.setAttribute("data-sfx-tile-dropdown",""),ti(this).appendChild(this._portalContainer),this._injectTileDropdownStyles()),a.render(a.html`<div class="sfx-tile-dropdown">
      ...
      <div class="drop-tile" @click=${this._onDropTileClick}>
        ...
        <input type="file" ?multiple=${this.multi} accept=${this.accept||a.nothing} @change=${this._onFileInput} />
      </div>
    `}render(){return a.html`
      <div class="grid">
        ${this.showDropTile&&this.mode!=="review"?this._renderDropTile():a.nothing}
        ${this.files.map((e,t)=>a.html`<sfx-file-item .file=${e} .mode=${this.mode} .getLocateUrl=${this.getLocateUrl} .showLocateButton=${this.showLocateButton} .showCopyCdnButton=${this.showCopyCdnButton} ${N({"--tile-index":String(t)})}></sfx-file-item>`)}
      </div>
    `}};At.styles=a.css`
    :host {
      display: block;
  ...
  `;let W=At;ee([w.property({attribute:!1})],W.prototype,"files");ee([w.property({type:Boolean})],W.prototype,"showDropTile");ee([w.property({attribute:!1})],W.prototype,"sources");ee([w.property({type:String})],W.prototype,"accept");ee([w.property({type:Boolean})],W.prototype,"multi");ee([w.property({type:String})],W.prototype,"mode");ee([w.property({attribute:!1})],W.prototype,"getLocateUrl");ee([w.property({type:Boolean})],W.prototype,"showLocateButton");ee([w.property({type:Boolean})],W.prototype,"showCopyCdnButton");ee([w.state()],W.prototype,"_moreOpen");ee([w.state()],W.prototype,"_dropTileMaxVisible");var On=Object.defineProperty,ge=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&On(e,t,o),o};const Bt=class Bt extends a.LitElement{constructor(){super(...arguments),this.mode="upload",this.showLocateButton=!1,this.showCopyCdnButton=!1,this._dims="",this._copied=!1,this._copiedTimer=null}updated(e){var t,r,o,n,s;if(e.has("file")){if(this._dims="",(r=(t=this.file)==null?void 0:t.previewUrl)!=null&&r.startsWith("blob:")){const l=this.file.previewUrl,d=new Image;d.onload=()=>{var u;((u=this.file)==null?void 0:u.previewUrl)===l&&(this._dims=`${d.naturalWidth}×${d.naturalHeight}`)},d.src=l}else if((s=(n=(o=this.file)==null?void 0:o.response)==null?void 0:n.file)!=null&&s.info){const l=this.file.response.file.info;l.img_w&&l.img_h&&(this._dims=`${l.img_w}×${l.img_h}`)}}}disconnectedCallback(){super.disconnectedCallback(),this._copiedTimer!=null&&(clearTimeout(this._copiedTimer),this._copiedTimer=null)}_emit(e,t){this.dispatchEvent(new CustomEvent(e,{detail:{fileId:this.file.id,...t},bubbles:!0,composed:!0}))}_remove(){this._emit("file-remove")}_retry(){this._emit("file-retry")}_pause(){this._emit("file-pause")}_resume(){this._emit("file-resume")}_rename(e){const t=e.target.value.trim();t&&this._emit("file-rename",{name:t})}_preview(e){e.stopPropagation(),this._emit("file-preview")}_locate(e){e.stopPropagation(),this.file&&this._emit("file-locate",{file:this.file})}async _copyCdn(e){var r,o,n,s;e.stopPropagation();const t=(s=(n=(o=(r=this.file)==null?void 0:r.response)==null?void 0:o.file)==null?void 0:n.url)==null?void 0:s.cdn;if(t){try{await navigator.clipboard.writeText(t)}catch{return}this._emit("file-copy-cdn",{file:this.file,cdnUrl:t}),this._copied=!0,this._copiedTimer&&clearTimeout(this._copiedTimer),this._copiedTimer=window.setTimeout(()=>{this._copied=!1,this._copiedTimer=null},1400)}}render(){var C,$;const e=this.file;if(!e)return a.nothing;const t=Jr(e),r=e.status==="complete",o=e.status==="uploading",n=e.status==="paused",s=e.status==="error"||e.status==="failed",l=e.status==="rejected",d=this.mode==="review",u=gn(e.name),h=["tile",r?"done":"",o?"uploading":"",n?"paused":"",l?"rejected":"",d?"review":""].filter(Boolean).join(" ");return a.html`
      <div class=${h} tabindex="0">
        ...
        <div class="preview">
          ${e.previewUrl?a.html`<img class="preview-img" src=${e.previewUrl} alt="" />`:a.html`
                ...
                <div class="type-icon">
                  <img
                    ...
                    src=${Rt(u)}
                    alt="${u?`${u} file`:"File"}"
                    @error=${z=>{const O=z.target,_=Tt();!O.dataset.fallback&&O.src!==_&&(O.dataset.fallback="1",O.src=_)}}
                  />
                </div>
              `}
          ...
          ${d&&r&&(($=(C=e.response)==null?void 0:C.file)!=null&&$.url)&&(this.showLocateButton||this.showCopyCdnButton)?a.html`
          ...
          ${!d&&(e.status==="uploading"||e.status==="paused")?a.html`
                <div class="progress">
                  <div class="progress-fill" ${N({transform:`scaleX(${Math.min(e.progress,100)/100})`})}></div>
                </div>
              `:a.nothing}
        ...
        <div class="info">
          <input class="name-input" type="text" .value=${e.name} title=${e.name}
            ...
            ?readonly=${d}
            @change=${d?a.nothing:this._rename} @click=${z=>z.stopPropagation()} />
          <div class="meta">${u||""}${e.size?` · ${se(e.size)}`:""}${this._dims?` · ${this._dims}`:""}</div>
        </div>
      </div>
    `}_formatDuration(e){const t=Math.floor(e/60),r=Math.floor(e%60);return`${t}:${r.toString().padStart(2,"0")}`}};Bt.styles=a.css`
    :host {
      display: block;
    }
  ...
  `;let te=Bt;ge([w.property({attribute:!1})],te.prototype,"file");ge([w.property({type:String})],te.prototype,"mode");ge([w.property({attribute:!1})],te.prototype,"getLocateUrl");ge([w.property({type:Boolean})],te.prototype,"showLocateButton");ge([w.property({type:Boolean})],te.prototype,"showCopyCdnButton");ge([w.state()],te.prototype,"_dims");ge([w.state()],te.prototype,"_copied");const qe=a.css`
  .btn,
  .btn-ghost,
  .btn-primary,
  .btn-sec,
...
`;var zn=Object.defineProperty,Pe=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&zn(e,t,o),o};const kr=7,Rn=4,It=class It extends a.LitElement{constructor(){super(...arguments),this.fileCount=0,this.totalSize=0,this.thumbnails=[],this.primaryLabel="Done",this.failedFiles=[],this._maxThumbs=kr,this._updateMaxThumbs=()=>{const e=window.innerWidth<=768?Rn:kr;e!==this._maxThumbs&&(this._maxThumbs=e)}}connectedCallback(){super.connectedCallback(),this._updateMaxThumbs(),window.addEventListener("resize",this._updateMaxThumbs)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("resize",this._updateMaxThumbs)}_uploadMore(){this.dispatchEvent(new CustomEvent("upload-more",{bubbles:!0,composed:!0}))}_reviewFiles(){this.dispatchEvent(new CustomEvent("review-files",{bubbles:!0,composed:!0}))}_primaryAction(){this.dispatchEvent(new CustomEvent("primary-action",{bubbles:!0,composed:!0}))}_retryFile(e){this.dispatchEvent(new CustomEvent("file-retry",{bubbles:!0,composed:!0,detail:{fileId:e}}))}_retryAll(){this.dispatchEvent(new CustomEvent("retry-all",{bubbles:!0,composed:!0}))}_close(){this.dispatchEvent(new CustomEvent("close-uploader",{bubbles:!0,composed:!0}))}render(){const e=this.thumbnails.slice(0,this._maxThumbs),t=this.thumbnails.length-this._maxThumbs,r=this.fileCount>0,o=this.failedFiles.length>0,n=o&&!r;return a.html`
  ...
  `;let de=Mt;Ye([w.property({attribute:!1})],de.prototype,"files");Ye([w.property({attribute:!1})],de.prototype,"getLocateUrl");Ye([w.property({type:Boolean})],de.prototype,"showLocateButton");Ye([w.property({type:Boolean})],de.prototype,"showCopyCdnButton");Ye([w.state()],de.prototype,"_filter");customElements.define("sfx-last-upload-review",de);var Dn=Object.defineProperty,oe=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&Dn(e,t,o),o};const Ht=class Ht extends a.LitElement{constructor(){super(...arguments),this.uploadState="idle",this.fileCount=0,this.totalSize=0,this.failedCount=0,this.showFillMetadata=!1,this.uploadDisabled=!1,this.uploadDisabledReason="",this.completedCount=0,this.uploadProgress=0}_clear(){this.dispatchEvent(new CustomEvent("clear-all",{bubbles:!0,composed:!0}))}_addMore(){this.dispatchEvent(new CustomEvent("add-more",{bubbles:!0,composed:!0}))}_fillMetadata(){this.dispatchEvent(new CustomEvent("fill-metadata",{bubbles:!0,composed:!0}))}_upload(){this.dispatchEvent(new CustomEvent("upload-start",{bubbles:!0,composed:!0}))}_retryAll(){this.dispatchEvent(new CustomEvent("retry-all",{bubbles:!0,composed:!0}))}render(){const e=this.uploadState==="uploading";return a.html`
    ...
    `];let Z=Ht;oe([w.property({type:String})],Z.prototype,"uploadState");oe([w.property({type:Number})],Z.prototype,"fileCount");oe([w.property({type:Number})],Z.prototype,"totalSize");oe([w.property({type:Number})],Z.prototype,"failedCount");oe([w.property({type:Boolean})],Z.prototype,"showFillMetadata");oe([w.property({type:Boolean})],Z.prototype,"uploadDisabled");oe([w.property({type:String})],Z.prototype,"uploadDisabledReason");oe([w.property({type:Number})],Z.prototype,"completedCount");oe([w.property({type:Number})],Z.prototype,"uploadProgress");const Ln='button:not([disabled]), [href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])';function Dt(i,e){return t=>{if(t.key!=="Tab")return;const r=i();if(!r)return;const o=r.querySelector(e);if(!o)return;const n=Array.from(o.querySelectorAll(Ln));if(n.length===0)return;const s=n[0],l=n[n.length-1],d=r.activeElement;t.shiftKey?(d===s||!o.contains(d))&&(t.preventDefault(),l.focus()):(d===l||!o.contains(d))&&(t.preventDefault(),s.focus())}}var jn=Object.defineProperty,Lt=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&jn(e,t,o),o};const qt=class qt extends a.LitElement{constructor(){super(...arguments),this._url="",this._name="",this._error="",this._onBackdropClick=e=>{e.target===e.currentTarget&&this._cancel()},this._onUrlInput=e=>{this._url=e.target.value,this._error="",this._autoName()},this._onNameInput=e=>{this._name=e.target.value},this._focusTrap=Dt(()=>this.shadowRoot,".card"),this._onKeyDown=e=>{var t;e.key==="Escape"&&this._cancel(),e.key==="Enter"&&((t=e.target)==null?void 0:t.tagName)==="INPUT"&&this._submit(),this._focusTrap(e)}}_autoName(){var e;if(!this._name)try{const t=new URL(this._url).pathname.split("/"),r=t[t.length-1];if(r){const o=(e=this.shadowRoot)==null?void 0:e.querySelector("#nameInput");o&&(o.placeholder=r)}}catch{}}_cancel(){this.dispatchEvent(new CustomEvent("url-cancel",{bubbles:!0,composed:!0}))}_submit(){const e=this._url.trim();if(!e){this._error="Please enter a URL";return}try{new URL(e)}catch{this._error="Please enter a valid URL";return}this._error="";let t=this._name.trim();if(!t)try{const r=new URL(e).pathname.split("/");t=r[r.length-1]||"imported-file"}catch{t="imported-file"}this.dispatchEvent(new CustomEvent("url-submit",{detail:{url:e,name:t},bubbles:!0,composed:!0}))}connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{var e,t;(t=(e=this.shadowRoot)==null?void 0:e.querySelector("#urlInput"))==null||t.focus()})}render(){return a.html`
  ...
  `];let ce=Yt;Ve([w.state()],ce.prototype,"_stream");Ve([w.state()],ce.prototype,"_recording");Ve([w.state()],ce.prototype,"_error");Ve([w.state()],ce.prototype,"_recordedBlob");Ve([w.state()],ce.prototype,"_previewUrl");var Bn=Object.defineProperty,ri=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&Bn(e,t,o),o};const Vt=class Vt extends a.LitElement{constructor(){super(...arguments),this.duration=6e3,this._toasts=[],this._nextId=0}show(e,t="error"){const r=++this._nextId;this._toasts=[...this._toasts,{id:r,message:e,type:t,leaving:!1}],setTimeout(()=>this._dismiss(r),this.duration)}_dismiss(e){const t=this._toasts.findIndex(o=>o.id===e);if(t===-1)return;const r=[...this._toasts];r[t]={...r[t],leaving:!0},this._toasts=r,setTimeout(()=>{this._toasts=this._toasts.filter(o=>o.id!==e)},200)}_iconForType(e){return e==="error"?a.html`<svg class="toast-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
  ...
  `;let Me=Vt;ri([w.property({type:Number})],Me.prototype,"duration");ri([w.state()],Me.prototype,"_toasts");customElements.define("sfx-toast",Me);var In=Object.defineProperty,I=(i,e,t,r)=>{for(var o=void 0,n=i.length-1,s;n>=0;n--)(s=i[n])&&(o=s(e,t,o)||o);return o&&In(e,t,o),o};const Cr=new Set(["unsplash"]),ye={isTus:!1,tusUploadUrl:null};var X;const B=(X=class extends a.LitElement{constructor(){super(),this.config=null,this._isOpen=!1,this._activeConnector=null,this._showUrlDialog=!1,this._showCameraDialog=!1,this._showScreenCastDialog=!1,this._previewFileId=null,this._previewDims="—",this._fileInfoOpen=!0,this._splitPct=58,this._isResizing=!1,this._splitRafId=0,this._previewDefaultApplied=!1,this._fullscreenPreviewUrl=null,this._fullscreenVideoFile=null,this._fullscreenZoomed=!1,this._fsPanX=0,this._fsPanY=0,this._fsDragging=!1,this._fsDragStartX=0,this._fsDragStartY=0,this._fsPanStartX=0,this._fsPanStartY=0,this._bodyDragOver=!1,this._isMinimized=!1,this._isPillExpanded=!1,this._metadataSchema=null,this._bulkMetadataOpen=!1,this._isReviewing=!1,this._reviewFiles=[],this._hasStoredReview=!1,this._metadataAutocomplete=null,this._videoBlobUrls=new Map,this._lastEta=0,this._engine=null,this._cachedSources=ke,this._cachedSourcesConfig=void 0,this._rejectedTimers=new Map,this._closeOnCompleteTimer=null,this._apiBase=null,this._authHeaders=null,this._authResolveId=0,this._prevStoreState=null,this._unsubStoreEvents=null,this._portalContainer=null,this._onFileRename=e=>{this._onPreviewRename(e.detail.fileId,e.detail.name)},this._onPreviewMetadataBlur=e=>{const t=this._previewFileId;if(!t)return;const{key:r,value:o}=e.detail,n=this._store.getState().files.get(t);if(!n)return;const s=new Map(this._store.getState().files);s.set(t,{...n,meta:{...n.meta,[r]:o}}),this._store.setState({files:s})},this._transformRemoteThumbnail=(e,t)=>{var o;const r=(o=this.config)==null?void 0:o.transformRemoteThumbnail;if(!r)return e;try{return r(e,t)||e}catch(n){return console.warn("[sfx-uploader] transformRemoteThumbnail threw:",n),e}},this._connectorThumbnailTransform=e=>{const t=this._activeConnector;return t?this._transformRemoteThumbnail(e,{source:"connector",providerId:t}):e},this._onFilesSelected=e=>{this._processIncomingFiles(e.detail.files)},this._onDropTileSourceClick=e=>{e.stopPropagation(),this._handleSourceActivation(e.detail.source.id)},this._onSourceClick=async e=>{this._handleSourceActivation(e.detail.source)},this._handleSourceActivation=async e=>{var o,n;const t=this._mergedSources.find(s=>s.id===e);if(t!=null&&t.onActivate){try{t.onActivate(this)}catch(s){console.error(`[sfx-uploader] onActivate for custom source "${e}" threw:`,s)}return}if(e==="device"){const s=this.shadowRoot.querySelector("sfx-drop-zone");s==null||s.browse();return}if(e==="url"){this._showUrlDialog=!0;return}if(e==="camera"){this._showCameraDialog=!0;return}if(e==="screen-cast"){this._showScreenCastDialog=!0;return}if((((n=(o=this.config)==null?void 0:o.connectors)==null?void 0:n.providers)??[]).includes(e)){if(Cr.has(e)){if(!customElements.get("sfx-search-provider-browser")){const{SfxSearchProviderBrowser:l}=await Promise.resolve().then(()=>require("./search-provider-browser-D22Szl5n.cjs"));customElements.define("sfx-search-provider-browser",l)}}else if(!customElements.get("sfx-provider-browser")){const{SfxProviderBrowser:l}=await Promise.resolve().then(()=>require("./provider-browser-C8woh3ul.cjs"));customElements.define("sfx-provider-browser",l)}this._activeConnector=e}},this._onUrlSubmit=e=>{var C,$,z;this._showUrlDialog=!1;const{url:t,name:r}=e.detail,o=(C=this.config)==null?void 0:C.callbacks,n=xn(r),s=n.startsWith("image/"),l=this._store.getState();if([...l.files.values()].some(O=>O.name===r&&O.status!=="rejected"&&O.status!=="cancelled"))return;const u=kt({name:r,size:0,type:n},l.restrictions,l.files);if(u){const O={id:be(),status:"rejected",file:null,remoteUrl:t,name:r,size:0,type:n,previewUrl:null,duration:null,progress:0,speed:0,bytesUploaded:0,error:u,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:null,...ye};ve(this._store,O),this._dispatchPublic(j.FILE_REJECTED,{file:O,reason:u}),($=o==null?void 0:o.onFileRejected)==null||$.call(o,O,u);return}const h={id:be(),status:"idle",file:null,remoteUrl:t,name:r,size:0,type:n,previewUrl:s?this._transformRemoteThumbnail(t,{source:"url-import"}):null,duration:null,progress:0,speed:0,bytesUploaded:0,error:null,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:null,...ye};ve(this._store,h),this._dispatchPublic(j.FILE_ADDED,{file:h}),(z=o==null?void 0:o.onFileAdded)==null||z.call(o,h),this._store.getState().queueConfig.autoProceed&&this.upload()},this._onUrlCancel=()=>{this._showUrlDialog=!1},this._onCameraCapture=e=>{this._showCameraDialog=!1,this._processIncomingFiles([e.detail.file])},this._onCameraCancel=()=>{this._showCameraDialog=!1},this._onScreenCastCapture=e=>{this._showScreenCastDialog=!1,this._processIncomingFiles([e.detail.file])},this._onScreenCastCancel=()=>{this._showScreenCastDialog=!1},this._onFileRemove=e=>{this._removeFile(e.detail.fileId)},this._onFilePreview=e=>{var r,o,n;const t=this._store.getState().files.get(e.detail.fileId);t&&(this._previewFileId=t.id,this._dispatchPublic(j.FILE_PREVIEW,{file:t}),(n=(o=(r=this.config)==null?void 0:r.callbacks)==null?void 0:o.onFilePreview)==null||n.call(o,t))},this._onFillMetadata=()=>{var t,r,o,n;const e=[...this._store.getState().files.values()].filter(s=>X._MODIFIABLE_STATUSES.has(s.status));(t=this.config)!=null&&t.metadataConfig&&this._metadataSchema&&(this._bulkMetadataOpen=!0),this._dispatchPublic(j.FILL_METADATA,{files:e}),(n=(o=(r=this.config)==null?void 0:r.callbacks)==null?void 0:o.onFillMetadata)==null||n.call(o,e)},this._onFileLocate=e=>{var r,o,n;const t=e.detail.file;t&&(this._dispatchPublic(j.FILE_LOCATE,{file:t}),(n=(o=(r=this.config)==null?void 0:r.callbacks)==null?void 0:o.onFileLocate)==null||n.call(o,t))},this._onFileCopyCdn=e=>{var o,n,s;const t=e.detail.file,r=e.detail.cdnUrl;!t||!r||(this._dispatchPublic(j.FILE_COPY_CDN,{file:t,cdnUrl:r}),(s=(n=(o=this.config)==null?void 0:o.callbacks)==null?void 0:n.onFileCopyCdn)==null||s.call(n,t,r))},this._onBulkMetadataSaveBatch=e=>{const{changes:t}=e.detail;if(!t.length)return;const r=new Map(this._store.getState().files);for(const{fileId:o,meta:n}of t){const s=r.get(o);s&&r.set(o,{...s,meta:{...s.meta,...n}})}this._store.setState({files:r})},this._onBulkMetadataClose=()=>{this._bulkMetadataOpen=!1},this._onFileRetry=e=>{var t;this._ensureEngine(),(t=this._engine)==null||t.retryFile(e.detail.fileId)},this._onFilePause=e=>{var t;(t=this._engine)==null||t.pauseFile(e.detail.fileId)},this._onFileResume=e=>{var t;(t=this._engine)==null||t.resumeFile(e.detail.fileId)},this._onRetryAll=()=>{var e;this._ensureEngine(),(e=this._engine)==null||e.retryAll()},this._onClearAll=()=>{var r,o,n;const e=(r=this.config)==null?void 0:r.callbacks;this._closeOnCompleteTimer&&(clearTimeout(this._closeOnCompleteTimer),this._closeOnCompleteTimer=null),(o=this._engine)==null||o.cancelAll();const t=[...this._store.getState().files.values()];for(const s of t)s.previewUrl&&URL.revokeObjectURL(s.previewUrl),this._dispatchPublic(j.FILE_REMOVED,{file:s}),(n=e==null?void 0:e.onFileRemoved)==null||n.call(e,s);this._revokeVideoBlobUrls();for(const s of this._rejectedTimers.values())clearTimeout(s);this._rejectedTimers.clear(),this._dimCache.clear(),this._previewFileId=null,this._fullscreenPreviewUrl=null,this._fullscreenVideoFile=null,this._lastEta=0,this._store.setState({files:new Map,isUploading:!1,totalProgress:0,totalSpeed:0,totalBytesUploaded:0,totalBytes:0})},this._onAddMore=()=>{var o;const e=this.shadowRoot.querySelector("sfx-drop-zone");if(e){e.browse();return}const t=this.shadowRoot.querySelector("sfx-file-list"),r=(o=t==null?void 0:t.shadowRoot)==null?void 0:o.querySelector('input[type="file"]');r==null||r.click()},this._onUploadStart=()=>{var e;if(this._phase==="complete"){((e=this.config)==null?void 0:e.clearOnComplete)!==!1&&this._onClearAll();return}this._hasUnfilledRequiredMetadata||this.upload()},this._onUploadMore=()=>{this._onClearAll()},this._onEnterReview=()=>{const e=[...this._store.getState().files.values()].filter(o=>o.status==="complete"||o.status==="failed"||o.status==="error");if(e.length>0){this._reviewFiles=e,this._isReviewing=!0;return}const t=this._lastUploadId;if(t==null)return;const r=Oe.load(t);!r||r.length===0||(this._reviewFiles=r,this._isReviewing=!0)},this._onExitReview=()=>{this._isReviewing=!1,this._reviewFiles=[]},this._onClearReview=()=>{const e=this._lastUploadId;e!=null&&Oe.clear(e),this._isReviewing=!1,this._reviewFiles=[],this._hasStoredReview=!1},this._onConnectorFilesSelected=e=>{var r,o,n;const t=(r=this.config)==null?void 0:r.callbacks;for(const s of e.detail.files){const l=this._store.getState();if([...l.files.values()].some($=>$.name===s.name&&$.size===s.size&&$.status!=="rejected"&&$.status!=="cancelled"))continue;const u=s.thumbnail?this._transformRemoteThumbnail(s.thumbnail,{source:"connector",providerId:s.provider}):null,h=kt({name:s.name,size:s.size,type:s.mimeType},l.restrictions,l.files);if(h){const $={id:be(),status:"rejected",file:null,remoteUrl:null,name:s.name,size:s.size,type:s.mimeType,previewUrl:u,duration:null,progress:0,speed:0,bytesUploaded:0,error:h,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:s,...ye};ve(this._store,$),this._dispatchPublic(j.FILE_REJECTED,{file:$,reason:h}),(o=t==null?void 0:t.onFileRejected)==null||o.call(t,$,h);continue}const C={id:be(),status:"idle",file:null,remoteUrl:null,name:s.name,size:s.size,type:s.mimeType,previewUrl:u,duration:null,progress:0,speed:0,bytesUploaded:0,error:null,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:s,...ye};ve(this._store,C),this._dispatchPublic(j.FILE_ADDED,{file:C}),(n=t==null?void 0:t.onFileAdded)==null||n.call(t,C)}this._activeConnector=null,this._store.getState().queueConfig.autoProceed&&this.upload()},this._onConnectorClose=()=>{this._activeConnector=null},this._onConnectorBackdropClick=e=>{e.target===e.currentTarget&&(this._activeConnector=null)},this._onPrimaryAction=()=>{var e,t,r,o,n;this._dispatchPublic(j.COMPLETE_ACTION,{}),(r=(t=(e=this.config)==null?void 0:e.callbacks)==null?void 0:t.onCompleteAction)==null||r.call(t),((o=this.config)==null?void 0:o.mode)==="modal"?this.close():((n=this.config)==null?void 0:n.clearOnComplete)!==!1&&this._onClearAll()},this._onInlineDismiss=()=>{var e,t,r;(r=(t=(e=this.config)==null?void 0:e.callbacks)==null?void 0:t.onCancel)==null||r.call(t),this._dispatchPublic(j.CANCEL,{})},this._onSuccessCardClose=()=>{var e,t,r,o;((e=this.config)==null?void 0:e.mode)==="inline"?(this._dispatchPublic(j.COMPLETE_ACTION,{}),(o=(r=(t=this.config)==null?void 0:t.callbacks)==null?void 0:r.onCompleteAction)==null||o.call(r),this._onClearAll()):this._onModalDismiss()},this._onModalDismiss=()=>{var e,t,r,o;this._phase==="uploading"&&((e=this._engine)==null||e.cancelAll()),(o=(r=(t=this.config)==null?void 0:t.callbacks)==null?void 0:r.onCancel)==null||o.call(r),this._dispatchPublic(j.CANCEL,{}),this.close()},this._onCancelUpload=()=>{var e,t,r,o;(e=this._engine)==null||e.cancelAll(),(o=(r=(t=this.config)==null?void 0:t.callbacks)==null?void 0:r.onCancel)==null||o.call(r),this._dispatchPublic(j.CANCEL,{}),this._onClearAll()},this._onMinimize=()=>{this._isMinimized=!0,this._isPillExpanded=!0,this.requestUpdate()},this._onPillClick=()=>{this._isPillExpanded=!this._isPillExpanded,this.requestUpdate()},this._onPillExpand=()=>{this._isMinimized=!1,this._isPillExpanded=!1,this._isOpen=!0,this.requestUpdate()},this._onPillDismiss=()=>{var e,t,r,o;this._isMinimized=!1,this._isPillExpanded=!1,this._phase==="uploading"&&((e=this._engine)==null||e.cancelAll()),(o=(r=(t=this.config)==null?void 0:t.callbacks)==null?void 0:r.onCancel)==null||o.call(r),this._dispatchPublic(j.CANCEL,{}),this.close()},this._onModalBackdropClick=e=>{e.target===e.currentTarget&&this._onModalDismiss()},this._bodyLeaveTimer=null,this._onBodyDragEnter=e=>{e.preventDefault(),this._bodyLeaveTimer&&(clearTimeout(this._bodyLeaveTimer),this._bodyLeaveTimer=null),this._bodyDragOver=!0},this._onBodyDragOver=e=>{e.preventDefault(),this._bodyLeaveTimer&&(clearTimeout(this._bodyLeaveTimer),this._bodyLeaveTimer=null),this._bodyDragOver=!0},this._onBodyDragLeave=e=>{e.preventDefault(),this._bodyLeaveTimer&&clearTimeout(this._bodyLeaveTimer),this._bodyLeaveTimer=setTimeout(()=>{this._bodyDragOver=!1,this._bodyLeaveTimer=null},80)},this._onBodyDrop=e=>{var r;e.preventDefault(),this._bodyLeaveTimer&&(clearTimeout(this._bodyLeaveTimer),this._bodyLeaveTimer=null),this._bodyDragOver=!1;const t=Array.from(((r=e.dataTransfer)==null?void 0:r.files)??[]);t.length>0&&this._onFilesSelected(new CustomEvent("files-selected",{detail:{files:t}}))},this._onKeyDown=e=>{var t,r;if(e.key==="Escape"){if(this._fullscreenPreviewUrl||this._fullscreenVideoFile){this._onFsClose();return}const o=((t=this.config)==null?void 0:t.mode)??"modal",n=((r=this.config)==null?void 0:r.header)??(o==="modal"?"close":!0);(n==="close"||n==="back")&&(o==="modal"&&this._isOpen?this._onModalDismiss():o==="inline"&&this._onInlineDismiss())}},this._dimCache=new Map,this._onSplitPointerDown=e=>{var r;e.preventDefault(),this._isResizing=!0;const t=(r=this.shadowRoot)==null?void 0:r.querySelector(".preview-layout");t==null||t.classList.add("resizing"),e.target.setPointerCapture(e.pointerId)},this._onSplitPointerMove=e=>{if(!this._isResizing||this._splitRafId)return;const t=e.clientX;this._splitRafId=requestAnimationFrame(()=>{var s;this._splitRafId=0;const r=(s=this.shadowRoot)==null?void 0:s.querySelector(".preview-layout");if(!r)return;const o=r.getBoundingClientRect(),n=(t-o.left)/o.width*100;this._splitPct=Math.max(25,Math.min(75,n))})},this._onSplitPointerUp=()=>{var t;this._isResizing=!1,this._splitRafId&&(cancelAnimationFrame(this._splitRafId),this._splitRafId=0);const e=(t=this.shadowRoot)==null?void 0:t.querySelector(".preview-layout");e==null||e.classList.remove("resizing")},this._onFsToggleZoom=e=>{e==null||e.stopPropagation(),this._fullscreenZoomed=!this._fullscreenZoomed,this._fullscreenZoomed||(this._fsPanX=0,this._fsPanY=0)},this._onFsOverlayClick=e=>{this._fsDragDidMove||this._onFsToggleZoom(e)},this._fsDragDidMove=!1,this._onFsPanStart=e=>{this._fullscreenZoomed&&(this._fsDragging=!0,this._fsDragDidMove=!1,this._fsDragStartX=e.clientX,this._fsDragStartY=e.clientY,this._fsPanStartX=this._fsPanX,this._fsPanStartY=this._fsPanY,e.preventDefault())},this._onFsPanMove=e=>{if(!this._fsDragging)return;const t=e.clientX-this._fsDragStartX,r=e.clientY-this._fsDragStartY;(Math.abs(t)>3||Math.abs(r)>3)&&(this._fsDragDidMove=!0),this._fsPanX=this._fsPanStartX+t,this._fsPanY=this._fsPanStartY+r,this.requestUpdate()},this._onFsPanEnd=()=>{this._fsDragging=!1,requestAnimationFrame(()=>{this._fsDragDidMove=!1})},this._onFsTouchStart=e=>{if(!this._fullscreenZoomed||e.touches.length!==1)return;const t=e.touches[0];this._fsDragging=!0,this._fsDragDidMove=!1,this._fsDragStartX=t.clientX,this._fsDragStartY=t.clientY,this._fsPanStartX=this._fsPanX,this._fsPanStartY=this._fsPanY},this._onFsTouchMove=e=>{if(!this._fsDragging||e.touches.length!==1)return;const t=e.touches[0],r=t.clientX-this._fsDragStartX,o=t.clientY-this._fsDragStartY;(Math.abs(r)>3||Math.abs(o)>3)&&(this._fsDragDidMove=!0),this._fsPanX=this._fsPanStartX+r,this._fsPanY=this._fsPanStartY+o,this.requestUpdate(),e.preventDefault()},this._onFsClose=e=>{e==null||e.stopPropagation(),this._fullscreenPreviewUrl=null,this._fullscreenVideoFile=null,this._fullscreenZoomed=!1,this._fsPanX=0,this._fsPanY=0},this._store=$r(),this._storeCtrl=new ni(this,this._store)}get _lastUploadId(){var r,o;const e=(r=this.config)==null?void 0:r.lastUploadReview;if(!e)return null;if(typeof e=="string")return e;const t=(o=this.config)==null?void 0:o.auth;return t?t.airboxPuid?`${t.container}:${t.airboxPuid}`:t.container:null}open(){var e,t,r;this._isMinimized&&(this._isMinimized=!1,this._isPillExpanded=!1),!this._isOpen&&(this._isOpen=!0,(r=(t=(e=this.config)==null?void 0:e.callbacks)==null?void 0:t.onOpen)==null||r.call(t),this._dispatchPublic(j.OPEN,{}),this.requestUpdate())}close(){var e,t,r,o;this._isOpen&&(this._isOpen=!1,this._closeOnCompleteTimer&&(clearTimeout(this._closeOnCompleteTimer),this._closeOnCompleteTimer=null),((e=this.config)==null?void 0:e.clearOnClose)!==!1&&this._onClearAll(),this._previewFileId=null,(o=(r=(t=this.config)==null?void 0:t.callbacks)==null?void 0:r.onClose)==null||o.call(r),this._dispatchPublic(j.CLOSE,{}),this.requestUpdate())}upload(){var o,n,s,l,d,u,h;if(this._ensureEngine(),!this._engine){console.warn("[sfx-uploader] Cannot upload: auth not resolved yet");return}const e=[...this._store.getState().files.values()].filter(C=>C.status==="idle"||C.status==="queued");if((n=(o=this.config)==null?void 0:o.callbacks)!=null&&n.onBeforeUpload&&this.config.callbacks.onBeforeUpload(e)===!1)return;const t=new CustomEvent(j.BEFORE_UPLOAD,{bubbles:!0,composed:!0,cancelable:!0,detail:{files:e}});this.dispatchEvent(t)&&(this._dispatchPublic(j.UPLOAD_STARTED,{files:e}),(d=(l=(s=this.config)==null?void 0:s.callbacks)==null?void 0:l.onUploadStarted)==null||d.call(l,e),this._engine.uploadAll(),(u=this.config)!=null&&u.minimizeOnUpload&&((h=this.config)==null?void 0:h.mode)!=="inline"&&(this._isMinimized=!0,this._isPillExpanded=!0,this.requestUpdate()))}addFiles(e){this._processIncomingFiles(e)}resumeUpload(e){var t;if(e&&e.length>0){const r=this._store.getState().files,o=new Map(r);let n=!1;for(const s of e){const l=r.get(s.id);l&&(o.set(s.id,{...l,...s}),n=!0)}n&&this._store.setState({files:o})}this._ensureEngine(),(t=this._engine)==null||t.uploadAll()}cancelUpload(){var e;(e=this._engine)==null||e.cancelAll()}pauseFile(e){var t;(t=this._engine)==null||t.pauseFile(e)}resumeFile(e){var t;(t=this._engine)==null||t.resumeFile(e)}getFiles(){return[...this._store.getState().files.values()]}getFile(e){return this._store.getState().files.get(e)}updateFileMeta(e,t,r){const o=this._store.getState().files,n=o.get(e);if(!n||!X._MODIFIABLE_STATUSES.has(n.status))return;const s=new Map(o);s.set(e,{...n,meta:t!=null?{...n.meta,...t}:n.meta,tags:r??n.tags}),this._store.setState({files:s})}updateFilesMeta(e){const t=this._store.getState().files,r=new Map(t);let o=!1;for(const{fileId:n,meta:s,tags:l}of e){const d=t.get(n);!d||!X._MODIFIABLE_STATUSES.has(d.status)||(r.set(n,{...d,meta:s!=null?{...d.meta,...s}:d.meta,tags:l??d.tags}),o=!0)}o&&this._store.setState({files:r})}updated(e){if(e.has("config")&&this.config&&this._applyConfig(this.config),e.has("_previewFileId")&&this._previewFileId){const t=this._previewFileId,r=this._store.getState().files.get(t);r?this._getImageDimensions(r).then(o=>{this._previewFileId===t&&(this._previewDims=o?`${o.w} × ${o.h}`:"—")}):this._previewDims="—"}this._applyDefaultPreviewWidth(),this._updateFloatingPortal()}_applyDefaultPreviewWidth(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector(".preview-layout");if(!e){this._previewDefaultApplied=!1;return}this._previewDefaultApplied||e.getBoundingClientRect().width<=0||(this._splitPct=62.5,this._previewDefaultApplied=!0)}_injectFloatStyles(){if(document.querySelector("style[data-sfx-upload-float-styles]"))return;const e=document.createElement("style");e.setAttribute("data-sfx-upload-float-styles",""),e.textContent=`
    ...
    `,document.head.appendChild(e)}_updateFloatingPortal(){const e=[...this._storeCtrl.state.files.values()];this._isMinimized&&e.length>0?(this._injectFloatStyles(),this._portalContainer||(this._portalContainer=document.createElement("div"),this._portalContainer.setAttribute("data-sfx-upload-float",""),document.body.appendChild(this._portalContainer)),a.render(this._renderFloatingPill(e),this._portalContainer)):this._portalContainer&&(a.render(a.nothing,this._portalContainer),this._portalContainer.remove(),this._portalContainer=null)}connectedCallback(){super.connectedCallback(),document.addEventListener("keydown",this._onKeyDown),this._prevStoreState=this._store.getState(),this._unsubStoreEvents=this._store.subscribe(()=>this._onStoreChange());const e=this._lastUploadId;this._hasStoredReview=e!=null&&Oe.exists(e)}disconnectedCallback(){var e,t,r,o;super.disconnectedCallback(),document.removeEventListener("keydown",this._onKeyDown),(e=this._unsubStoreEvents)==null||e.call(this),this._unsubStoreEvents=null,this._prevStoreState=null,(t=this._portalContainer)==null||t.remove(),this._portalContainer=null,document.querySelector("[data-sfx-upload-float]")||(r=document.querySelector("style[data-sfx-upload-float-styles]"))==null||r.remove(),this._revokeVideoBlobUrls();for(const n of this._rejectedTimers.values())clearTimeout(n);this._rejectedTimers.clear(),this._closeOnCompleteTimer&&(clearTimeout(this._closeOnCompleteTimer),this._closeOnCompleteTimer=null);for(const n of this._store.getState().files.values())n.previewUrl&&URL.revokeObjectURL(n.previewUrl);(o=this._engine)==null||o.destroy(),this._engine=null}_applyConfig(e){const t={};if(e.targetFolder&&(t.targetFolder=e.targetFolder),(e.restrictions||e.forceName!=null)&&(t.restrictions={...this._store.getState().restrictions,...e.restrictions},e.forceName!=null&&(t.restrictions.maxNumberOfFiles=1)),e.concurrency!=null){const o=this._store.getState().queueConfig;t.queueConfig={...o,concurrency:e.concurrency}}if(e.autoProceed!=null){const o=t.queueConfig??this._store.getState().queueConfig;t.queueConfig={...o,autoProceed:e.autoProceed}}Object.keys(t).length>0&&this._store.setState(t);const r=this._lastUploadId;this._hasStoredReview=r!=null&&Oe.exists(r),this._resolveAuthAndEngine(e),(e.mode==="inline"||!e.mode)&&(this._isOpen=!0)}async _resolveAuthAndEngine(e){var o,n;const t=e.auth;if(t.mode==="sass-key"){this._apiBase=nt(t.container),this._authHeaders=et(t),this._ensureEngine(),(o=this._engine)==null||o.updateConfig({apiBase:this._apiBase,authHeaders:this._authHeaders,tusConfig:this._normalizeTusConfig(),resolveUploadParams:this._buildUploadParamsResolver()}),this._preloadMetadataSchema(e);return}const r=++this._authResolveId;try{const s=await Kr(t);if(r!==this._authResolveId)return;this._apiBase=s.apiBase,this._authHeaders=s.headers,this._ensureEngine(),(n=this._engine)==null||n.updateConfig({apiBase:this._apiBase,authHeaders:this._authHeaders,tusConfig:this._normalizeTusConfig(),resolveUploadParams:this._buildUploadParamsResolver()}),this._preloadMetadataSchema(e)}catch(s){if(r!==this._authResolveId)return;console.error("[sfx-uploader] Auth resolution failed:",s),this._showToast(this._formatAuthError(s))}}_formatAuthError(e){var r,o;const t=e instanceof Error?e.message:String(e);return(o=(r=this.config)==null?void 0:r.auth)!=null&&o.container?t.includes("HTTP 404")?`Authentication failed: container "${this.config.auth.container}" not found. Check your container name.`:t.includes("HTTP 401")||t.includes("HTTP 403")?"Authentication failed: invalid security template ID. Check your credentials in the Auth panel.":t.includes("timed out")?"Authentication failed: request timed out. Check your network connection.":t.includes("Failed to fetch")||t.includes("NetworkError")?"Authentication failed: network error. Check your internet connection.":`Authentication failed: ${t}`:"Authentication failed: no container specified. Open the Auth panel and enter your credentials."}_showToast(e,t="error"){var o;const r=(o=this.shadowRoot)==null?void 0:o.querySelector("sfx-toast");r==null||r.show(e,t)}_normalizeTusConfig(){var t;const e=(t=this.config)==null?void 0:t.tusConfig;return e===!0?{}:e||void 0}get _allowMulti(){var t;if(((t=this.config)==null?void 0:t.forceName)!=null)return!1;const e=this._storeCtrl.state.restrictions.maxNumberOfFiles;return e==null||e>1}_buildUploadParamsResolver(){const e=this.config;if(!e)return;const{forceName:t,getUploadParams:r}=e;if(!(t==null&&!r))return o=>{const n={};if(t!=null){const l=typeof t=="function"?t():t;l&&(n.opt_force_name=l)}const s=r==null?void 0:r(o);return s&&Object.assign(n,s),Object.keys(n).length>0?n:void 0}}_ensureEngine(){!this._engine&&this._apiBase&&this._authHeaders&&(this._engine=new Wr(this._store,{apiBase:this._apiBase,authHeaders:this._authHeaders,tusConfig:this._normalizeTusConfig(),resolveUploadParams:this._buildUploadParamsResolver()}),this._engine.start())}async _preloadMetadataSchema(e){const t=e.metadataConfig;if(!(!t||!this._apiBase||!this._authHeaders))try{const{fetchMetadataSchema:r,createTagsAutocomplete:o}=await Promise.resolve().then(()=>require("./index-CoNPDj5G.cjs"));this._metadataSchema=await r(this._apiBase,this._authHeaders,t.projectUuid,t),this._metadataAutocomplete=o(this._apiBase,this._authHeaders)}catch(r){console.error("[sfx-uploader] Failed to load metadata schema:",r),this._showToast("Failed to load metadata schema","warning")}}_onPreviewRename(e,t){const r=t.trim();if(!r)return;const o=this._store.getState().files.get(e);if(!o||o.name===r)return;const n=new Map(this._store.getState().files);n.set(e,{...o,name:r}),this._store.setState({files:n})}get _metadataEnforcing(){var t;const e=(t=this.config)==null?void 0:t.metadataConfig;return!e||!this._metadataSchema?!1:e.enforceRequiredBeforeUpload===!0?!0:e.enforceRequiredBeforeUpload==="auto"?this._metadataSchema.forceFillingOnUpload:!1}get _hasUnfilledRequiredMetadata(){if(!this._metadataEnforcing||!this._metadataSchema)return!1;const e=this._metadataSchema.fields.filter(r=>{var n;const o=(n=this.config)==null?void 0:n.metadataConfig;return o!=null&&o.requiredFields?o.requiredFields.includes(r.ckey):r.required===1});if(e.length===0)return!1;const t=[...this._store.getState().files.values()].filter(r=>r.status==="idle"||r.status==="queued"||r.status==="rejected");return e.some(r=>t.some(o=>{const n=o.meta[r.key];return n==null?!0:Array.isArray(n)||typeof n=="string"?n.length===0:!n}))}_dispatchPublic(e,t){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0,detail:t}))}_onStoreChange(){var o,n,s,l,d,u,h,C,$,z;const e=this._store.getState(),t=this._prevStoreState;if(this._prevStoreState=e,!t)return;e.isUploading&&!t.isUploading&&(this._lastEta=0);const r=(o=this.config)==null?void 0:o.callbacks;for(const[O,_]of e.files){const P=t.files.get(O);if(P){if(P.status!==_.status)switch(_.status){case"uploading":P.status==="paused"&&(this._dispatchPublic(j.UPLOAD_RESUMED,{file:_}),(n=r==null?void 0:r.onUploadResumed)==null||n.call(r,_));break;case"complete":_.response&&(this._dispatchPublic(j.UPLOAD_COMPLETE,{file:_,response:_.response}),(s=r==null?void 0:r.onUploadComplete)==null||s.call(r,_,_.response));break;case"error":case"failed":{const S=new Error(_.error??"Upload failed");this._dispatchPublic(j.UPLOAD_ERROR,{file:_,error:S}),(l=r==null?void 0:r.onUploadError)==null||l.call(r,_,S);break}case"retrying":this._dispatchPublic(j.UPLOAD_RETRY,{file:_,attempt:_.retryCount}),(d=r==null?void 0:r.onUploadRetry)==null||d.call(r,_,_.retryCount);break;case"paused":this._dispatchPublic(j.UPLOAD_PAUSED,{file:_}),(u=r==null?void 0:r.onUploadPaused)==null||u.call(r,_);break}_.status==="uploading"&&P.progress!==_.progress&&(this._dispatchPublic(j.UPLOAD_PROGRESS,{file:_,progress:_.progress,speed:_.speed}),(h=r==null?void 0:r.onUploadProgress)==null||h.call(r,_,_.progress,_.speed))}}if(e.totalProgress!==t.totalProgress||e.totalSpeed!==t.totalSpeed){const O=e.totalSpeed>0?(e.totalBytes-e.totalBytesUploaded)/e.totalSpeed:e.isUploading?this._lastEta:0;e.totalSpeed>0&&(this._lastEta=O),this._dispatchPublic(j.TOTAL_PROGRESS,{percentage:e.totalProgress,speed:e.totalSpeed,eta:O}),(C=r==null?void 0:r.onTotalProgress)==null||C.call(r,e.totalProgress,e.totalSpeed,O)}if(t.isUploading&&!e.isUploading){const O=[...e.files.values()];if(!O.some(P=>P.status==="cancelled")){const P=O.filter(g=>g.status==="complete"),S=O.filter(g=>g.status==="failed"||g.status==="error"),m=this._lastUploadId;if(m!=null){const g=[...P,...S];Oe.save(m,g),this._hasStoredReview=g.length>0}this._dispatchPublic(j.ALL_COMPLETE,{successful:P,failed:S}),($=r==null?void 0:r.onAllComplete)==null||$.call(r,P,S);const v=(z=this.config)==null?void 0:z.closeOnComplete;if(v){const g=typeof v=="number"?v:1500;this._closeOnCompleteTimer=setTimeout(()=>{var x,R,U;this._closeOnCompleteTimer=null,this._phase==="complete"&&(this._dispatchPublic(j.COMPLETE_ACTION,{}),(U=(R=(x=this.config)==null?void 0:x.callbacks)==null?void 0:R.onCompleteAction)==null||U.call(R),this.close())},g)}}}}get _mergedSources(){var d;const e=(d=this.config)==null?void 0:d.connectors;if(e===this._cachedSourcesConfig)return this._cachedSources;if(this._cachedSourcesConfig=e,!e)return this._cachedSources=ke,this._cachedSources;const t=e.providers.length>0?ei(e.providers):[],r=e.customSources??[],o=ke.filter(u=>u.id==="device"||u.id==="url"),n=ke.filter(u=>u.id!=="device"&&u.id!=="url"),s=new Set,l=[];for(const u of[...o,...t,...n,...r])if(!s.has(u.id)){if(X._RESERVED_IDS.has(u.id)&&u.onActivate){console.warn(`[sfx-uploader] Custom source id "${u.id}" conflicts with a built-in source and was skipped.`);continue}s.add(u.id),l.push(u)}return this._cachedSources=l,this._cachedSources}get _phase(){const e=this._storeCtrl.state,t=[...e.files.values()];if(t.length===0)return"empty";if(e.isUploading)return"uploading";const r=new Set(["complete","rejected","cancelled","failed"]);return t.every(o=>r.has(o.status))&&t.some(o=>o.status==="complete"||o.status==="failed")?"complete":"ready"}_processIncomingFiles(e){var r,o,n,s;const t=(r=this.config)==null?void 0:r.callbacks;this._isReviewing&&(this._isReviewing=!1,this._reviewFiles=[]);for(const l of e){const d=this._store.getState();if([...d.files.values()].some(z=>z.name===l.name&&z.size===l.size&&z.status!=="rejected"&&z.status!=="cancelled"))continue;const h=yn(l,d.restrictions,d.files);if(h){const z=l.type.startsWith("image/")?URL.createObjectURL(l):null,O={id:be(),status:"rejected",file:l,remoteUrl:null,name:l.name,size:l.size,type:l.type,previewUrl:z,duration:null,progress:0,speed:0,bytesUploaded:0,error:h,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:null,...ye};ve(this._store,O),this._dispatchPublic(j.FILE_REJECTED,{file:O,reason:h}),(o=t==null?void 0:t.onFileRejected)==null||o.call(t,O,h);const _=(n=this.config)==null?void 0:n.rejectedFileAutoRemoveDelay,P=_===!1||_===0||_===void 0?0:_;if(P>0){const S=O.id,m=setTimeout(()=>{this._rejectedTimers.delete(S);const v=this._store.getState().files.get(S);v&&v.status==="rejected"&&Wt(this._store,S)},P);this._rejectedTimers.set(S,m)}continue}let C=null;l.type.startsWith("image/")&&(C=URL.createObjectURL(l));const $={id:be(),status:"idle",file:l,remoteUrl:null,name:l.name,size:l.size,type:l.type,previewUrl:C,duration:null,progress:0,speed:0,bytesUploaded:0,error:null,retryCount:0,response:null,addedAt:Date.now(),meta:{},tags:[],remoteInfo:null,...ye};if(ve(this._store,$),this._dispatchPublic(j.FILE_ADDED,{file:$}),(s=t==null?void 0:t.onFileAdded)==null||s.call(t,$),l.type.startsWith("video/")){bn(l).then(O=>{if(!O)return;const _=this._store.getState(),P=_.files.get($.id);if(P){const S=new Map(_.files);S.set($.id,{...P,previewUrl:O}),this._store.setState({files:S})}else URL.revokeObjectURL(O)});const z=document.createElement("video");z.preload="metadata",z.src=URL.createObjectURL(l),z.onerror=()=>{URL.revokeObjectURL(z.src)},z.onloadedmetadata=()=>{const O=z.duration;if(URL.revokeObjectURL(z.src),!isFinite(O))return;const _=this._store.getState(),P=_.files.get($.id);if(P){const S=new Map(_.files);S.set($.id,{...P,duration:O}),this._store.setState({files:S})}}}}this._store.getState().queueConfig.autoProceed&&this.upload()}_removeFile(e){var n,s,l,d;const t=this._store.getState().files.get(e);if(!t)return;const r={...t};if((this._fullscreenPreviewUrl&&this._fullscreenPreviewUrl===t.previewUrl||this._fullscreenVideoFile&&this._fullscreenVideoFile===t.file)&&(this._fullscreenPreviewUrl=null,this._fullscreenVideoFile=null),t.previewUrl&&URL.revokeObjectURL(t.previewUrl),t.file){const u=this._videoBlobUrls.get(t.file);u&&(URL.revokeObjectURL(u),this._videoBlobUrls.delete(t.file))}(t.status==="uploading"||t.status==="queued"||t.status==="retrying"||t.status==="paused")&&((n=this._engine)==null||n.cancelFile(e)),Wt(this._store,e),this._dimCache.delete(e);const o=this._rejectedTimers.get(e);if(o&&(clearTimeout(o),this._rejectedTimers.delete(e)),this._previewFileId===e){const u=[...this._store.getState().files.values()];this._previewFileId=u.length>0?u[0].id:null}this._dispatchPublic(j.FILE_REMOVED,{file:r}),(d=(l=(s=this.config)==null?void 0:s.callbacks)==null?void 0:l.onFileRemoved)==null||d.call(l,r)}render(){var r;const e=((r=this.config)==null?void 0:r.mode)??"modal",t=[...this._storeCtrl.state.files.values()];return e==="modal"?a.html`
    ...
    `}_renderFsOverlay(){if(!this._fullscreenPreviewUrl&&!this._fullscreenVideoFile)return a.nothing;const e=[...this._store.getState().files.values()].filter(r=>r.previewUrl||r.type.startsWith("video/")&&r.file),t=e.findIndex(r=>r.id===this._previewFileId);return a.html`
      ...
      >
        ${this._fullscreenVideoFile?a.html`<video class="fs-img" src=${this._getVideoBlobUrl(this._fullscreenVideoFile)} controls playsinline draggable="false" @click=${r=>r.stopPropagation()}></video>`:a.html`<img class="fs-img" src=${this._fullscreenPreviewUrl} alt="" ${N(this._fullscreenZoomed?{transform:`scale(2) translate(${this._fsPanX}px, ${this._fsPanY}px)`}:null)} draggable="false" />`}
      </div>
    ...
    `}_renderHeader(){var s,l,d;if(this._phase==="complete")return a.nothing;const e=((s=this.config)==null?void 0:s.mode)??"modal";if(this._phase==="uploading"){const h=[...this._storeCtrl.state.files.values()],C=h.filter($=>$.status==="complete").length;return a.html`
          ...
          <div class="float-header-left">
            ...
            <div>
              ...
              <div class="float-subtitle">
                ${C} of
                ${h.length}${this._lastEta>0?` · ~${pt(this._lastEta)} left`:""}
              </div>
            </div>
          </div>
        </div>
    ...
    `}_getImageDimensions(e){return e.previewUrl?this._dimCache.has(e.id)?Promise.resolve(this._dimCache.get(e.id)):new Promise(t=>{const r=new Image;r.onload=()=>{const o={w:r.naturalWidth,h:r.naturalHeight};this._dimCache.set(e.id,o),t(o)},r.onerror=()=>{this._dimCache.set(e.id,null),t(null)},r.src=e.previewUrl}):Promise.resolve(null)}_renderUploadOverlay(e){var n;const t=this._storeCtrl.state,r=Math.round(t.totalProgress??0),o=e.filter(s=>s.status==="complete").length;return a.html`
      <div class="upload-overlay">
        ...
        <div class="upload-overlay-subtitle">
          ${o} of ${e.length}
          complete${this._lastEta>0?a.html` · ~${pt(this._lastEta)} left`:a.nothing}
        </div>
        <div class="upload-overlay-bar">
          <div class="upload-overlay-bar-fill" ${N({width:`${r}%`})}></div>
        </div>
        ...
        <div class="float-header">
          <div class="float-header-left">
            ...
            <div>
              ...
              <div class="float-subtitle">
                ${o?`${n} ${n===1?"file":"files"} uploaded${s>0?`, ${s} failed`:""}`:`${n} of ${e.length}${this._lastEta>0?` · ~${pt(this._lastEta)} left`:""}`}
              </div>
            </div>
          </div>
          ...
          ${e.map(l=>{const d=l.status==="failed"||l.status==="error";return a.html`
              <div class="float-item">
                <div
                  ...
                  ${N(l.previewUrl?{"background-image":`url(${l.previewUrl})`,"background-size":"cover","background-position":"center"}:null)}
                >
                ...
                <div class="float-item-info">
                  ...
                  <div class="float-item-size">${se(l.size)}</div>
                </div>
                <div class="float-item-status">
                  ${l.status==="complete"?a.html`<div class="float-item-done">
                        ...
                        <button
                          ...
                          @click=${()=>{var u;this._ensureEngine(),(u=this._engine)==null||u.retryFile(l.id)}}
                        >
                          <svg
                            viewBox="0 0 24 24"
                            fill="none"
    ...
    `}_renderPreviewLayout(e){var n,s,l,d,u;if(e.length===0)return a.nothing;const t=e.find(h=>h.id===this._previewFileId)??e[0],r=((n=t.name.split(".").pop())==null?void 0:n.toUpperCase())||"";new Date(t.addedAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),this._store.getState().targetFolder;const o=e.reduce((h,C)=>h+(C.size||0),0);return a.html`
      <div class="preview-topbar"></div>
      <div class="preview-layout">
        <div class="file-grid-side" ${N({flex:String(this._splitPct)})}>
        ...
        ></div>
        <div class="preview-panel" ${N({flex:String(100-this._splitPct)})}>
                ...
                <div class="preview-media-area">
                  <div class="preview-doc-wrap ${Jr(t)}">
                    <img
                      ...
                      src=${Rt(r)}
                      alt="${r?`${r} file`:"File"}"
                      @error=${h=>{const C=h.target,$=Tt();!C.dataset.fallback&&C.src!==$&&(C.dataset.fallback="1",C.src=$)}}
                    />
                  </div>
                  <button
                    class="preview-nav prev"
          ...
          ${this._metadataSchema&&((d=this.config)!=null&&d.metadataConfig)?a.html`<div class="preview-meta-list">
                <div class="preview-file-info">
                  ${r}${t.size?` · ${se(t.size)}`:""}${this._previewDims!=="—"?` · ${this._previewDims}`:""}
                </div>
              </div>`:a.nothing}
          ${this._metadataSchema&&((u=this.config)!=null&&u.metadataConfig)?a.html`
                <div
    ...
    `}_navigatePreview(e,t){var n;const o=e.findIndex(s=>s.id===this._previewFileId)+t;if(o>=0&&o<e.length){const s=(n=this.shadowRoot)==null?void 0:n.querySelector(".preview-image[controls]");s&&(s.pause(),s.removeAttribute("src"),s.load()),this._previewFileId=e[o].id}}_renderBody(){var s,l,d,u,h,C,$,z,O,_,P;const e=this._storeCtrl.state,t=[...e.files.values()],r=this._phase,o=yr(e.restrictions),n=t.length>0;return a.html`
      ...
      >
          ...
          ${this._isReviewing?a.html`
              ...
              `:r==="uploading"?this._renderUploadOverlay(t):a.html`
                ${n?a.nothing:a.html`<sfx-drop-zone
                        ...
                        .accept=${o}
                        .sources=${this._mergedSources}
                        .sourcesLayout=${((C=this.config)==null?void 0:C.sourcesLayout)??"pills"}
        ...
        ${this._activeConnector&&((_=this.config)!=null&&_.connectors)?a.html`
              <div
                class="connector-modal-backdrop"
                @click=${this._onConnectorBackdropClick}
              >
                <div class="connector-modal">
                  ${Cr.has(this._activeConnector)?a.html`
        ...
        ${this._bulkMetadataOpen&&this._metadataSchema?a.html`
              <sfx-bulk-metadata-modal
                .schema=${this._metadataSchema}
                .files=${[...this._store.getState().files.values()].filter(S=>X._MODIFIABLE_STATUSES.has(S.status))}
  ...
  `,X._MODIFIABLE_STATUSES=new Set(["idle","queued","rejected"]),X._RESERVED_IDS=new Set(["device","camera","url","screen-cast"]),X);I([w.property({attribute:!1})],B.prototype,"config");I([w.state()],B.prototype,"_isOpen");I([w.state()],B.prototype,"_activeConnector");I([w.state()],B.prototype,"_showUrlDialog");I([w.state()],B.prototype,"_showCameraDialog");I([w.state()],B.prototype,"_showScreenCastDialog");I([w.state()],B.prototype,"_previewFileId");I([w.state()],B.prototype,"_previewDims");I([w.state()],B.prototype,"_fileInfoOpen");I([w.state()],B.prototype,"_splitPct");I([w.state()],B.prototype,"_fullscreenPreviewUrl");I([w.state()],B.prototype,"_fullscreenVideoFile");I([w.state()],B.prototype,"_fullscreenZoomed");I([w.state()],B.prototype,"_bodyDragOver");I([w.state()],B.prototype,"_isMinimized");I([w.state()],B.prototype,"_isPillExpanded");I([w.state()],B.prototype,"_metadataSchema");I([w.state()],B.prototype,"_bulkMetadataOpen");I([w.state()],B.prototype,"_isReviewing");I([w.state()],B.prototype,"_reviewFiles");I([w.state()],B.prototype,"_hasStoredReview");let Mn=B;exports.AuthExpiredError=ot;exports.CORE_SOURCES=ke;exports.PublicEvents=j;exports.SfxActionsBar=Z;exports.SfxCameraDialog=he;exports.SfxDropZone=En;exports.SfxFileItem=te;exports.SfxFileList=W;exports.SfxImportDivider=St;exports.SfxScreenCastDialog=ce;exports.SfxSourcePills=rt;exports.SfxSuccessCard=ie;exports.SfxUploader=Mn;exports.SfxUrlDialog=$e;exports.Store=Sr;exports.UploadEngine=Wr;exports.brandIcon=ae;exports.buildAuthHeaders=et;exports.createStore=$r;exports.cspStyle=N;exports.exchangeSassKey=Xr;exports.formatFileSize=se;exports.getApiBase=nt;exports.getAuthUrl=di;exports.getDefaultFileTypeIconUrl=Tt;exports.getFileTypeIconUrl=Rt;exports.getProviderSources=ei;exports.listFiles=ci;exports.listNextPage=pi;exports.logout=hi;exports.resolveAuth=Kr;exports.searchProvider=ui;
