import { H as $t, I as G, J as Bt, K as St, L as h, M as Ct, N as Et, O as ee, Q as we, R as Kt, T as Nt, V as ne, W as ht, X as Mt, Y as jt, Z as Vt, _ as Ut, $ as ft, a0 as Ht, a1 as Gt, a2 as Yt } from "./sfx-uploader-CsfmZlqJ.js";
import { a3 as bs, a4 as vs, a5 as ys } from "./sfx-uploader-CsfmZlqJ.js";
import { html as l, LitElement as P, css as _, nothing as m, svg as S } from "lit";
import { property as u, state as x } from "lit/decorators.js";
import { classMap as It } from "lit/directives/class-map.js";
function Tt(n, e, t = !1) {
  const i = (e == null ? void 0 : e.language) ?? "en", s = n.model ?? [], r = n.store ?? {}, o = s.find((g) => g.applies_to === "FILES");
  let a = (o == null ? void 0 : o.groups) ?? [];
  if (Array.isArray(e == null ? void 0 : e.fields)) {
    const g = new Set(e.fields);
    a = a.map((v) => ({
      ...v,
      fields: v.fields.filter((y) => g.has(y.ckey))
    })).filter((v) => v.fields.length > 0);
  }
  a = a.map((g) => ({
    ...g,
    fields: g.fields.filter((v) => !v.hide)
  })).filter((g) => g.fields.length > 0);
  const d = a.flatMap((g) => g.fields), c = new Map(d.map((g) => [g.key, g])), p = typeof r.force_filling_metadata_on_upload == "boolean" ? r.force_filling_metadata_on_upload : void 0, f = r.regional_variants_groups ?? [];
  return {
    groups: a,
    fields: d,
    fieldsByKey: c,
...
const ze = "https://hub.scaleflex.com/api", At = "The Hub API uses session-based auth (not the Filerobot SASS key); pass metadataConfig.hubHeaders with x-company-token, x-project-token and x-session-token. If the session token arrives asynchronously, assign a new config object once it is available — in-place mutation is not detected.", Jt = (n) => n.replace(/\/+$/, "");
function qe(n) {
  return !n || Jt(n) === ze;
}
function Be(n) {
  const e = n == null ? void 0 : n.hubHeaders;
  return e && Object.keys(e).length > 0 ? e : void 0;
}
function Ot(n) {
  return Object.keys(n).some((e) => e.toLowerCase() === "x-session-token");
}
function ns(n) {
  return Be(n) ? !0 : !qe(n == null ? void 0 : n.hubApiBase);
}
const ue = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map();
function as(n) {
  return ue.has(n);
}
async function ls(n, e, t, i) {
...
async function Qt(n, e, t) {
  var p, f, g, v, y;
  const i = (t == null ? void 0 : t.hubApiBase) ?? ze, s = `${i}/project/${encodeURIComponent(e)}`, r = Be(t) ?? n;
  if (qe(i) && !Ot(r))
    throw new Error(
      `Cannot fetch the metadata schema: no Hub session headers. ${At}`
    );
  const o = await fetch(s, { headers: r });
  if (!o.ok)
    throw new Error(
      `Failed to fetch metadata schema (HTTP ${o.status})`
    );
  const a = await o.json(), d = ((f = (p = a.data) == null ? void 0 : p.project) == null ? void 0 : f.data) ?? ((g = a.project) == null ? void 0 : g.data);
  if (!(d != null && d.metadata))
    throw new Error("No metadata in project response");
  const c = (t == null ? void 0 : t.productsEnabled) ?? ((y = (v = d == null ? void 0 : d.airstore) == null ? void 0 : v.ui) == null ? void 0 : y.products_enabled) === !0;
  return Tt(d.metadata, t, c);
}
function ds(n) {
  var e;
  n ? (ue.delete(n), (e = ae.get(n)) == null || e.catch(() => {
  }), ae.delete(n)) : (ue.clear(), ae.clear());
}
...
function ei(n) {
  ...
  return {
    ...
    active: n.dep_active,
    formatMimetypes: Xt(n.dep_scope),
    ...
    triggerValues: e,
    actions: (n.dep_actions ?? []).map(Zt)
  };
}
function ti(n) {
  if (!n) return [];
  const e = Array.isArray(n) ? n : n.dependencies;
  return Array.isArray(e) ? e.filter((t) => t == null ? void 0 : t.dep_active).map(ei) : [];
}
function ii(n, e, t) {
  if (n.length === 0 || t.length === 0) return /* @__PURE__ */ new Map();
  const i = n.map(
    (o) => $t(
      { mime: o.mime, meta: o.meta },
      e,
      t
    )
  ), s = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Set();
  for (const o of i) for (const a of o.keys()) r.add(a);
  for (const o of r) {
    const a = i.map((b) => b.get(o)), d = a.every((b) => (b == null ? void 0 : b.hidden) === !0), c = a.some((b) => (b == null ? void 0 : b.required) === !0);
    let p;
    if (a.every((b) => Array.isArray(b == null ? void 0 : b.allowedValues))) {
      let b;
      for (const re of a) {
        const J = re.allowedValues;
        if (b = b === void 0 ? [...J] : b.filter((qt) => J.includes(qt)), b.length === 0) break;
      }
      p = b;
    }
    let g;
    const v = a.map((b) => b == null ? void 0 : b.setValue).filter((b) => b !== void 0);
    v.length === n.length && si(v) && (g = v[0]);
    const y = /* @__PURE__ */ new Set();
    for (const b of a)
      if (b)
        for (const re of b.contributingDependencyUuids) y.add(re);
    const E = {
      hidden: d,
      required: c,
      contributingDependencyUuids: [...y]
    };
    p !== void 0 && (E.allowedValues = p), g !== void 0 && (E.setValue = g), s.set(o, E);
  }
  return s;
}
function si(n) {
...
const pe = /* @__PURE__ */ new Map(), le = /* @__PURE__ */ new Map();
function cs(n) {
  return pe.has(n);
}
...
async function ri(n, e) {
  const t = (e == null ? void 0 : e.hubApiBase) ?? ze, i = `${t}/metadata/dependencies`, s = Be(e) ?? n;
  if (qe(t) && !Ot(s))
    throw new Error(
      `Cannot fetch metadata dependencies: no Hub session headers. ${At}`
    );
  const r = await fetch(i, { headers: s });
  if (!r.ok)
    throw new Error(
      `Failed to fetch metadata dependencies (HTTP ${r.status})`
    );
...
function ps(n) {
  var e;
  n ? (pe.delete(n), (e = le.get(n)) == null || e.catch(() => {
  }), le.delete(n)) : (pe.clear(), le.clear());
}
const oi = 300, gt = 2, ni = 50, mt = "regvar:api", ai = "#ut", li = {
  CREATE_ONLY: "create_only",
  UPSERT: "upsert"
}, di = (n) => typeof n == "string" && n.startsWith(ai), ve = (n) => `~${n.toUpperCase()}`, ci = (n, e) => {
  if (!(!n || !e))
    return n[e] ?? n[ve(e)];
}, ye = (n, e, t) => {
  var o, a, d;
  const i = (o = n.i18n) == null ? void 0 : o[e];
  if (i)
    return { value: i, isFallback: !1, sourceLang: e };
  const s = (a = n.i18n) == null ? void 0 : a[ve(e)];
  if (s)
    return { value: s, isFallback: !0, sourceLang: ve(e) };
  const r = ci(n.i18n, t);
  if (r) {
    const c = (d = n.i18n) != null && d[t] ? t : ve(t);
    return { value: r, isFallback: !0, sourceLang: c };
  }
  return { value: "", isFallback: !1, sourceLang: null };
}, xt = (n) => (typeof n == "string" ? n : "").toLowerCase().trim().replace(/[^\d\w]/g, "_").replace(/[\s]/g, "_").replace(/[_]{2,}/g, "_").replace(/[_]*$/g, "").replace(/^[_]*/g, ""), ui = (n) => {
  const e = {}, t = {};
  for (const i of n || [])
    i.sid && (e[i.sid] = i), i.slug && (t[i.slug] = i);
  return { bySid: e, bySlug: t };
}, _e = (n) => {
  const e = [], t = /* @__PURE__ */ new Map(), i = (r) => r.sid || r.slug || "", s = (r, o) => {
    if (typeof r == "string") {
      if (!r || t.has(r)) return;
      t.set(r, e.length), e.push({ slug: r });
      return;
    }
    ...
    const f = { ...p.i18n, ...a.i18n || {} };
    o && a.label && (f[o] = a.label), Object.keys(f).length > 0 && (p.i18n = f), c !== void 0 ? e[c] = p : (t.set(d, e.length), a.slug && a.slug !== d && t.set(a.slug, e.length), e.push(p));
  };
  if (Array.isArray(n))
    ...
    for (const [o, a] of Object.entries(r))
      if (Array.isArray(a))
        for (const d of a) s(d, o);
  }
  return e;
}, xe = (n) => typeof n == "string" ? [n] : !n || typeof n != "object" ? [] : [n.sid, n.slug, n.uuid].filter(Boolean), Ie = (n, e, t) => {
  if (t) {
    const r = new Set(e.flatMap(xe));
    return n.filter((o) => !xe(o).some((a) => r.has(a)));
  }
  const i = new Set(n.flatMap(xe)), s = e.filter(
    (r) => !xe(r).some((o) => i.has(o))
  );
  return [...n, ...s];
}, pi = (n, e) => n.map((t) => {
...
function Ke(n, e, t, i) {
  ...
  switch (n.type) {
    case "geopoint": {
      const o = e;
      !o || o.latitude === "" || o.latitude == null || o.longitude === "" || o.longitude == null ? s = null : s = `(${o.latitude},${o.longitude})`;
      break;
    }
    case "boolean":
    ...
    case "date": {
      if (!e)
        ...
        const o = e instanceof Date ? e : new Date(e), a = o.getFullYear(), d = String(o.getMonth() + 1).padStart(2, "0"), c = String(o.getDate()).padStart(2, "0");
        s = `${a}-${d}-${c}`;
      }
      break;
    }
    case "tags":
      s = Array.isArray(e) ? e.map((o) => (o == null ? void 0 : o.label) ?? "") : [];
      break;
    case "ultratags":
      s = Array.isArray(e) ? e.map((o) => typeof o == "string" ? o : o.slug).filter((o) => !!o) : [];
      break;
    case "select-one":
      s = e === "" ? null : e;
      break;
    case "numeric": {
      ...
      const o = Number(e);
      s = Number.isFinite(o) ? Math.round(o) : null;
      break;
    }
    case "decimal2": {
      if (e === "" || e == null) {
  ...
  if (n.regional_variants_group_uuid && n.type !== "ultratags") {
    const o = i ?? "en";
    return { ...((r = t == null ? void 0 : t.meta) == null ? void 0 : r[n.key]) ?? {}, [o]: s };
  }
  return s;
}
function hi(n) {
...
function hs(n, e) {
  ...
  return {
    search(o, a, d) {
      if (i && clearTimeout(i), s && s.abort(), r = !1, !a.trim()) {
        d([]);
        return;
      }
      i = setTimeout(async () => {
        ...
        try {
          const p = `${t}/v5/metadata/autocomplete?q=${encodeURIComponent(a.trim())}&meta_key=_${encodeURIComponent(o)}&limit=20`, f = await fetch(p, { headers: e, signal: s.signal });
          if (r) return;
          if (!f.ok) {
            d([]);
            return;
          ...
          if (r) return;
          const v = ((c = g.data) == null ? void 0 : c.tags) ?? g.tags ?? [];
          d(
            v.map((y) => ({
              sid: y.sid || void 0,
              value: y.tag || y.value || y.label || "",
              label: y.tag || y.label || y.value || ""
            }))
          );
        } catch {
          r || d([]);
    ...
    cancel() {
      r = !0, i && clearTimeout(i), s && s.abort();
    }
  };
}
const bt = { base_node: null, nodes: [] };
function fs(n, e) {
  const t = n.replace(/\/$/, "");
  let i = null, s = null, r = !1, o = null, a = null;
  return {
    fetchTaxonomies() {
      return a || (a = (async () => {
        var c;
        ...
        try {
          ...
          const f = await p.json(), g = (f == null ? void 0 : f.taxonomies) ?? ((c = f == null ? void 0 : f.data) == null ? void 0 : c.taxonomies) ?? (f == null ? void 0 : f.data) ?? f;
          return Array.isArray(g) ? g : (console.warn(
    ...
    async fetchNodes(d, c = "", p = 10) {
      ...
      try {
        ...
        c && f.set("base", c), f.set("limit", String(p));
        const g = `${t}/v5/taxonomy/${encodeURIComponent(d)}/nodes?${f.toString()}`, v = await fetch(g, { headers: e, signal: o.signal });
        if (!v.ok) return bt;
        const y = await v.json(), E = (y == null ? void 0 : y.data) ?? y;
        return {
          base_node: (E == null ? void 0 : E.base_node) ?? null,
          nodes: Array.isArray(E == null ? void 0 : E.nodes) ? E.nodes : []
        };
      } catch {
        return bt;
      }
    ...
    autocomplete(d, c, p) {
      if (i && clearTimeout(i), s && s.abort(), r = !1, !c.trim()) {
        p([]);
        return;
      }
      i = setTimeout(async () => {
        ...
        s = new AbortController();
        try {
          const g = `${t}/v5/metadata/autocomplete?q=${encodeURIComponent(c.trim())}&meta_key=_${encodeURIComponent(d)}`, v = await fetch(g, {
            headers: e,
            signal: s.signal
          });
          if (r) return;
          ...
          if (r) return;
          const E = ((f = y == null ? void 0 : y.data) == null ? void 0 : f.tags) ?? (y == null ? void 0 : y.tags) ?? [];
          p(
            E.map((b) => ({
              tag: String(b.tag ?? b.path ?? ""),
              path: String(b.path ?? b.tag ?? ""),
              suid: String(b.suid ?? ""),
              uuid: String(b.uuid ?? ""),
              approx_count: typeof b.approx_count == "number" ? b.approx_count : void 0
            }))
          );
        } catch {
          r || p([]);
    ...
    cancel() {
      r = !0, i && clearTimeout(i), s && s.abort(), o && o.abort();
    }
  };
}
const Ne = "/v5/meta/ultratags", fi = (n, e) => {
...
function gs(n, e) {
  ...
  return {
    list(r) {
      return s(), new Promise((o, a) => {
        t = setTimeout(async () => {
          t = null, i = new AbortController();
          try {
            const d = fi(n, r), c = await fetch(d, {
              method: "GET",
              headers: e,
              signal: i.signal
            });
...
var mi = Object.defineProperty, F = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && mi(e, t, s), s;
};
const xi = l`<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
...
</svg>`, Ue = class Ue extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.schema = null, this.meta = {}, this.config = null, this.taxonodes = null, this.resolvedSchema = null, this.dependencies = [], this.disabled = !1, this.hideFilter = !1, this._collapsed = /* @__PURE__ */ new Set(), this._filterQuery = "";
  }
  ...
  _buildConflictsByCkey() {
    ...
    const t = Bt(e, this.resolvedSchema);
  ...
  _visibleFieldsFor(e, t) {
    ...
    if (i && e.ckey && ((r = i.get(e.ckey)) != null && r.hidden)) return [];
    let s = i ? e.fields.filter((o) => !St(o, e, i)) : e.fields;
    return t && !e.name.toLowerCase().includes(t) && (s = s.filter((o) => o.title.toLowerCase().includes(t))), s;
  }
  _renderFilter(e, t, i) {
    if (this.hideFilter) return m;
    if (!this.schema || this.schema.fields.length === 0) return m;
    const s = this._filterQuery, r = this._isAllCollapsed(t), o = r ? h("expandAll", "Expand all") : h("collapseAll", "Collapse all");
    return l`
      <div class="form-filter" role="search">
        <div class="filter-input-wrap">
          <span class="filter-icon" aria-hidden="true">${xi}</span>
          <input
            class="filter-input"
            type="text"
  ...
  _renderGroup(e, t, i, s, r) {
    const o = r ? !0 : !this._collapsed.has(e.uuid);
    return l`
      <div class="group">
  ...
  render() {
    if (!this.schema || this.schema.groups.length === 0)
      return l`<div class="empty" role="status" aria-live="polite">${h("noMetadataFieldsConfigured", "No metadata fields configured")}</div>`;
    const e = this._buildConflictsByCkey(), t = this._buildDependencyNames(), i = this._filterQuery.trim(), s = i.toLowerCase(), r = s !== "", o = [];
    for (const p of this.schema.groups) {
      const f = this._visibleFieldsFor(p, s);
      f.length !== 0 && o.push({ group: p, fields: f });
    }
    const a = o.map((p) => p.group.uuid), d = this.schema.groups.length > 1 && (r || o.length > 0), c = this._renderFilter(d, a, r);
    if (o.length === 0) {
      const p = r ? h("noFieldsMatch", 'No fields match "{{query}}"', { query: i }) : h("allMetadataFieldsHidden", "All metadata fields are currently hidden");
      return l`
        ${c}
    ...
    return l`
      ${c}
      ${o.map(
      ({ group: p, fields: f }) => this._renderGroup(p, f, e, t, r)
    )}
    `;
  }
};
...
F([
  u({ attribute: !1 })
], C.prototype, "taxonodes");
F([
  u({ attribute: !1 })
], C.prototype, "resolvedSchema");
F([
  u({ attribute: !1 })
...
], C.prototype, "hideFilter");
F([
  x()
], C.prototype, "_collapsed");
F([
  x()
], C.prototype, "_filterQuery");
...
`;
var wi = Object.defineProperty, A = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && wi(e, t, s), s;
};
const He = class He extends P {
  ...
  get _isRequired() {
    var e;
    return (e = this.resolvedState) != null && e.required ? !0 : Ct(this.field, this.config ?? void 0);
  }
  /**
  ...
  _conflictTooltip() {
    ...
    const t = (d) => {
      var c, p, f;
      return ((f = (p = (c = this.field) == null ? void 0 : c.possible_values) == null ? void 0 : p.find((g) => g.internal_unique_value === d)) == null ? void 0 : f.label) ?? d;
    }, i = (o = this.resolvedState) == null ? void 0 : o.allowedValues, s = (a = this.resolvedState) == null ? void 0 : a.setValue;
    let r;
    if (e.kind === "allow_values" && i ? r = `Current value is no longer allowed. Allowed: ${i.map(t).join(", ")}` : e.kind === "set_values" && s !== void 0 ? r = `Value should be: ${(Array.isArray(s) ? s : [s]).map(t).join(", ")}` : r = "Value conflicts with a dependency rule", e.dependencyUuids.length > 0 && this.dependencyNames.size > 0) {
...
Controlled by ${c}: ${d.join(", ")}`;
  ...
  _onFieldBlur(e) {
    const { key: t, value: i } = e.detail, s = Et(this.field, i, this.config ?? void 0);
    if (s) {
      this._error = s;
      return;
    }
    this._error = null;
    const r = { meta: { [this.field.key]: this.value } }, o = ee(this.field, this.config), a = Ke(this.field, i, r, o);
  ...
  _renderField(e, t) {
    ...
    const s = ((r = this.resolvedState) == null ? void 0 : r.allowedValues) ?? null;
    switch (e.type) {
      case "text":
      case "attachment-uri":
  ...
  render() {
    ...
    const e = this.field;
    if (!e) return m;
    const t = ee(e, this.config), i = Rt(e, this.value, t), s = Kt(
      e,
      this.regionalVariantsGroups,
      (d = this.config) == null ? void 0 : d.regionalFilters,
      (c = this.config) == null ? void 0 : c.language
    ), o = e.type === "textarea" ? "field-row field-row--top" : "field-row", a = this.conflict ? this._conflictTooltip() : "";
    return l`
      <div class=${o} aria-required=${this._isRequired ? "true" : "false"}>
        <div class="field-label" id="label-${e.key}">
          ...
          ${this._isRequired ? l`<span class="field-required" aria-hidden="true">*</span>` : m}
        ...
        <div class="field-content">
          ...
          ${s ? l`<div class="field-regional-hint" title=${s}>${s}</div>` : m}
          ${this._error ? l`<div class="field-error" id="error-${e.key}" role="alert">${this._error}</div>` : m}
        </div>
      </div>
    `;
  }
...
A([
  u({ attribute: !1 })
], k.prototype, "resolvedState");
A([
  u({ attribute: !1 })
], k.prototype, "conflict");
A([
  u({ attribute: !1 })
], k.prototype, "dependencyNames");
A([
  u({ type: Boolean })
...
class D extends P {
  constructor() {
    super(...arguments), this.value = "", this.disabled = !1, this.i18nController = new G(this);
  }
  /**
   ...
   */
  _selectPlaceholder(e) {
    var i;
    const t = ((i = this.field) == null ? void 0 : i.title) ?? "";
    return t ? h("selectFieldPlaceholder", "Select {{field}}", { field: t.toLowerCase() }) : e ?? h("selectAnOption", "Select an option");
  }
...
let Ae = Ye;
customElements.define("sfx-meta-textarea-field", Ae);
var $i = Object.defineProperty, ke = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && $i(e, t, s), s;
};
const Je = class Je extends D {
  ...
  get _filtered() {
    ...
    return this._selectableOptions.filter((t) => t.label.toLowerCase().includes(e));
  }
  get _selectedLabel() {
    var e;
  ...
  render() {
    var t;
    const e = this._selectPlaceholder();
    return l`
      <button class="trigger" ?disabled=${this.disabled}
        ...
        ${this._selectedLabel && !this.disabled ? l`
          <span class="trigger-clear" role="button" tabindex="0" aria-label=${h("clear", "Clear")}
            @click=${this._clear}
            @keydown=${(i) => {
      (i.key === "Enter" || i.key === " ") && (i.preventDefault(), this._clear(i));
    }}>&times;</span>
        ` : m}
        ...
        <div class="dropdown" role="listbox" @keydown=${this._onKeydown}>
          ...
          ${this._filtered.length ? this._filtered.map((i, s) => l`
                <div class="option ${i.value === this.value ? "selected" : ""} ${s === this._activeIndex ? "active" : ""}"
                  ...
                  @mousedown=${(r) => {
      r.preventDefault(), this._onSelect(i);
    }}
                  @mouseenter=${() => {
      this._activeIndex = s;
    }}>
...
ke([
  u({ attribute: !1 })
], te.prototype, "allowedValues");
ke([
  x()
], te.prototype, "_open");
ke([
  x()
], te.prototype, "_search");
ke([
  x()
], te.prototype, "_activeIndex");
customElements.define("sfx-meta-select-field", te);
var Si = Object.defineProperty, $e = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Si(e, t, s), s;
};
const Qe = class Qe extends D {
  constructor() {
    super(...arguments), this.allowedValues = null, this._open = !1, this._search = "", this._activeIndex = -1, this._boundOutsideClick = this._onOutsideClick.bind(this);
  }
  get _selected() {
  ...
  render() {
    var i;
    const e = this._selected, t = this._selectPlaceholder();
    return l`
      <div class="trigger"
        ...
        ${e.length ? e.map((s) => l`
              <span class="chip">
                ${this._labelFor(s)}
                <button class="chip-x" aria-label=${h("removeItem", "Remove {{item}}", { item: this._labelFor(s) })} @click=${(r) => {
      r.stopPropagation(), this._remove(s);
    }}>&times;</button>
          ...
          <div class="options-list">
            ${this._filtered.length ? this._filtered.map((s, r) => l`
                  <div class="option ${r === this._activeIndex ? "active" : ""}" role="option" aria-selected=${e.includes(s.value)}
                    @mousedown=${(o) => {
      o.preventDefault(), this._toggle(s);
    }}
                    @mouseenter=${() => {
      this._activeIndex = r;
    }}>
          ...
          ${this._options.length > 0 ? l`
            <div class="bulk-actions">
              <button type="button" class="bulk-btn" @mousedown=${(s) => {
      s.preventDefault(), this._selectAll();
    }}>${h("selectAll", "Select all")}</button>
              <button type="button" class="bulk-btn bulk-btn--muted" @mousedown=${(s) => {
      s.preventDefault(), this._clearAll();
    }}>${h("clearAll", "Clear all")}</button>
            </div>
          ` : m}
        </div>
      ` : m}
...
customElements.define("sfx-meta-multi-select-field", ie);
function X(n, e) {
  var t, i;
...
function Ci(n) {
  return Ft(n).replace(/\s/g, "-");
}
function be(n) {
  return { label: Ft(n), value: Ci(n) };
}
var Ei = Object.defineProperty, de = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ei(e, t, s), s;
};
const We = class We extends D {
  ...
  _onInput(e) {
    ...
    this._loading = !0, (r = this.autocomplete) == null || r.search(this.field.ckey, t, (o) => {
      this._results = o, this._loading = !1;
    });
  }
  _addTag(e) {
    if (this._tags.some((i) => X(i, e))) return;
  ...
  _removeTag(e) {
    const t = this._tags.filter((i) => !X(i, e));
    this.value = t, this._emit("field-change", t);
  }
  _onBlur() {
    ...
    if (this._activeIndex >= t && (this._activeIndex = Math.max(t - 1, -1)), !(t === 0 && e.key !== "Enter"))
      switch (e.key) {
        ...
        case "Enter": {
          ...
          this._activeIndex >= 0 && this._activeIndex < i.length ? this._addTag(i[this._activeIndex]) : this._activeIndex === i.length && this._canCreate ? this._addTag(be(this._query)) : this._activeIndex === -1 && this._canCreate ? this._addTag(be(this._query)) : this._activeIndex === -1 && i.length && this._addTag(i[0]);
          break;
        }
      }
  }
  get _suggestions() {
    var r;
    const e = this._query.toLowerCase().trim(), t = this._tags, i = (((r = this.field) == null ? void 0 : r.possible_values) ?? []).map((o) => ({ value: o.api_value || o.internal_unique_value, label: o.label })).filter((o) => !t.some((a) => X(a, o))).filter((o) => !e || o.label.toLowerCase().includes(e)), s = this._results.filter(
      (o) => !t.some((a) => X(a, o)) && !i.some((a) => X(a, o))
    );
    return [...i, ...s];
  }
  get _canCreate() {
    ...
    const t = be(e);
    return !this._tags.some((i) => X(i, t)) && !this._suggestions.some((i) => X(i, t));
  }
  render() {
    var s, r;
    ...
    return l`
      ...
      var o;
      return (o = this.renderRoot.querySelector(".input")) == null ? void 0 : o.focus();
    }}>
        ${e.map((o) => l`
          <span class="chip">
            ${o.label}
            <button class="chip-x" aria-label=${h("removeItem", "Remove {{item}}", { item: o.label })} @click=${(a) => {
      a.stopPropagation(), this._removeTag(o);
    }}>&times;</button>
          </span>`)}
      ...
      ${this._dropdownOpen && (this._query.trim() || t.length) ? l`
        <div class="dropdown" role="listbox">
          ${this._loading ? l`<div class="loading">${h("loading", "Loading")}</div>` : m}
          ${t.map((o, a) => l`
            <div class="option ${a === this._activeIndex ? "active" : ""}" role="option"
              @mousedown=${(d) => {
      d.preventDefault(), this._addTag(o);
    }}
              @mouseenter=${() => {
      this._activeIndex = a;
    }}>
          ...
          ${this._canCreate ? l`
            <div class="option create ${i === this._activeIndex ? "active" : ""}"
              @mousedown=${(o) => {
      o.preventDefault(), this._addTag(be(this._query));
    }}
              @mouseenter=${() => {
      this._activeIndex = i;
    }}>
          ...
          ${!this._loading && !t.length && !this._canCreate ? l`<div class="empty">${h("noResults", "No results")}</div>` : m}
        </div>
      ` : m}
    `;
  }
...
de([
  u({ attribute: !1 })
], U.prototype, "autocomplete");
de([
  x()
], U.prototype, "_query");
de([
  x()
], U.prototype, "_results");
de([
  x()
], U.prototype, "_loading");
de([
  x()
], U.prototype, "_dropdownOpen");
de([
  x()
], U.prototype, "_activeIndex");
customElements.define("sfx-meta-tags-field", U);
var Ii = Object.defineProperty, Dt = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ii(e, t, s), s;
};
const Z = () => [
  { label: h("booleanTrue", "True"), value: "true" },
  { label: h("booleanFalse", "False"), value: "false" }
  ...
  get _currentLabel() {
    ...
    return ((t = Z().find((i) => i.value === e)) == null ? void 0 : t.label) ?? "";
  }
  disconnectedCallback() {
    super.disconnectedCallback(), document.removeEventListener("mousedown", this._boundOutsideClick);
  }
  _openDropdown() {
    ...
    this._activeIndex = Math.max(Z().findIndex((t) => t.value === e), 0), document.addEventListener("mousedown", this._boundOutsideClick), this.updateComplete.then(() => {
      var t;
  ...
  _onKeydown(e) {
    ...
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault(), this._activeIndex = Math.min(this._activeIndex + 1, Z().length - 1), this._scrollActive();
        break;
      case "ArrowUp":
        e.preventDefault(), this._activeIndex = Math.max(this._activeIndex - 1, 0), this._scrollActive();
        break;
      case "Home":
        ...
        break;
      case "End":
        e.preventDefault(), this._activeIndex = Z().length - 1, this._scrollActive();
        break;
      case "Enter":
      case " ":
        this._activeIndex >= 0 && this._activeIndex < Z().length && (e.preventDefault(), this._onSelect(Z()[this._activeIndex], !0));
        break;
    }
  }
  render() {
    var i;
    const e = this.value == null ? "" : String(this.value), t = this._selectPlaceholder();
    return l`
      <button class="trigger" ?disabled=${this.disabled}
        role="combobox" aria-expanded=${this._open} aria-haspopup="listbox"
        @click=${() => this._open ? this._closeAndSubmit(!0) : this._openDropdown()}
        ...
        ${this._currentLabel && !this.disabled ? l`
          <span class="trigger-clear" role="button" tabindex="0" aria-label=${h("clear", "Clear")}
            @click=${this._clear}
            @keydown=${(s) => {
      (s.key === "Enter" || s.key === " ") && (s.preventDefault(), this._clear(s));
    }}>&times;</span>
        ` : m}
        <span class="trigger-chevron ${this._open ? "open" : ""}" aria-hidden="true">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="dropdown" role="listbox" tabindex="-1" @keydown=${this._onKeydown}>
          ${Z().map((s, r) => l`
            <div class="option ${s.value === e ? "selected" : ""} ${r === this._activeIndex ? "active" : ""}"
              ...
              @mousedown=${(o) => {
      o.preventDefault(), this._onSelect(s);
    }}
              @mouseenter=${() => {
      this._activeIndex = r;
    }}>
...
Dt([
  x()
], he.prototype, "_activeIndex");
customElements.define("sfx-meta-boolean-field", he);
...
Ze.styles = [fe];
let Oe = Ze;
customElements.define("sfx-meta-number-field", Oe);
    ...
    return l`
      <div class="date-wrap">
        <input
          ...
          @keydown=${this._onKeydown}
        />
        ${t ? l`<span class="date-placeholder">${h("pickADate", "Pick a date")}</span>` : m}
...
let Fe = tt;
customElements.define("sfx-meta-geo-point-field", Fe);
var Ti = Object.defineProperty, M = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ti(e, t, s), s;
};
const oe = { uuid: "__root__", name: "", ltree: "" }, it = class it extends D {
  constructor() {
    super(...arguments), this.entry = null, this._open = !1, this._query = "", this._drillStack = [oe], this._currentNodes = [], this._searchResults = [], this._loading = !1, this._activeIndex = -1, this._resolvedTaxonomyUuid = null, this._taxonomyResolutionFailed = !1, this._boundOutsideClick = this._onOutsideClick.bind(this), this._searchSeq = 0;
  }
  get _taxonomySuid() {
    var t, i;
  ...
  async _resolveTaxonomyUuid() {
    ...
    return i ? (this._resolvedTaxonomyUuid = i.uuid, this._taxonomyResolutionFailed = !1, i.uuid) : (console.warn(
      `[sfx-uploader] taxonomy '${e}' not found in catalogue. Field "${((s = this.field) == null ? void 0 : s.ckey) ?? ((r = this.field) == null ? void 0 : r.key)}" model:`,
      (o = this.field) == null ? void 0 : o.model,
  ...
  _seedDrillStackFromEntry() {
    const e = this.entry;
    if (!(e != null && e.lineage)) return [oe];
    const t = e.lineage.split(".").filter(Boolean);
    if (t.length <= 1) return [oe];
    const i = t.slice(0, -1), r = (e.path ? e.path.split(/\s*[›>]\s*/).filter(Boolean) : []).slice(0, -1), o = [oe];
    let a = "";
    for (let d = 0; d < i.length; d++)
      a = a ? `${a}.${i[d]}` : i[d], o.push({
        uuid: `__seed_${a}`,
        name: r[d] ?? i[d],
        ltree: a
      });
  ...
  async _loadCurrentNodes() {
    ...
    const r = this._selectedScalar, o = r ? this._currentNodes.findIndex(
      (a) => a.uuid === r || a.slug === r
    ) : -1;
    this._activeIndex = o, o >= 0 && this._scrollActive();
  }
  _onSearchInput(e) {
    var s;
    const t = e.target.value;
    ...
    this.taxonomyService.autocomplete(this.field.ckey, t, (r) => {
      i === this._searchSeq && (this._searchResults = r, this._loading = !1);
    });
  }
  async _drillInto(e) {
    this._drillStack = [
  ...
  _buildTreeEntry(e) {
    const i = [...this._drillStack.filter((s) => s.uuid !== oe.uuid).map((s) => s.name), e.name].filter(Boolean).join(" › ");
    return {
      uuid: e.uuid,
      suid: e.slug,
      name: e.name,
  ...
  _onKeydown(e) {
    ...
    if (e.key === "ArrowRight" && !this._isSearchMode) {
      if (this._activeIndex >= 0 && this._activeIndex < this._currentNodes.length) {
        const r = this._currentNodes[this._activeIndex];
        r.children.count_direct > 0 && (e.preventDefault(), this._drillInto(r));
      }
      return;
    }
    ...
    if (e.key === "Enter") {
      ...
      if (e.preventDefault(), this._isSearchMode) {
        const r = this._searchResults[this._activeIndex];
        r && this._selectAutocomplete(r);
      } else {
        const r = this._currentNodes[this._activeIndex];
        r && this._selectTreeNode(r);
      }
    }
  }
  _renderBreadcrumb() {
    const e = this._drillStack;
    return e.length <= 1 ? m : l`
      <div class="breadcrumb">
        ${e.map((t, i) => {
      const s = i === e.length - 1, r = t.uuid === oe.uuid ? h("rootNode", "Root") : t.name;
      return l`
            ${i > 0 ? l`<span class="crumb-sep">›</span>` : m}
  ...
  _renderTree() {
    if (this._loading && this._currentNodes.length === 0)
      return l`<div class="empty">${h("loading", "Loading")}</div>`;
    if (this._taxonomyResolutionFailed)
      return l`<div class="empty">${h("taxonomyNotFound", "Taxonomy not found")}</div>`;
    if (this._currentNodes.length === 0)
      return l`<div class="empty">${h("noNodes", "No nodes")}</div>`;
    const e = this._selectedScalar;
    return l`
      <div class="scroll" role="listbox">
        ${this._currentNodes.map((t, i) => {
      const s = t.children.count_direct > 0, r = !!e && (e === t.uuid || e === t.slug);
      return l`
        ...
        this._activeIndex = i;
              ...
              <span class="tree-radio ${r ? "checked" : ""}" role="button"
                ...
                @click=${(o) => {
        o.stopPropagation(), this._selectTreeNode(t);
      }}></span>
              <span class="tree-name" title=${t.name}>${t.name}</span>
              ${s ? l`<span class="tree-count" aria-hidden="true">(${t.children.count_direct})</span>` : m}
  ...
  _renderSearch() {
    return this._loading && this._searchResults.length === 0 ? l`<div class="empty">${h("loading", "Loading")}</div>` : this._searchResults.length === 0 ? l`<div class="empty">${h("noResults", "No results")}</div>` : l`
      <div class="scroll" role="listbox">
      ...
      this._activeIndex = t;
            ...
            ${e.path && e.path !== e.tag ? l`<span class="ac-path">${e.path}</span>` : m}
          </div>
        `)}
      </div>
    `;
  ...
  render() {
    ...
    if (!this._taxonomySuid)
      return l`<div class="misconfigured" role="alert">${h("missingTaxonomyConfig", "Field is missing taxonomy config")}</div>`;
    const e = this._selectPlaceholder(h("selectANode", "Select a node")), t = this._displayPath, i = !!t;
    return l`
      <button class="trigger" type="button"
        ...
        @keydown=${(r) => {
      !this._open && (r.key === "ArrowDown" || r.key === "Enter" || r.key === " ") && (r.preventDefault(), this._openDropdown());
    }}>
...
M([
  x()
], O.prototype, "_drillStack");
M([
  x()
], O.prototype, "_currentNodes");
M([
  x()
], O.prototype, "_searchResults");
M([
  x()
...
M([
  x()
], O.prototype, "_resolvedTaxonomyUuid");
M([
  x()
], O.prototype, "_taxonomyResolutionFailed");
customElements.define("sfx-meta-taxonomy-node-field", O);
var Ai = Object.defineProperty, Y = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ai(e, t, s), s;
};
const Oi = (n, e) => !!(e.uuid && n.uuid === e.uuid || e.sid && n.sid === e.sid || e.slug && n.slug === e.slug), st = class st extends D {
  constructor() {
  ...
  get _items() {
    return Array.isArray(this.value) ? this.value.map((e) => typeof e == "string" ? di(e) ? { sid: e } : { slug: e } : e) : [];
  }
  get _currentLang() {
  ...
  _labelForItem(e) {
    ...
    return ye(t, this._currentLang, this._defaultLang).value || e.slug || e.sid || "";
  }
  ...
  get _dropdownOptions() {
    const e = this._selectedKeys(), t = (r) => !!r.uuid && e.has(r.uuid) || !!r.sid && e.has(r.sid) || e.has(r.slug), s = (this._isRestricted ? this._restrictedEntries : this._results).filter((r) => !t(r)).map((r) => ({
      entry: r,
      label: ye(r, this._currentLang, this._defaultLang).value || r.slug
    }));
    if (this._isRestricted) {
      const r = this._query.trim().toLowerCase();
      return r ? s.filter((o) => o.label.toLowerCase().includes(r)) : s;
    }
    return s;
  }
  get _isSearching() {
    return this._query.trim().length >= gt;
  }
  get _canCreate() {
    if (this._isRestricted || !this._isSearching || this._loading) return !1;
    const e = this._query.trim(), t = xt(e);
  ...
  _onInput(e) {
    ...
    if (!this._isSearching || !((r = this.field) != null && r.key)) {
      this._results = [], this._loading = !1, (o = this.ultratags) == null || o.cancel();
      return;
    }
  ...
  async _createFromQuery() {
    ...
    if (!t || !((r = this.field) != null && r.key)) return;
    const i = xt(e);
    if (!i) return;
    ...
    try {
      ...
      }), d = (o = a == null ? void 0 : a.output) == null ? void 0 : o[0], c = {
      ...
      this.value = p, this._query = "", this._results = [], this._dropdownOpen = !1, this._activeIndex = -1, this._emit("field-change", p), this.updateComplete.then(() => {
        var f;
        (f = this.renderRoot.querySelector(".input")) == null || f.focus();
      });
    } catch {
  ...
  _removeItem(e) {
    const t = this._items.filter((i) => !Oi(i, e));
    this.value = t, this._emit("field-change", t);
  }
  _onBlur() {
    this._blurTimeout && clearTimeout(this._blurTimeout), this._blurTimeout = setTimeout(() => {
  ...
  render() {
    var o, a;
    const e = this._items, t = this._dropdownOptions, i = t.length, s = this._query.trim(), r = ((o = this.field) == null ? void 0 : o.placeholder) || (this._isRestricted ? h("searchTagsToRemove", "Search tags to remove") : h("addCustomTags", "Add custom tags"));
    return l`
      <div class="container" @click=${() => {
      var d;
      ...
      ${this._dropdownOpen ? l`
        <div class="dropdown" role="listbox">
          ${!this._isRestricted && !this._isSearching ? l`<div class="hint">${h("typeAtLeastNChars", "Type at least {{count}} characters to search.", { count: gt })}</div>` : m}
          ${!this._isRestricted && this._isSearching && this._loading ? l`<div class="loading">${h("loading", "Loading")}</div>` : m}
      ...
      this._activeIndex = c;
          ...
          ${(this._isRestricted || this._isSearching && !this._loading) && t.length === 0 && !this._canCreate ? l`<div class="empty">${h("noResults", "No results")}</div>` : m}
          ${this._canCreate ? l`
              <div class="option create ${i === this._activeIndex ? "active" : ""}"
                @mousedown=${(d) => {
...
Y([
  u({ attribute: !1 })
], B.prototype, "ultratags");
Y([
  u({ attribute: !1 })
], B.prototype, "language");
Y([
  u({ attribute: !1 })
], B.prototype, "defaultLanguage");
Y([
  u({ attribute: !1 })
], B.prototype, "restrictToItems");
Y([
  x()
], B.prototype, "_query");
Y([
  x()
], B.prototype, "_results");
Y([
  x()
], B.prototype, "_loading");
Y([
  x()
], B.prototype, "_dropdownOpen");
Y([
  x()
], B.prototype, "_activeIndex");
customElements.define("sfx-meta-ultratags-field", B);
const Ve = () => h("unsupportedFieldMessage", "This field is not supported during upload. You can edit it later in the asset library."), Lt = l`
  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    ${S`<rect x="3" y="7" width="10" height="7" rx="1.5"/><path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2"/>`}
  </svg>
`, rt = class rt extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this);
  }
  render() {
    const e = Ve();
    return l`
...
rt.styles = _`
  ...
  `;
let De = rt;
customElements.define("sfx-meta-unsupported-field", De);
var Ri = Object.defineProperty, j = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ri(e, t, s), s;
};
...
var Fi = Object.defineProperty, me = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Fi(e, t, s), s;
};
const nt = class nt extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.taxonomyEntry = null;
  }
  _formatValue() {
    var i, s, r, o;
    ...
    switch (t) {
      ...
      case "ultratags": {
        ...
        return e.map((c) => typeof c == "string" ? c : ye(
      ...
      case "taxonomy-node":
        return (r = this.taxonomyEntry) != null && r.path ? this.taxonomyEntry.path : (o = this.taxonomyEntry) != null && o.name ? this.taxonomyEntry.name : e == null || e === "" ? "" : String(e);
      case "geopoint": {
        const a = e;
...
customElements.define("sfx-metadata-field-view", Q);
var Di = Object.defineProperty, Se = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Di(e, t, s), s;
};
const at = class at extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.groups = [], this.selectedFilters = {}, this._open = !1, this._activeIndex = -1, this._boundOutsideClick = this._onOutsideClick.bind(this);
  }
  /**
  ...
  _triggerSummary(e) {
    var s;
    const t = this._filteredGroups;
    if (t.length === 0) return e;
    const i = [];
    for (const r of t) {
      const o = this.selectedFilters[r.uuid] ?? ((s = r.variants[0]) == null ? void 0 : s.api_value), a = r.variants.find((d) => d.api_value === o);
      a && i.push(t.length === 1 ? a.label : `${r.label}: ${a.label}`);
    }
    return i.length ? i.join(", ") : e;
  }
  render() {
    if (this._filteredGroups.length === 0) return m;
    const t = this._options, i = h("regionalSettings", "Regional settings"), s = this._triggerSummary(i);
    return l`
      <button
        class="trigger"
        type="button"
      ...
      ${this._open ? l`
            <div class="dropdown" role="listbox" aria-label=${i}>
              ${t.map(
      (r, o) => this._renderOption(r, o, r.value === this.selectedFilters[r.groupUuid])
    )}
            </div>
          ` : m}
    `;
  ...
  _renderOption(e, t, i) {
    ...
    return l`
      ${e.isGroupStart ? l`<div class="group-header">${e.groupLabel}</div>` : m}
      <div
...
customElements.define("sfx-regional-settings", se);
...
function Li(n) {
  return Nt(n) ? [] : Ce.has(n) ? [
    { key: "SET", label: h("bulkOpSet", "Set") },
    { key: "ADD", label: h("bulkOpAddTo", "Add to") },
    { key: "DELETE", label: h("bulkOpRemoveFrom", "Remove from") }
  ] : Ee.has(n) ? [
    { key: "SET", label: h("bulkOpSet", "Set") },
    { key: "ADD", label: h("bulkOpAppend", "Append") },
    { key: "DELETE", label: h("bulkOpRemove", "Remove") }
  ] : [
    { key: "SET", label: h("bulkOpSet", "Set") },
    { key: "DELETE", label: h("bulkOpClear", "Clear") }
  ];
}
function Le(n, e) {
  return n === "DELETE" ? Ce.has(e) || Ee.has(e) : !0;
}
function Pi(n, e, t, i) {
  const s = Ce.has(i), r = Ee.has(i);
  switch (n) {
    case "SET":
      return t;
    case "ADD": {
      if (s) {
        const o = Array.isArray(e) ? e : [], a = Array.isArray(t) ? t : [];
        ...
        if (i === "tags") {
          const p = new Set(o.map((g) => g)), f = [...o];
          for (const g of a) {
            const v = typeof g == "string" ? g : String(g);
            p.has(v) || (p.add(v), f.push(v));
          }
          return f;
        }
        const d = new Set(o.map((p) => JSON.stringify(p))), c = [...o];
        for (const p of a) {
          const f = JSON.stringify(p);
          d.has(f) || (d.add(f), c.push(p));
        }
        return c;
      }
      if (r) {
    ...
    case "DELETE": {
      ...
      if (r) {
        const o = typeof t == "string" ? t : "";
        return o ? (typeof e == "string" ? e : "").replaceAll(o, "").replace(/\s{2,}/g, " ").trim() : "";
      }
...
function Pt(n, e, t, i, s) {
  const r = s ?? "en", o = !!n.regional_variants_group_uuid, a = {
    meta: { [n.key]: e }
  }, d = Ke(n, t, a, s), c = (v) => o && v !== null && typeof v == "object" && !Array.isArray(v), p = c(e) ? e[r] : e, f = c(d) ? d[r] : d, g = Pi(i, p, f, n.type);
  return o ? {
    ...c(e) ? e : {},
    [r]: g
  } : g;
...
`;
var ji = Object.defineProperty, $ = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && ji(e, t, s), s;
};
const lt = class lt extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.files = [], this.config = null, this.initialFieldKey = null, this.dependencies = [], this._activeFieldKey = "", this._staged = /* @__PURE__ */ new Map(), this._stagedTaxonodes = /* @__PURE__ */ new Map(), this._selected = /* @__PURE__ */ new Set(), this._sortAsc = !0, this._pendingOp = null, this._confirmVisible = !1, this._missingRequiredFieldKey = null, this._missingRequiredKeys = /* @__PURE__ */ new Set(), this._confirmResolve = null, this._cachedBulkResolved = /* @__PURE__ */ new Map(), this._cachedPerFileResolved = /* @__PURE__ */ new Map(), this._originalFiles = /* @__PURE__ */ new Map(), this._onKeyDown = async (e) => {
      ...
      const s = i[0], r = i[i.length - 1], o = (d = this.shadowRoot) == null ? void 0 : d.activeElement;
      e.shiftKey && o === s ? (e.preventDefault(), r.focus()) : !e.shiftKey && o === r && (e.preventDefault(), s.focus());
    }, this._onPendingChange = (e) => {
      const { operation: t, value: i } = e.detail, s = this._activeField;
      ne(i) && (!s || Le(t, s.type)) ? this._pendingOp = null : this._pendingOp = { operation: t, value: i };
      ...
      if (!t) return;
      const { operation: i, value: s, taxonomyEntry: r } = e.detail, o = ee(t, this.config), a = [];
      for (const d of this._selected) {
        const c = this._staged.get(d), p = c != null && c.has(t.key) ? c.get(t.key) : this._originalValue(d, t.key) ?? null, f = Pt(
          t,
          p,
          s,
          i,
      ...
      this._setStagedBulk(a), t.type === "taxonomy-node" && r !== void 0 && this._setStagedTaxonodeBulk(this._selected, t.key, r);
      ...
      const e = [], t = [];
      for (const [s, r] of this._staged) {
        if (!this._originalFiles.get(s)) continue;
        const a = {}, d = {};
        for (const [c, p] of r) {
          const f = this._originalValue(s, c);
          if (JSON.stringify(p) !== JSON.stringify(f))
            if (ht(c)) {
              const g = ft(c);
              if (!g) continue;
              const v = p === "" || p == null;
              g === "position" ? d.position = v ? void 0 : Number(p) : d.ref = v ? void 0 : String(p);
            } else
              a[c] = p;
        }
      ...
      const i = [];
      for (const [s, r] of this._stagedTaxonodes) {
        const o = this._originalFiles.get(s);
        if (!o) continue;
        const a = o.taxonodes ?? {}, d = {};
        for (const [c, p] of r) {
          const f = a[c] ?? null;
          JSON.stringify(p ?? null) !== JSON.stringify(f ?? null) && (d[c] = p ?? null);
        }
  ...
  _initStaged() {
    var a, d;
    const e = /* @__PURE__ */ new Map(), t = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Set(), s = /* @__PURE__ */ new Map(), r = ((a = this.schema) == null ? void 0 : a.productsEnabled) === !0;
    for (const c of this.files) {
      const p = /* @__PURE__ */ new Map();
      if (c.meta)
        for (const [f, g] of Object.entries(c.meta))
          p.set(f, g);
      if (r) {
        const f = c.product;
        f.ref !== void 0 && p.set(Mt, f.ref), f.position !== void 0 && p.set(jt, f.position);
      }
    ...
    o && ((d = this.schema) != null && d.fieldsByKey.has(o)) ? this._activeFieldKey = o : this.schema && this.schema.fields.length > 0 && (this._activeFieldKey = this.schema.fields[0].key);
  }
  // -----------------------------------------------------------------------
  // Immutability helpers
  // -----------------------------------------------------------------------
  _setStagedValue(e, t, i) {
    const s = new Map(this._staged), r = new Map(s.get(e) ?? /* @__PURE__ */ new Map());
    r.set(t, i), s.set(e, r), this._staged = s;
  }
  _setStagedBulk(e) {
    const t = new Map(this._staged);
    for (const [i, s, r] of e) {
      const o = new Map(t.get(i) ?? /* @__PURE__ */ new Map());
      o.set(s, r), t.set(i, o);
    }
    this._staged = t;
  }
  _setStagedTaxonodeBulk(e, t, i) {
    ...
    for (const r of e) {
      const o = new Map(s.get(r) ?? /* @__PURE__ */ new Map());
      o.set(t, i), s.set(r, o);
    }
    this._stagedTaxonodes = s;
  }
  _setStagedTaxonodeSingle(e, t, i) {
    const s = new Map(this._stagedTaxonodes), r = new Map(s.get(e) ?? /* @__PURE__ */ new Map());
    r.set(t, i), s.set(e, r), this._stagedTaxonodes = s;
  }
  // -----------------------------------------------------------------------
  ...
  get _ultratagsPresentOnSelection() {
    ...
    let t = [];
    for (const r of this._selected) {
      const o = this._originalFiles.get(r), a = (i = this._staged.get(r)) == null ? void 0 : i.get(e.key), d = a !== void 0 ? a : (s = o == null ? void 0 : o.meta) == null ? void 0 : s[e.key], c = _e(d);
      c.length && (t = Ie(t, c, !1));
    }
    return t.filter((r) => typeof r != "string");
  }
  /**
  ...
  _originalValue(e, t) {
    ...
    if (i) {
      if (ht(t)) {
        const o = ft(t);
        return o ? (s = i.product) == null ? void 0 : s[o] : void 0;
      }
      return (r = i.meta) == null ? void 0 : r[t];
    }
  }
  /**
  ...
  _refreshMissingRequired() {
    const t = !!this.schema && !!this.config && Vt(this.schema, this.config) ? Ut(
      this._staged,
      this._originalFiles,
      this.schema,
      this.config ?? void 0,
    ...
    i !== this._missingRequiredFieldKey && (this._missingRequiredFieldKey = i);
    const s = this._missingRequiredKeys;
    let r = s.size !== t.size;
    if (!r) {
      for (const o of t)
        if (!s.has(o)) {
  ...
  _advanceActiveFieldIfHidden() {
    ...
    const t = this.schema.fields.find(
      (r) => {
        var o;
        return !((o = this._cachedBulkResolved.get(r.ckey)) != null && o.hidden);
      }
    );
  ...
  get _filledFields() {
    ...
    for (const i of ((t = this.schema) == null ? void 0 : t.fields) ?? [])
      for (const [s, r] of this._staged) {
        const o = r.get(i.key), a = this._originalValue(s, i.key);
        if (o !== void 0 && !ne(o) && JSON.stringify(o) !== JSON.stringify(a)) {
          e.add(i.key);
          break;
        }
      }
  ...
  get _hasPendingValue() {
    return this._pendingOp != null && !ne(this._pendingOp.value);
  }
  ...
  render() {
    var v, y, E, b, re;
    if (!((y = (v = this.schema) == null ? void 0 : v.fields) != null && y.length))
      return l`
...
$([
  u({ attribute: !1 })
], w.prototype, "initialFieldKey");
$([
  u({ attribute: !1 })
], w.prototype, "dependencies");
$([
  x()
], w.prototype, "_activeFieldKey");
$([
  x()
], w.prototype, "_staged");
$([
  x()
], w.prototype, "_stagedTaxonodes");
$([
  x()
], w.prototype, "_selected");
$([
  x()
], w.prototype, "_sortAsc");
$([
  x()
], w.prototype, "_pendingOp");
$([
  x()
], w.prototype, "_confirmVisible");
$([
  x()
], w.prototype, "_missingRequiredFieldKey");
$([
  x()
], w.prototype, "_missingRequiredKeys");
customElements.define("sfx-bulk-metadata-modal", w);
const vt = {
...
var Ui = Object.defineProperty, W = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ui(e, t, s), s;
};
const dt = class dt extends P {
  ...
  _isRequired(e) {
    var t, i;
    return (i = (t = this.bulkResolvedSchema) == null ? void 0 : t.get(e.ckey)) != null && i.required ? !0 : Ct(e, this.config ?? void 0);
  }
  _toggleGroup(e) {
    const t = new Set(this._collapsed);
    t.has(e) ? t.delete(e) : t.add(e), this._collapsed = t;
  ...
  render() {
    if (!this.schema) return m;
    const e = this.bulkResolvedSchema;
    return l`
      ${this.schema.groups.map((t) => {
      var r;
      const i = this._isNarrow || !this._collapsed.has(t.uuid);
      if (e && t.ckey && ((r = e.get(t.ckey)) != null && r.hidden))
        return m;
      const s = e ? t.fields.filter((o) => !St(o, t, e)) : t.fields;
      return s.length === 0 ? m : l`
          <button
            class="group-label"
        ...
        (o) => l`
                  ...
                  >
                    ...
                    ${this.filledFields.has(o.key) ? l`<span class="field-dot"></span>` : m}
                    ${this._isRequired(o) ? l`<span
                          class=${It({
          "field-required": !0,
          unmet: this.missingRequiredKeys.has(o.key)
        })}
                          aria-hidden="true"
                        >*</span>` : m}
                  </button>
...
W([
  u({ attribute: !1 })
], N.prototype, "activeFieldKey");
W([
  u({ attribute: !1 })
], N.prototype, "filledFields");
W([
  u({ attribute: !1 })
], N.prototype, "missingRequiredKeys");
W([
  u({ attribute: !1 })
], N.prototype, "config");
W([
  u({ attribute: !1 })
], N.prototype, "bulkResolvedSchema");
W([
  x()
], N.prototype, "_collapsed");
W([
  x()
], N.prototype, "_isNarrow");
customElements.define("sfx-bulk-meta-sidebar", N);
var Hi = Object.defineProperty, z = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Hi(e, t, s), s;
}, V;
const L = (V = class extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.ultratagsPresentOnSelection = [], this.config = null, this.selectedCount = 0, this.allowedValues = null, this._operation = "SET", this._value = void 0, this._pendingTaxonode = null, this._opDropdownOpen = !1, this._onTaxonomyEntryChange = (e) => {
      ...
      const t = (r = this.field) == null ? void 0 : r.type;
      if (!t || !V._ENTER_APPLY_TYPES.has(t)) return;
      const i = e.composedPath().find((o) => o instanceof HTMLElement);
      if ((i == null ? void 0 : i.tagName) === "TEXTAREA") return;
      e.preventDefault();
      const s = e.composedPath().find((o) => o instanceof HTMLInputElement);
  ...
  get _availableOps() {
    return this.field ? Li(this.field.type) : [];
  }
  ...
  willUpdate(e) {
    e.has("field") && this.field && (this._operation = "SET", this._value = void 0, this._pendingTaxonode = null, this._emitPendingChange());
  }
  _onOpSelect(e) {
  ...
  get _isApplyDisabled() {
    var e, t;
    return this.selectedCount === 0 ? !0 : this._operation === "DELETE" ? Ce.has((e = this.field) == null ? void 0 : e.type) ? ne(this._value) : Ee.has((t = this.field) == null ? void 0 : t.type) ? ne(this._value) : !1 : ne(this._value);
  }
  render() {
    var s;
    if (!this.field) return m;
    if (we(this.field)) {
      const r = Ve();
      return l`
        <div class="op-bar">
    ...
    const e = this._availableOps, t = e.length > 1, i = e.find((r) => r.key === this._operation);
    return l`
      <div class="op-bar">
...
}, V.styles = [Bi], V._ENTER_APPLY_TYPES = /* @__PURE__ */ new Set([
  "text",
  "numeric",
  "decimal2",
  "date",
...
let Gi = L;
customElements.define("sfx-bulk-meta-op-bar", Gi);
var Yi = Object.defineProperty, K = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Yi(e, t, s), s;
};
...
K([
  u({ attribute: !1 })
], T.prototype, "stagedTaxonodes");
K([
  u({ attribute: !1 })
], T.prototype, "selected");
K([
  u({ attribute: !1 })
], T.prototype, "pendingOp");
K([
  u({ attribute: !1 })
], T.prototype, "config");
K([
...
K([
  u({ attribute: !1 })
], T.prototype, "perFileResolved");
customElements.define("sfx-bulk-meta-table", T);
var Ji = Object.defineProperty, q = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && Ji(e, t, s), s;
};
const ut = class ut extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.taxonomyEntry = null, this.selected = !1, this.pendingOp = null, this.config = null, this.allowedValues = null, this._error = null, this._onFieldBlur = (e) => {
      e.stopPropagation();
      const { value: t } = e.detail, i = Et(this.field, t, this.config ?? void 0);
      if (i) {
        this._error = i;
        return;
      }
      ...
      const s = {
        ...
        t,
        s,
        ee(this.field, this.config)
      );
      JSON.stringify(r) !== JSON.stringify(this.value) && this.dispatchEvent(
        new CustomEvent("row-field-change", {
          detail: { fileId: this.file.id, value: r },
          bubbles: !0,
          composed: !0
        })
      );
  ...
  _computePreviewValue() {
    ...
    return !e || !this.field ? this.value : Pt(
      this.field,
      this.value,
      e.value,
      e.operation,
    ...
    return l`
      <div class="row">
        ...
        ${e.previewUrl ? l`<img class="row-thumb" src=${e.previewUrl} alt="" />` : l`<img class="row-thumb row-thumb-fallback"
              ...
              @error=${(s) => {
      const r = s.target, o = Gt();
      !r.dataset.fallback && r.src !== o && (r.dataset.fallback = "1", r.src = o);
    }}
            />`}
        ...
        >
          ${this.pendingOp && this.selected ? l`<sfx-bulk-meta-diff-view
              ...
              ${this._error ? l`<div class="row-error" role="alert">${this._error}</div>` : m}`}
        </div>
      </div>
    `;
  }
...
q([
  u({ attribute: !1 })
], I.prototype, "allowedValues");
q([
  x()
], I.prototype, "_error");
customElements.define("sfx-bulk-meta-row", I);
const Qi = /* @__PURE__ */ new Set(["multi-select", "tags", "ultratags"]);
function yt(n, e, t) {
...
function Wi(n, e) {
  const t = n.map((a) => typeof a == "string" ? a : String(a)), i = e.map((a) => typeof a == "string" ? a : String(a)), s = new Set(t), r = new Set(i), o = [];
  for (const a of i)
    o.push({ label: a, state: s.has(a) ? "kept" : "added" });
  for (const a of t)
    r.has(a) || o.push({ label: a, state: "removed" });
  return o;
}
function Xi(n, e, t) {
  const i = _e(n), s = _e(e), r = t || "en", o = (f) => f.sid || f.slug || f.uuid || "", a = (f) => ye(
    { i18n: f.i18n, slug: f.slug || "" },
    r,
    r
  ).value || f.slug || f.sid || "", d = new Set(i.map(o).filter(Boolean)), c = new Set(s.map(o).filter(Boolean)), p = [];
  for (const f of s) {
    const g = o(f);
    p.push({ label: a(f), state: d.has(g) ? "kept" : "added" });
  }
  for (const f of i) {
    const g = o(f);
    c.has(g) || p.push({ label: a(f), state: "removed" });
  }
  return p;
}
function Zi(n, e, t) {
  const i = new Set(n.map((o) => JSON.stringify(o))), s = new Set(e.map((o) => JSON.stringify(o))), r = [];
  for (const o of e) {
    const a = JSON.stringify(o), d = typeof o == "string" ? Pe(t, o) : String(o);
    r.push({ label: d, state: i.has(a) ? "kept" : "added" });
  }
  for (const o of n) {
    const a = JSON.stringify(o);
    if (!s.has(a)) {
      const d = typeof o == "string" ? Pe(t, o) : String(o);
      r.push({ label: d, state: "removed" });
    }
  }
  return r;
}
function es(n, e, t, i) {
  const s = ee(n, i), r = i == null ? void 0 : i.language, o = yt(e, n, s), a = yt(t, n, s);
  if (Qi.has(n.type)) {
    if (n.type === "ultratags")
      return {
        kind: "array",
    ...
    const d = _t(o), c = _t(a);
    return n.type === "tags" ? { kind: "array", items: Wi(d, c) } : { kind: "array", items: Zi(d, c, n) };
  }
  return {
    kind: "scalar",
    oldDisplay: kt(n, o),
...
var ts = Object.defineProperty, ce = (n, e, t, i) => {
  for (var s = void 0, r = n.length - 1, o; r >= 0; r--)
    (o = n[r]) && (s = o(e, t, s) || s);
  return s && ts(e, t, s), s;
};
const pt = class pt extends P {
  constructor() {
    super(...arguments), this._i18nController = new G(this), this.oldTaxonomyEntry = null, this.newTaxonomyEntry = null, this.config = null;
  }
  _renderArrayDiff(e) {
    const t = {
      added: h("added", "Added"),
      removed: h("removed", "Removed"),
      kept: h("kept", "Kept")
    };
    return l`
  ...
  _renderScalarDiff(e) {
    const t = h("willChangeFromTo", "Will change from {{from}} to {{to}}", {
      from: e.oldEmpty ? h("emptyValue", "empty") : e.oldDisplay,
      to: e.newEmpty ? h("emptyValue", "empty") : e.newDisplay
    });
    return l`
      <div class="diff-wrap diff-scalar-text" aria-label=${h("bulkOperationPreview", "Bulk operation preview")}>
        <span class="sr-only">${t}</span>
        ${e.newEmpty ? m : l`<span class="diff-new" aria-hidden="true">${e.newDisplay}</span>`}
      </div>
    `;
  }
  _renderTaxonomyScalar() {
    var o, a;
    const e = ((o = this.newTaxonomyEntry) == null ? void 0 : o.path) ?? "", t = ((a = this.oldTaxonomyEntry) == null ? void 0 : a.path) ?? "", i = !t, s = !e, r = h("willChangeFromTo", "Will change from {{from}} to {{to}}", {
      from: i ? h("emptyValue", "empty") : t,
      to: s ? h("emptyValue", "empty") : e
    });
    return l`
      <div class="diff-wrap diff-scalar-text" aria-label=${h("bulkOperationPreview", "Bulk operation preview")}>
        <span class="sr-only">${r}</span>
        ${s ? m : l`<span class="diff-new" aria-hidden="true">${e}</span>`}
      </div>
    `;
  }
  render() {
...
ce([
  u({ attribute: !1 })
], H.prototype, "oldValue");
ce([
  u({ attribute: !1 })
], H.prototype, "newValue");
ce([
  u({ attribute: !1 })
], H.prototype, "oldTaxonomyEntry");
ce([
  u({ attribute: !1 })
], H.prototype, "newTaxonomyEntry");
ce([
  u({ attribute: !1 })
], H.prototype, "config");
customElements.define("sfx-bulk-meta-diff-view", H);
export {
  ze as DEFAULT_HUB_API_BASE,
  At as HUB_HEADERS_HINT,
  w as SfxBulkMetadataModal,
  ns as canReachHub,
  ps as clearDependenciesCache,
  ds as clearSchemaCache,
  hs as createTagsAutocomplete,
  fs as createTaxonomyService,
  gs as createUltratagsService,
  bs as deepMergeMeta,
  Be as effectiveHubHeaders,
  us as fetchDependencies,
  ls as fetchMetadataSchema,
  vs as getFilesWithMissingRequired,
  cs as hasCachedDependencies,
  as as hasCachedSchema,
  Ot as hasSessionToken,
  ys as isAssetHasMetadataValue,
  qe as isDefaultHubBase,
  ne as isEmpty,
  Rt as mapValueFromBackend,
  Ke as mapValueToBackend,
  Tt as parseMetadataSchema,
  $t as resolveForFileWithSchema,
  Et as validateField
};
